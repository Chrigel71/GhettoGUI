# GhettoGUI_V8.2.1.2.ps1
#
# --- VERSION V8.2.1.2 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V8.2.1.2 / 28.12.2025"
...
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V53.1 (Fix: Ignore AlreadyExists)
# - FIX REGISTRATION: Ignoriert den Fehler 'vim.fault.AlreadyExists'.
#                     Wenn die VM schon da ist, gilt das jetzt als Erfolg (INFO), nicht als ERROR.
# - RETAIN: Alle vorherigen Features (Upload, Prompt, Size-Calc) bleiben erhalten.
...
# Hilfsvariable: Suchbegriff für Datastore
...
cleanup() {
    ...
    # FIX: Byte-zu-GB Umrechnung mit awk (Nachher)
    STORAGE_INFO=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "esxcli storage filesystem list | grep '${DS_SEARCH_TERM}' | awk '{ printf \"Total: %.2f GB | Frei: %.2f GB\", \$6/1073741824, \$7/1073741824 }'" < /dev/null)
...
# FIX: Byte-zu-GB Umrechnung mit awk (Vorher)
STORAGE_INFO_PRE=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "esxcli storage filesystem list | grep '${DS_SEARCH_TERM}' | awk '{ printf \"Total: %.2f GB | Frei: %.2f GB\", \$6/1073741824, \$7/1073741824 }'" < /dev/null)
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    # --- SCHRITT 1: VMX & NVRAM KOPIEREN (VOR SNAPSHOT) ---
    log "[3/7] Kopiere VMX/NVRAM (Clean State)..."
    VMX_FILE_NAME=$(/usr/bin/basename "${VMX_FULL_PATH}")
    VM_NVRAM_FILE=$(/bin/grep -i 'nvram' "${VMX_FULL_PATH}" | /usr/bin/awk -F '"' '{print $2}')
    
    scp ${SSH_OPTIONS} "${VMX_FULL_PATH}" root@${TARGET_HOST}:"${TARGET_VM_PATH}/${VMX_FILE_NAME}"
    if [ -n "${VM_NVRAM_FILE}" ]; then scp ${SSH_OPTIONS} "${VMX_DIR}/${VM_NVRAM_FILE}" root@${TARGET_HOST}:"${TARGET_VM_PATH}/${VM_NVRAM_FILE}"; fi

    # ===============================================
    # VORBEREITUNG PATCH-SCRIPT (Lokal erstellen)
    # ===============================================
    PATCH_SCRIPT="/tmp/patch_vmx_${UNIQUE_ID}.sh"
    echo "#!/bin/sh" > "${PATCH_SCRIPT}"
    echo "cd '${TARGET_VM_PATH}' || exit 1" >> "${PATCH_SCRIPT}"
    echo "ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\" ! -name \"*~\" | head -n 1)" >> "${PATCH_SCRIPT}"
    echo "RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"" >> "${PATCH_SCRIPT}"
    echo "mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"" >> "${PATCH_SCRIPT}"
    
    # Allgemeine Aufräumarbeiten
    echo "/bin/sed -i '/sched.swap.derivedName/d' \"\${RENAMED_VMX_FILE}\"" >> "${PATCH_SCRIPT}"
    # USER-FIX: UUIDs BEHALTEN!
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        ...
        log "[4/7] Starte ONLINE Replikation (Snapshot)..."; vim-cmd vmsvc/snapshot.create ${VMID} "ghetto-repl-${UNIQUE_ID}" "Ghetto" ${SNAP_MEM} ${SNAP_QUIESCE}; sleep 15
        ...
        while read -r line; do
            ...
            log "  -> Klone Disk ${DISK_IDX} (${DEVICE_ID}): ${SRC_BASENAME_ONLY} -> ${TARGET_FILENAME}"
            /sbin/vmkfstools -i "${SOURCE_DISK_PATH_FOR_CLONE}" -d thin "${DESTINATION_DISK_PATH}" > /dev/null 2>&1
        ...
        while kill -0 ${TAR_PID} >/dev/null 2>&1; do
            ...
            if [ -n "${PROGRESS}" ]; then log "  ...Transfer läuft: ${PROGRESS} von ~${SOURCE_SIZE_TAR}"; fi
            sleep 30
        done
        wait ${TAR_PID}
        if [ $? -ne 0 ]; then log_error "Fehler beim Daten-Transfer!"; OVERALL_STATUS="ERROR"; else log "  -> Transfer abgeschlossen."; fi
        
        rm -rf "${TEMP_CLONE_DIR}"
        /bin/vim-cmd vmsvc/snapshot.removeall ${VMID}

    elif [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "off" ]; then
        # COLD COPY PATH
        log "[4/7] Führe COLD-COPY via SCP aus..."
        ...
        while read -r line; do
            ...
            log "  -> Processing ${DEVICE_ID}: ${SRC_BASENAME_ONLY} -> ${TARGET_DISK_NAME}"
            
            scp ${SSH_OPTIONS} "${SOURCE_DISK_PATH}" root@${TARGET_HOST}:"${TARGET_VM_PATH}/${TARGET_DISK_NAME}" < /dev/null
            scp ${SSH_OPTIONS} -C "${SOURCE_FLAT_PATH}" root@${TARGET_HOST}:"${TARGET_VM_PATH}/${TARGET_FLAT_NAME}" < /dev/null
            if [ $? -ne 0 ]; then CLONE_ERROR=1; log_error "SCP Fehler!"; break; fi
            
            # Update Deskriptor on Target via SSH
            ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "sed -i 's/${FLAT_FILENAME}/${TARGET_FLAT_NAME}/g' '${TARGET_VM_PATH}/${TARGET_DISK_NAME}'"
        ...
        VMX_FILE_NAME=$(/usr/bin/basename "${VMX_FULL_PATH}")
        VM_NVRAM_FILE=$(/bin/grep -i 'nvram' "${VMX_FULL_PATH}" | /usr/bin/awk -F '"' '{print $2}')
        scp ${SSH_OPTIONS} "${VMX_FULL_PATH}" root@${TARGET_HOST}:"${TARGET_VM_PATH}/${VMX_FILE_NAME}" < /dev/null
        if [ -n "${VM_NVRAM_FILE}" ]; then scp ${SSH_OPTIONS} "${VMX_DIR}/${VM_NVRAM_FILE}" root@${TARGET_HOST}:"${TARGET_VM_PATH}/${VM_NVRAM_FILE}" < /dev/null; fi
        
    else
        log_error "FEHLER: Unbekannte Methode."
        OVERALL_STATUS="ERROR"
        continue
    fi
    ...
    log "[6/7] Passe Zieldateien an (Safe Script Method)..."
    
    TARGET_PATCH_SCRIPT="${TARGET_VM_PATH}/patch_vmx.sh"
    scp ${SSH_OPTIONS} "${PATCH_SCRIPT}" root@${TARGET_HOST}:"${TARGET_PATCH_SCRIPT}" < /dev/null
    ssh ${SSH_OPTIONS} root@${TARGET_HOST} "chmod +x '${TARGET_PATCH_SCRIPT}' && '${TARGET_PATCH_SCRIPT}' && rm '${TARGET_PATCH_SCRIPT}'"
    
    if [ $? -eq 0 ]; then log "INFO: VMX-Patch erfolgreich ausgeführt."; else log_error "VMX-Patch fehlgeschlagen!"; fi
    rm -f "${PATCH_SCRIPT}"

    log "[7/7] Registriere VM..."
    # FIX: Intelligente Registrierung (Toleriert "AlreadyExists")
    REG_OUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "vim-cmd solo/registervm '${TARGET_VM_PATH}/${REPLICATED_VM_NAME}.vmx'" 2>&1)
    
    if echo "${REG_OUT}" | grep -qE '^[0-9]+$'; then
        log "  -> VM erfolgreich registriert (ID: ${REG_OUT})."
    elif echo "${REG_OUT}" | grep -q "AlreadyExists"; then
        log "  -> INFO: VM war bereits registriert (Ignoriere 'AlreadyExists' Fehler)."
    else
        # Fallback: Echter Fehler
        if [ $? -eq 0 ]; then
             log "  -> VM registriert (Exit Code 0, aber unerwarteter Output)."
        else
             log_error "Registrierung Warnung/Fehler: ${REG_OUT}"
             OVERALL_STATUS="ERROR"
        fi
    fi
...
if [ -n "${TARGET_VM_PATH}" ]; then
    ...
    DIRECTORY_LISTING_CONTENT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lh '${TARGET_VM_PATH}' 2>/dev/null")
fi
