...
# - FIX: Master-Helper-Skript (V6.2.0.3) mit speziellem Diagnose-Schritt
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V6.2.0.3"
...
$buttonSaveSchedule.Add_Click({
    ...
    try {
        ...
        if ($radioScheduleReplication.Checked) {
            ...
            # ##################################################################
            # HIER BEGINNT DIE LOGIK FÜR DIE REPLIKATION (BLEIBT UNVERÄNDERT)
            # ##################################################################
            Write-GuiLog "Erstelle autarkes Replikations-Skript (ID: $jobId)..."
            ...
            # Das Template für das Replikations-Skript (aus Deiner Datei übernommen)
...
VM_INFO_LINE=$(/bin/vim-cmd vmsvc/getallvms | grep -E "\s$SOURCE_VM_NAME\s+\[")
if [ -z "$VM_INFO_LINE" ]; then log_action "FEHLER: VM '$SOURCE_VM_NAME' konnte nicht gefunden werden."; OVERALL_STATUS="FEHLGESCHLAGEN"; continue; fi
VMID=$(echo "$VM_INFO_LINE" | /bin/awk '{print $1}')
DATASTORE=$(echo "$VM_INFO_LINE" | /bin/sed -n 's/.*\[\(.*\)\]\s.*/\1/p')
VMX_FILE_PATH=$(echo "$VM_INFO_LINE" | /bin/sed -n 's/.*\]\s*\(.*\.vmx\).*/\1/p')
VMX_FULL_PATH="/vmfs/volumes/$DATASTORE/$VMX_FILE_PATH"
...
'@
            ...
            # ################################################################
            # HIER ENDET DIE LOGIK FÜR DIE REPLIKATION
            # ################################################################
            ...
            $rotation = $textboxRotation.Text; $diskFormat = $comboboxDiskFormat.SelectedItem.ToString(); $snapMem = if ($checkboxSnapMem.Checked) { 1 } else { 0 }; $snapQuiesce = if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 };
            ...
            $emailUser = $textboxEmailUser.Text; $emailPass = $textboxEmailPassword.Text; $emailSubjectTemplate = $textboxEmailSubject.Text;
            $emailSubject = $emailSubjectTemplate.Replace('%h', $Global:ESXiConnectedHostName)
            $emailBinPath = "$ghettoPath/sendmail"
            
            # KORREKTUR: Alle Pfade und Strings in der Config werden nun konsistent in doppelte Anführungszeichen gesetzt.
            $confLines = @(
...
function Get-BackupJobLog {
    ...
    $findLatestLogCmd = "ls -t $logDir/backup-run-*.log $logDir/ghettoVCB-last_manual_run.log 2>/dev/null | head -n 1"
