# GhettoGUI_V7.4.0.ps1
#
# --- VERSION V7.4.0 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V7.4.0 / 14.09.2025"
...
for SOURCE_VM_NAME in ${VM_LIST}; do
    ...
    log "initiate backup for ${CLONED_VM_NAME}" # Wichtig für E-Mail-Parser
...
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V43.2 (Final Subshell/Scope Fix)
# - Groups loop and cleanup in a subshell to preserve variables for the final report.
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
cleanup() { EXIT_CODE=$?; if [ "${EXIT_CODE}" -ne 0 ] && [ "${OVERALL_STATUS}" = "OK" ]; then OVERALL_STATUS="ERROR"; fi; log_raw "--- ENDE DES DETAILLOGS ---"; END_TIME_S=$(date '+%Y-%m-%d %H:%M:%S'); log_raw "Endzeit: ${END_TIME_S}"; END_TIME=$(date +%s); DURATION=$((END_TIME - START_TIME)); MINUTES=$((DURATION / 60)); SECONDS=$((DURATION % 60)); DURATION_STRING="${MINUTES} Minuten, ${SECONDS} Sekunden"; send_email_notification "${DURATION_STRING}"; log "====== MASTERHELPER REPLICATION HELPER BEENDET (ID: ${UNIQUE_ID}) ======"; }
...
# --- KORREKTUR: Die gesamte Logik wird in eine Sub-Shell gekapselt, um die Variablen zu erhalten ---
(
    trap cleanup EXIT
    ...
    trap - EXIT # Deaktiviere den Fehler-Trap für den sauberen Abschluss
)
# Der Aufruf der cleanup-Funktion findet jetzt implizit durch den trap am Ende der Subshell statt
'@
