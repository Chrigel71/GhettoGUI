...
# - FIX: Master-Helper-Skript (V4.6.0) mit speziellem Diagnose-Schritt
...
$form.Text = "ESXi & GhettoVCB Manager V4.6.0"
...
# --- Logik für Assistenten-Buttons (Host-zu-Host Setup) ---
$buttonGenerateKey.Add_Click({
    $sourceIp = $textboxSourceIp.Text; $targetIp = $textboxTargetIp.Text; if ([string]::IsNullOrWhiteSpace($sourceIp) -or [string]::IsNullOrWhiteSpace($targetIp)) { [System.Windows.Forms.MessageBox]::Show("Bitte zuerst die IPs für Quelle und Ziel in die Felder oben eintragen.", "Fehlende Eingabe", "OK", "Warning"); return }
    Write-ConsoleLog $sourceConsoleOutput "Schritt 1: Bereite alles auf dem PC vor..." ([System.Drawing.Color]::Cyan)
    $sshConsoleForm.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    try {
        # ... (komplette Logik für buttonGenerateKey.Add_Click von vorher)
    } catch { #...
    } finally { $sshConsoleForm.Cursor = [System.Windows.Forms.Cursors]::Default }
})
$buttonInjectKey.Add_Click({
    ...
    $sourceCommands = "mkdir -p /.ssh`nchmod 700 /.ssh`nchmod 600 /.ssh/id_ecdsa"; Write-ConsoleLog $sourceConsoleOutput "---" ([System.Drawing.Color]::Gray); Write-ConsoleLog $sourceConsoleOutput "AUFGABE: Führe diese Befehle im QUELL-HOST (links) aus:" ([System.Drawing.Color]::Yellow); Write-ConsoleLog $sourceConsoleOutput $sourceCommands ([System.Drawing.Color]::Lime)
    $targetCommands = "chmod 600 /etc/ssh/keys-root/authorized_keys"; Write-ConsoleLog $targetConsoleOutput "---" ([System.Drawing.Color]::Gray); Write-ConsoleLog $targetConsoleOutput "AUFGABE: Führe diesen Befehl im ZIEL-HOST (rechts) aus:" ([System.Drawing.Color]::Yellow); Write-ConsoleLog $targetConsoleOutput $targetCommands ([System.Drawing.Color]::Lime)
    [System.Windows.Forms.MessageBox]::Show("Die notwendigen `chmod`-Befehle wurden in die jeweiligen Konsolenfenster geschrieben.`n`nBitte kopiere sie von dort in die Eingabezeile und führe sie aus.", "Schritt 2: Berechtigungen", "OK", "Information")
})
$buttonTestKey.Add_Click({
    if (-not ($Global:sourceConsoleSession -and $Global:sourceConsoleSession.Connected)) { [System.Windows.Forms.MessageBox]::Show("Bitte zuerst den Quell-Host verbinden.", "Fehler", "OK", "Warning"); return }
    $targetIp = $textboxTargetIp.Text; if ([string]::IsNullOrWhiteSpace($targetIp)) { [System.Windows.Forms.MessageBox]::Show("Bitte die IP des Ziel-Hosts im rechten Feld eingeben.", "Fehler", "OK", "Warning"); return }
    Write-ConsoleLog $sourceConsoleOutput "---" ([System.Drawing.Color]::Gray); Write-ConsoleLog $sourceConsoleOutput "Führe finalen Verbindungstest von Quelle zu Ziel aus..." ([System.Drawing.Color]::Yellow)
    $testCmd = "ssh -i /.ssh/id_ecdsa -o 'StrictHostKeyChecking=no' root@$targetIp `"echo 'ERFOLG! Die Host-zu-Host Verbindung steht!'`""; Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $testCmd -outputBox $sourceConsoleOutput
})
...
# Button 1: Erstellt die Schlüssel auf dem PC und eine .bat-Datei für den Transfer
$buttonGenerateKey.Add_Click({
    ...
    $sshConsoleForm.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    try {
        $keyFile = "C:\temp\esxi_final_key"
        # Der Inhalt für die Batch-Datei, die alles auf dem PC ausführt
        $batFileContent = @"
@echo off
cls
echo.
...
echo ====================================================================
echo.
echo Dieses Skript wird Dich jetzt durch die Einrichtung auf Deinem PC fuehren.
echo.
echo --- SCHRITT A: SSH Schluessel auf diesem PC erstellen ---
echo.
ssh-keygen -t ecdsa -f "$keyFile"
IF %ERRORLEVEL% NEQ 0 (
    echo. & echo FEHLER: ssh-keygen fehlgeschlagen. Ist der OpenSSH-Client installiert? & pause & exit /b
)
echo. & echo -> Schluesselpaar erfolgreich in C:\temp erstellt. & echo.
...
rem --- NEUER AUFRÄUM-ABSCHNITT ---
echo --- SCHRITT C: Raeume temporaere Dateien auf ---
del `"$($Global:ScriptPath)\Bitte`" /q /f > nul 2>&1
del `"$($Global:ScriptPath)\Kopiere`" /q /f > nul 2>&1
del `"$($Global:ScriptPath)\Schluesselpaar`" /q /f > nul 2>&1
echo -> Ueberfluessige Dateien im Programmverzeichnis entfernt.
rem --- ENDE AUFRÄUM-ABSCHNITT ---
...
"@
        ...
        # Set-Content mit -Encoding Oem stellt sicher, dass Umlaute etc. in der CMD korrekt dargestellt werden
        Set-Content -Path $batFilePath -Value $batFileContent -Encoding Oem

        # Starte die Batch-Datei in einem neuen Fenster
...
# Button 2: Schreibt die Befehle zum Setzen der Berechtigungen in die Konsolen
$buttonInjectKey.Add_Click({
    if (-not ($Global:sourceConsoleSession -and $Global:sourceConsoleSession.Connected)) { [System.Windows.Forms.MessageBox]::Show("Bitte zuerst den Quell-Host verbinden.", "Fehler", "OK", "Warning"); return }
    if (-not ($Global:targetConsoleSession -and $Global:targetConsoleSession.Connected)) { [System.Windows.Forms.MessageBox]::Show("Bitte zuerst den Ziel-Host verbinden.", "Fehler", "OK", "Warning"); return }

    # Befehle für den Quell-Host vorbereiten und anzeigen
    $sourceCommands = "mkdir -p /.ssh`nchmod 700 /.ssh`nchmod 600 /.ssh/id_ecdsa"
    Write-ConsoleLog $sourceConsoleOutput "---" ([System.Drawing.Color]::Gray)
    Write-ConsoleLog $sourceConsoleOutput "AUFGABE: Führe diese Befehle im QUELL-HOST (links) aus:" ([System.Drawing.Color]::Yellow)
    Write-ConsoleLog $sourceConsoleOutput $sourceCommands ([System.Drawing.Color]::Lime)

    # Befehle für den Ziel-Host vorbereiten und anzeigen (jetzt mit korrigierter Variable)
    $targetCommands = "chmod 600 /etc/ssh/keys-root/authorized_keys"
    Write-ConsoleLog $targetConsoleOutput "---" ([System.Drawing.Color]::Gray)
    Write-ConsoleLog $targetConsoleOutput "AUFGABE: Führe diesen Befehl im ZIEL-HOST (rechts) aus:" ([System.Drawing.Color]::Yellow)
    Write-ConsoleLog $targetConsoleOutput $targetCommands ([System.Drawing.Color]::Lime)

    [System.Windows.Forms.MessageBox]::Show("Die notwendigen `chmod`-Befehle wurden in die jeweiligen Konsolenfenster geschrieben.`n`nBitte kopiere sie von dort in die Eingabezeile und führe sie aus.", "Schritt 2: Berechtigungen", "OK", "Information")
})
# Button 3: Führt den finalen Verbindungstest aktiv aus
$buttonTestKey.Add_Click({
    if (-not ($Global:sourceConsoleSession -and $Global:sourceConsoleSession.Connected)) {
