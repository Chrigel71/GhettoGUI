# GhettoGUI_8.9.1.ps1
#
# --- VERSION 8.9.1 (Final Stable Release by Gemini & Chrigel#71)
...
# - NEU: Proxmox 9.0
# - ghettoVCB_patch_V9.8.2.sh & sendmail_V9.5.py 24.02.2026
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager 8.9.1 / 24.02.2026  https://files.drucksprint.ch/ghetto"
        ...
        if ($confirmInstall -eq [System.Windows.Forms.DialogResult]::Yes) {
            try {
                ...
                if (-not (Get-PackageProvider -Name NuGet -ErrorAction SilentlyContinue)) {
                    ...
                    Install-PackageProvider -Name NuGet -MinimumVersion 2.8.7.701 -Force | Out-Null
...
$buttonRestore.ForeColor = [System.Drawing.Color]::Darkblue
...
# Die technischen Werte bleiben gleich (So=0), damit Cron korrekt funktioniert
$days = @{ "So" = 0; "Mo" = 1; "Di" = 2; "Mi" = 3; "Do" = 4; "Fr" = 5; "Sa" = 6 }; 
$dayCheckboxX = $schedulesGcOffsetX + 45

# NEUE LOGIK: Wir sortieren nach einer benutzerdefinierten Liste, um Mo an den Anfang zu stellen
$customOrder = "Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"

foreach ($dName in $customOrder) { 
    $dValue = $days[$dName]
    $cb = New-Object System.Windows.Forms.CheckBox; 
    $cb.Text = $dName; 
    $cb.Tag = $dValue; # Der technische Wert (0 für So) bleibt erhalten
    $cb.Location = New-Object System.Drawing.Point($dayCheckboxX, $schedulesGcOffsetY); 
    $cb.AutoSize = $true; 
    $checkboxDays[$dName] = $cb; 
    $dayCheckboxX += 43 
}
$schedulesGcOffsetY += 30
...
$buttonGetEsxiTime = New-Object System.Windows.Forms.Button; $buttonGetEsxiTime.Text = "ESXi-Zeit"; $buttonGetEsxiTime.Location = New-Object System.Drawing.Point($schedulesGcOffsetX, $schedulesGcOffsetY); $buttonGetEsxiTime.Size = New-Object System.Drawing.Size(90, 23)
$buttonSaveSchedule = New-Object System.Windows.Forms.Button; $buttonSaveSchedule.Text = "Zeitplan speichern"; $buttonSaveSchedule.Location = New-Object System.Drawing.Point(($buttonGetEsxiTime.Right + 10), $schedulesGcOffsetY); $buttonSaveSchedule.Size = New-Object System.Drawing.Size(120, 23); $buttonSaveSchedule.ForeColor = [System.Drawing.Color]::DarkRed
$buttonshowSchedule = New-Object System.Windows.Forms.Button; $buttonshowSchedule.Text = "Zeitplan anzeigen"; $buttonshowSchedule.Location = New-Object System.Drawing.Point(($buttonSaveSchedule.Right + 10), $schedulesGcOffsetY); $buttonshowSchedule.Size = New-Object System.Drawing.Size(120, 23); $buttonshowSchedule.ForeColor = [System.Drawing.Color]::Blue


# Alle Controls zur Gruppe hinzufügen
$allScheduleControls = New-Object System.Collections.ArrayList
[void]$allScheduleControls.AddRange(@($labelScheduleTime, $textboxScheduleHour, $buttonshowSchedule, $labelScheduleSeparator, $textboxScheduleMinute, $comboRepeatMode, $labelScheduleDays, $radioScheduleBackup, $radioScheduleReplication, $radioScheduleRestoreClone, $labelUtcInfo, $buttonSaveSchedule, $buttonGetEsxiTime, $labelEsxiTime))
...
$buttonStartBackup = New-Object System.Windows.Forms.Button; $buttonStartBackup.Text = "GhettoVCB Backup starten man."; $buttonStartBackup.Location = New-Object System.Drawing.Point($column2X, $currentY_Col2); $buttonStartBackup.Size = New-Object System.Drawing.Size(180, 25)
...
$buttonCancelBackup = New-Object System.Windows.Forms.Button; $buttonCancelBackup.Text = "Alle Backups/Repl. abbrechen"; $buttonCancelBackup.Location = New-Object System.Drawing.Point($column2X, $currentY_Col2); $buttonCancelBackup.Size = New-Object System.Drawing.Size(180, 25); $buttonCancelBackup.Enabled = $true
...
$jobParams = @{
    ...
    VmSuffix                = $textboxRepSuffix.Text.Trim()
...
# --- Spalte 2: Host & Job Management (Optimierte Version V8.7) ---
...
# Button-Größe vereinheitlicht (beide 165,25)
$buttonRebootHost = New-Object System.Windows.Forms.Button; $buttonRebootHost.Text = "Host NEU STARTEN"; $buttonRebootHost.Location = New-Object System.Drawing.Point(15, 55); $buttonRebootHost.Size = New-Object System.Drawing.Size(165, 25); $buttonRebootHost.ForeColor = [System.Drawing.Color]::DarkRed

# NEU: Der neue Button rechts von "Host neu starten"
$buttonIntegrityCheck = New-Object System.Windows.Forms.Button; $buttonIntegrityCheck.Text = "Storage & VM-Integrität"; $buttonIntegrityCheck.Location = New-Object System.Drawing.Point(190, 55); $buttonIntegrityCheck.Size = New-Object System.Drawing.Size(165, 25); $buttonIntegrityCheck.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold); $buttonIntegrityCheck.ForeColor = [System.Drawing.Color]::DarkBlue
...
# SSH-Workflow & USB (Positionen beibehalten)
...
$groupManagement.Controls.AddRange(@($buttonIntegrityCheck, $buttonConsolidatePowerCLI, $buttonRemoveUsbWizard, $buttonMaintenanceEnter, $buttonWipeUsb, $buttonMaintenanceExit, $buttonRebootHost, $buttonKillOrphans, $buttonGenerateSshKey, $buttonRegisterSshKey, $buttonMountUsb))
...
# =====================================================================================
# --- START OF BLOCK TO COPY (Kill-Button V9.5 - Direct Pattern Fix) ---
# =====================================================================================

$buttonKillOrphans.Add_Click({
    Write-ConsoleLog $sourceConsoleOutput "--- Starte universelle Bereinigung (V9.5 FINAL) ---" ([System.Drawing.Color]::Yellow)
    if (-not ($Global:sourceConsoleSession -and $Global:sourceConsoleSession.Connected)) {
    ...
    $confirmResult = [System.Windows.Forms.MessageBox]::Show(
        "WARNUNG: Dieser Vorgang analysiert laufende Prozesse und beendet GhettoVCB-Jobs, Klon-Vorgänge (vmkfstools) und Replikationen (scp). Die VM kann beschädigt werden. Das Backup wird unvollständig sein."+
        "`n`nSind Sie sicher?",
        "Bereinigung starten",
        [System.Windows.Forms.MessageBoxButtons]::YesNo,
    ...
    if ($confirmResult -ne 'Yes') {
        Write-ConsoleLog $sourceConsoleOutput "Abbruch durch Benutzer." ([System.Drawing.Color]::Gray)
        return
    }

    $sshConsoleForm.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    try {
        # --- SCHRITT 1: Intelligente Prozess-Analyse ---
        Write-ConsoleLog $sourceConsoleOutput "`n-> 1. Analysiere laufende Prozesse..." ([System.Drawing.Color]::Cyan)
        
        $killCmd = @'
# Wir suchen direkt im Befehl, ohne Variable, um "Echo-Fehler" zu vermeiden.
# Sucht nach: ghettoVCB, scheduled_replication, ghetto_clone, ghetto_restore, launcher, master_replication, vmkfstools, scp
# grep -v grep: Filtert den grep selbst
# grep -v $$: Filtert die aktuelle Shell
# awk '{print $1}': Holt nur die PID

CANDIDATE_PIDS=$(ps -c | grep -E "ghettoVCB|scheduled_replication|ghetto_clone|ghetto_restore|launcher|master_replication|vmkfstools|scp" | grep -v grep | grep -v $$ | awk '{print $1}')

if [ -n "$CANDIDATE_PIDS" ]; then
    echo "Kandidaten (PIDs) gefunden: $CANDIDATE_PIDS"
    echo "Starte Detail-Analyse..."
    
    for PID in $CANDIDATE_PIDS; do
        # Wir validieren, ob es eine echte Zahl ist
        case $PID in
             ''|*[!0-9]*) echo " -> Ignoriere ungültige PID: $PID"; continue ;;
        esac

        # Wir holen uns die volle Befehlszeile zu dieser PID
        FULL_CMD=$(ps | grep "$PID" | grep -v grep)
        
        # Prüfung 1: Ist der Prozess überhaupt noch da?
        if [ -z "$FULL_CMD" ]; then
            echo " -> Prozess $PID hat sich bereits beendet (Geister-Prozess)."
            continue
        fi

        # Prüfung 2: Ist es unser eigener Such-Befehl?
        IS_SEARCHER=$(echo "$FULL_CMD" | grep -E "grep|ps -c")
        
        if [ -n "$IS_SEARCHER" ]; then
            echo " -> IGNORIERE PID $PID: Ist Teil der Such-Logik."
        else
            echo " -> TREFFER: Beende Prozess $PID"
            kill -9 "$PID"
        fi
    done
    echo "INFO: Prozess-Analyse abgeschlossen."
else
    echo "INFO: Keine relevanten Prozesse gefunden."
fi
'@
        ...
        # --- SCHRITT 2: Snapshots (Unverändert) ---
        Write-ConsoleLog $sourceConsoleOutput "`n-> 2. Suche und entferne verbliebene GhettoGUI-Snapshots..." ([System.Drawing.Color]::Cyan)
        $cleanupSnapshotsCmd = @'
vim-cmd vmsvc/getallvms | awk '{print $1}' | grep -E '^[0-9]+' | while read vmid; do
    if vim-cmd vmsvc/snapshot.get $vmid | grep -q "ghettoVCB-snapshot"; then
        VM_NAME=$(vim-cmd vmsvc/getallvms | grep "^$vmid " | awk '{print $2}')
        echo "INFO: Entferne Ghetto-Snapshot von VM ''$VM_NAME'' (ID: $vmid)..."
...
'@
        ...
        # --- SCHRITT 3: Klon-Ordner (Unverändert) ---
        Write-ConsoleLog $sourceConsoleOutput "`n-> 3. Suche und lösche temporäre Klon-Verzeichnisse..." ([System.Drawing.Color]::Cyan)
        $cleanupClonesCmd = @'
COUNT=$(find /vmfs/volumes -maxdepth 2 -type d -name "ghetto_clone_*" | wc -l)
if [ "$COUNT" -gt 0 ]; then
    echo "INFO: Lösche $COUNT verwaiste Klon-Verzeichnisse..."
    find /vmfs/volumes -maxdepth 2 -type d -name "ghetto_clone_*" -exec rm -rf {} +
else
    echo "INFO: Keine verwaisten Klon-Verzeichnisse gefunden."
fi
'@
        $cleanupClonesCmd = $cleanupClonesCmd.Replace("`r`n", "`n")
        Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $cleanupClonesCmd -outputBox $sourceConsoleOutput

        # --- SCHRITT 4: Temp-Dateien (Unverändert) ---
        Write-ConsoleLog $sourceConsoleOutput "`n-> 4. Räume /tmp auf..." ([System.Drawing.Color]::Cyan)
        $cleanupTmpCmd = "rm -rf /tmp/ghettoVCB.pid /tmp/ghettoVCB.work /tmp/vmware-root/ghettoVCB.pid; " +
                         "rm -rf /tmp/sendmail.py /tmp/ghettoVCB_patch.sh; " +
                         "rm -rf /tmp/scheduled_replication_*.sh /tmp/ghetto_clone_*.sh /tmp/ghetto_restore_*.sh /tmp/launcher_*.sh /tmp/master_replication_*.sh; " +
                         "rm -rf /tmp/ghetto_pipe_error_* /tmp/ghetto_*.lock /var/run/ghettovcb.lock"
        
        Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $cleanupTmpCmd -outputBox $sourceConsoleOutput
...
# =====================================================================================
# --- FINALE VERSION V8.8.5 (Integrität, VAAI, Storage & HP-Controller Check) ---
# =====================================================================================


$buttonIntegrityCheck.Add_Click({
    if (-not ($Global:sourceConsoleSession -and $Global:sourceConsoleSession.Connected)) {
        [System.Windows.Forms.MessageBox]::Show("Bitte zuerst den Quell-Host verbinden.", "Verbindung fehlt", "OK", "Warning")
        return
    }

    $diagPopup = New-Object System.Windows.Forms.Form
    $diagPopup.Text = "Erweiterte Diagnose & Optimierung (ESXi 8)"
    $diagPopup.Size = New-Object System.Drawing.Size(500, 530) # Höhe angepasst für 4. Button
    $diagPopup.StartPosition = "CenterParent"
    $diagPopup.FormBorderStyle = 'FixedDialog'
    $diagPopup.FormBorderStyle = 'Sizable'
    $diagPopup.MaximizeBox = $true
    $diagPopup.MinimizeBox = $true
    $diagPopup.AutoScroll = $true # Aktiviert die AutoScroll-Funktion

$toolTip = New-Object System.Windows.Forms.ToolTip

    $diagLabel = New-Object System.Windows.Forms.Label; $diagLabel.Text = "Wartungsfunktionen für $($textboxSourceIp.Text):"; $diagLabel.Location = New-Object System.Drawing.Point(20, 15); $diagLabel.Size = New-Object System.Drawing.Size(450, 25); $diagLabel.Font = New-Object System.Drawing.Font("Segoe UI", 10, [System.Drawing.FontStyle]::Bold)

    # --- BUTTONS ---
    $btnLockCheck = New-Object System.Windows.Forms.Button; $btnLockCheck.Text = "KRITISCHE FEHLER PRÜFEN (Lock & Snaps)"; $btnLockCheck.Location = New-Object System.Drawing.Point(20, 50); $btnLockCheck.Size = New-Object System.Drawing.Size(210, 35); $btnLockCheck.ForeColor = [System.Drawing.Color]::DarkRed; $btnLockCheck.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    $btnSnapMonitor = New-Object System.Windows.Forms.Button; $btnSnapMonitor.Text = "Snapshot Monitor"; $btnSnapMonitor.Location = New-Object System.Drawing.Point(250, 50); $btnSnapMonitor.Size = New-Object System.Drawing.Size(210, 35); $btnSnapMonitor.ForeColor = [System.Drawing.Color]::DarkRed; $btnSnapMonitor.Font = New-Object System.Drawing.Font("Segoe UI", 9, [System.Drawing.FontStyle]::Bold)
    
	
    $btnVaaiCheck = New-Object System.Windows.Forms.Button; $btnVaaiCheck.Text = "VAAI-STATUS & DIENST-CHECK"; $btnVaaiCheck.Location = New-Object System.Drawing.Point(20, 95); $btnVaaiCheck.Size = New-Object System.Drawing.Size(210, 35); $btnVaaiCheck.ForeColor = [System.Drawing.Color]::DarkGreen
    $btnVaaiCheck1 = New-Object System.Windows.Forms.Button; $btnVaaiCheck1.Text = "VAAI-STATUS & ISCSI-CHECK"; $btnVaaiCheck1.Location = New-Object System.Drawing.Point(250, 95); $btnVaaiCheck1.Size = New-Object System.Drawing.Size(210, 35); $btnVaaiCheck1.ForeColor = [System.Drawing.Color]::Green

    $btnStorageOpt = New-Object System.Windows.Forms.Button; $btnStorageOpt.Text = "STORAGE-OPTIMIERUNG (iSCSI & Disk-Retries)"; $btnStorageOpt.Location = New-Object System.Drawing.Point(20, 140); $btnStorageOpt.Size = New-Object System.Drawing.Size(440, 35); $btnStorageOpt.ForeColor = [System.Drawing.Color]::DarkBlue

    # NEU: HP-Controller Check
    $btnHpCheck = New-Object System.Windows.Forms.Button; $btnHpCheck.Text = "HP ARRAY(Controller & SSD Status)"; $btnHpCheck.Location = New-Object System.Drawing.Point(20, 185); $btnHpCheck.Size = New-Object System.Drawing.Size(210, 35); $btnHpCheck.ForeColor = [System.Drawing.Color]::SaddleBrown
    $btnILOCheck = New-Object System.Windows.Forms.Button; $btnILOCheck.Text = "HP Ilo-CHECK"; $btnILOCheck.Location = New-Object System.Drawing.Point(250, 185); $btnILOCheck.Size = New-Object System.Drawing.Size(210, 35); $btnILOCheck.ForeColor = [System.Drawing.Color]::SaddleBrown

    $diagOutput = New-Object System.Windows.Forms.RichTextBox
    $diagOutput.Location = New-Object System.Drawing.Point(20, 235)
    $diagOutput.Size = New-Object System.Drawing.Size(440, 230)
    $diagOutput.BackColor = [System.Drawing.Color]::Black
    $diagOutput.ForeColor = [System.Drawing.Color]::White
    $diagOutput.ReadOnly = $true
    $diagOutput.Font = New-Object System.Drawing.Font("Consolas", 9)
    $diagOutput.Multiline = $true
    $diagOutput.ScrollBars = [System.Windows.Forms.RichTextBoxScrollBars]::Both

    # Das sorgt dafür, dass die Box mit dem Fenster mitwächst:
    $diagOutput.Anchor = "Top, Bottom, Left, Right"


    # $diagOutput = New-Object System.Windows.Forms.RichTextBox; $diagOutput.Location = New-Object System.Drawing.Point(20, 235); $diagOutput.Size = New-Object System.Drawing.Size(440, 230); $diagOutput.BackColor = [System.Drawing.Color]::Black; $diagOutput.ForeColor = [System.Drawing.Color]::White; $diagOutput.ReadOnly = $true; $diagOutput.FormBorderStyle = 'Sizable'; $diagOutput.AutoScroll = $true; $diagOutput.Font = New-Object System.Drawing.Font("Consolas", 9)
    # $diagOutput.Anchor = [System.Windows.Forms.AnchorStyles]::Top -bor 
    #                [System.Windows.Forms.AnchorStyles]::Bottom -bor 
    #                 [System.Windows.Forms.AnchorStyles]::Left -bor 
    #                 [System.Windows.Forms.AnchorStyles]::Right

    # --- LOGIK 1: Lock & Error Check (Bestehend) ---
    $btnLockCheck.Add_Click({
        $diagOutput.Clear(); $diagOutput.SelectionColor = [System.Drawing.Color]::Cyan; $diagOutput.AppendText("-> Prüfe auf kritische Fehler...`n")
        $lockCmd = "find /vmfs/volumes/ -name '*.vmdk' -not -name '*-flat.vmdk' -not -name '*-delta.vmdk' -not -name '*-sesparse.vmdk' | head -n 5 | xargs -n 1 vmkfstools -D"
        Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $lockCmd -outputBox $sourceConsoleOutput
        $logCmd = "grep -iE 'failed|error|locked|ScsiDeviceIO|ScsiReservation' /var/log/vmkernel.log | tail -n 20"
        $dummyBox = New-Object System.Windows.Forms.RichTextBox; Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $logCmd -outputBox $dummyBox
        if ($dummyBox.Text.Trim().Length -gt 5) {
            $diagOutput.SelectionColor = [System.Drawing.Color]::OrangeRed; $diagOutput.AppendText("!! KRITISCHE EINTRÄGE GEFUNDEN !!`n"); $diagOutput.SelectionColor = [System.Drawing.Color]::White; $diagOutput.AppendText($dummyBox.Text)
        } else {
            $diagOutput.SelectionColor = [System.Drawing.Color]::LawnGreen; $diagOutput.AppendText("OK: Keine kritischen Fehler im Log.`n")
        }
    })

        # --- LOGIK 2: VAAI Check (Bestehend) ---
    $btnVaaiCheck.Add_Click({
        $diagOutput.Clear(); $diagOutput.SelectionColor = [System.Drawing.Color]::Cyan; $diagOutput.AppendText("-> Analysiere VAAI NAS-Dienst...`n")
        $vaaiStatusCmd = 'esxcli software vib list | grep vaa'
		Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $vaaiStatusCmd -outputBox $diagOutput
		$vaaiStatusCmd = 'ps | grep vaai-nasd | grep -v grep; esxcli storage core device vaai list 2>/dev/null | grep -A 5 "VAAI Plugin Name: VMW_VAAIP_NAS" | grep -E "Device|Status"'
		Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $vaaiStatusCmd -outputBox $diagOutput
		if ($diagOutput.Text -match "vaai-nasd") {
            $diagOutput.SelectionColor = [System.Drawing.Color]::LawnGreen; $diagOutput.AppendText("`n[OK] VAAI-Dienst läuft.`n")
        } else {
            $diagOutput.SelectionColor = [System.Drawing.Color]::Red; $diagOutput.AppendText("`n[WARNUNG] VAAI-NASD Dienst nicht aktiv!`n")
        }
    })

        # --- LOGIK 2.5: VAAI Check  ---
    $btnVaaiCheck1.Add_Click({
        $diagOutput.Clear(); $diagOutput.SelectionColor = [System.Drawing.Color]::Cyan; $diagOutput.AppendText("-> ISCSI Hardwarebeschleunigung `n")
        $vaaiStatusCmd = 'esxcli storage core device vaai status get'
        Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $vaaiStatusCmd -outputBox $diagOutput
        if ($diagOutput.Text -match "vaai-nasd") {
            $diagOutput.SelectionColor = [System.Drawing.Color]::LawnGreen; $diagOutput.AppendText("`n[OK] VAAI-Dienst läuft.`n")
        } else {
            $diagOutput.SelectionColor = [System.Drawing.Color]::Red; $diagOutput.AppendText("`n[Information] VAAI-Dienst läuft NICHT, Synology VAAI Plugin installieren`n")
        }
    })

    # --- LOGIK 3: Storage Optimierung (Erfolgreich Finalisiert) ---
    $btnStorageOpt.Add_Click({
        $diagOutput.Clear()
        if ([System.Windows.Forms.MessageBox]::Show("ESXi Storage-Parameter für vmhba64 finalisieren?", "Optimierung", [System.Windows.Forms.MessageBoxButtons]::YesNo) -eq 'Yes') {
            
            # Korrektur der Abfrage-Syntax für ESXi 8
            # 'get' benötigt kein -k, wir greifen das Ergebnis mit grep ab
            $optCmd = "esxcfg-advcfg -s 1000 /Disk/DiskRetryPeriod; esxcfg-advcfg -s 100 /Disk/ResetMaxRetries; esxcli iscsi adapter list | grep 'vmhba' | awk '{print `$1}' | xargs -I {} esxcli iscsi adapter param set -A {} -k LoginTimeout -v 30; echo '--- PRÜFUNG DER WERTE ---'; esxcfg-advcfg -g /Disk/DiskRetryPeriod; esxcfg-advcfg -g /Disk/ResetMaxRetries; esxcli iscsi adapter param get -A vmhba64 | grep LoginTimeout"
            
            Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $optCmd -outputBox $diagOutput
            $diagOutput.AppendText("`n-> Optimierung für vmhba64 erfolgreich abgeschlossen.`n")
        }
    })
	
    # --- KORREKTUR: LOGIK 4: HP-Controller Check (Einzeiler für ESXi Shell) ---
    $btnHpCheck.Add_Click({
        $diagOutput.Clear(); $diagOutput.SelectionColor = [System.Drawing.Color]::Cyan; $diagOutput.AppendText("-> Starte HP Smart-Array Diagnose...`n")
        
        # Wir nutzen den Pfad, von dem wir wissen, dass er bei dir funktioniert.
        # Durch '||' wird der alternative Pfad nur versucht, wenn der erste fehlschlägt.
        $hpCmd = "/opt/smartstorageadmin/ssacli/bin/ssacli ctrl all show config detail || /opt/hp/hpssacli/bin/hpssacli ctrl all show config detail"
        
        Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $hpCmd -outputBox $diagOutput
        $diagOutput.ScrollToCaret()
    })
	
    # --- LOGIK 5: Hardware-Management (Robust-Fix) ---
    $btnILOCheck.Add_Click({
        $diagOutput.Clear()
        $diagOutput.AppendText("-> Prüfe HP iLO Treiber & Sensoren...`n")
        
        # -i bei grep ignoriert Groß/Kleinschreibung
        # Wir listen alle Module auf, die 'ilo' im Namen haben
        $iloCmd = "echo '--- TREIBER STATUS ---'; esxcli system module list | grep -i 'ilo'; echo ''; echo '--- iLO FIRMWARE ---'; esxcli hardware ipmi mc get | grep -i 'Revision'; echo ''; echo '--- SENSOREN (Top 5) ---'; esxcli hardware ipmi sdr list | grep -E 'Fan|Temp' | head -n 5"
        
        Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $iloCmd -outputBox $diagOutput
        
        # Logik-Auswertung: Wir prüfen nun einfach, ob Sensordaten da sind
        if ($diagOutput.Text -match "degrees C") {
            $diagOutput.SelectionColor = [System.Drawing.Color]::Green
            $diagOutput.AppendText("`n[OK] Kommunikation stabil. Sensordaten werden empfangen.`n")
        } else {
            $diagOutput.SelectionColor = [System.Drawing.Color]::Red
            $diagOutput.AppendText("`n[WARNUNG] Keine Hardware-Daten empfangen. iLO-Status prüfen!`n")
        }
        
        $diagOutput.SelectionColor = [System.Drawing.Color]::Black
        $diagOutput.AppendText("`n-> Check abgeschlossen.`n")
    })
	
    # --- LOGIK 6: Snapshot-Monitor (Ultra-Stable Version) ---
    $btnSnapMonitor.Add_Click({
        $diagOutput.Clear()
        $diagOutput.AppendText("-> Suche nach Snapshots (Rohwerte in MB)...`n")
        
                $snapCmd = "echo '--- Top Snapshots (Groesse in MB) ---'; find /vmfs/volumes/ -name '*0000*.vmdk' -not -name '*ctk*' -exec du -m {} + 2>/dev/null | sort -rn | head -n 10"
        
        Invoke-DualConsoleCommand -session $Global:sourceConsoleSession -command $snapCmd -outputBox $diagOutput
        
        # Manuelle Prüfung in der GUI Logik statt in der Shell
        if ($diagOutput.Text -match "[0-9]{5,}") { # Findet Zahlen mit 5 oder mehr Stellen (>10GB)
            $diagOutput.SelectionColor = [System.Drawing.Color]::Orange
            $diagOutput.AppendText("`n[HINWEIS] Grosse Snapshots erkannt (>10GB).`n")
        }
    })

    $diagPopup.Controls.AddRange(@($diagLabel, $btnLockCheck, $btnSnapMonitor, $btnVaaiCheck, $btnVaaiCheck1, $btnILOCheck, $btnStorageOpt, $btnHpCheck, $diagOutput))
    $diagPopup.ShowDialog() | Out-Null
})
...
# =======================================================================
# DATEI: CloneScript_Templates_V8.5.0.ps1
# VERSION: 8.9.1 (Fix: Log-Filename for Live-View & Stable Logic)
# =======================================================================

# -----------------------------------------------------------------------
# 1. MANUELLER MULTI-VM HOT-KLON
# -----------------------------------------------------------------------
$multiCloneScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Clone Helper 8.9.1 (Final Log & Email Fix)
...
# Log-Pfad (Exakt wie von GUI erwartet)
...
send_email_notification() {
    ...
    log_raw "-----------------------------------------"
    if [ "${OVERALL_STATUS}" = "OK" ]; then FINAL_STATUS_MSG="## Final status: Klonen ERFOLGREICH! ##"; else FINAL_STATUS_MSG="## Final status: ERROR: Klonen fehlgeschlagen! ##"; fi
...
log_raw "  - Typ: Manueller Multi-VM Hot-Klon 8.9.1"
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    if [ -n "${OLD_VMID}" ]; then
        log "  -> Alter Klon gefunden. Wird entfernt..."
        /bin/vim-cmd vmsvc/power.off ${OLD_VMID} >/dev/null 2>&1 || true; sleep 2; /bin/vim-cmd vmsvc/unregister ${OLD_VMID} >/dev/null 2>&1
    fi
	
    log "[2/7] Erstelle Zielverzeichnis..."; rm -rf "${TARGET_VM_PATH}"; mkdir -p "${TARGET_VM_PATH}";

    # --- PRE-FLIGHT CHECK ---
    log "[2.5/7] Lese Disk-Konfiguration (Pre-Flight)..."
    DISK_DEFS_ORIG=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)')
    # VMX Kopieren (Clean State)
    ...
    log "[4/7] Bereite Ziel-VMX vor...";
    cd "${TARGET_VM_PATH}"; ORIG_VMX_FILE=$(find . -maxdepth 1 -name "*.vmx"); ORIG_BASENAME=$(basename "${ORIG_VMX_FILE}" .vmx);
    if [ "${ORIG_BASENAME}" != "${NEW_VM_NAME}" ]; then mv "${ORIG_VMX_FILE}" "${NEW_VM_NAME}.vmx"; fi
    NEW_VMX_FILE="${TARGET_VM_PATH}/${NEW_VM_NAME}.vmx"
    ...
    sed -i '/^uuid\.action/d' "${NEW_VMX_FILE}"; echo "uuid.action = \"${UUID_ACTION}\"" >> "${NEW_VMX_FILE}";

    log "[5/7] Klone Festplatten (SCSI-ID Logic)...";
    CLONE_ERROR=0;
    OLD_IFS=$IFS; IFS='
'; for line in ${DISK_DEFS_ORIG}; do
        IFS=$OLD_IFS; 
        ORIG_DISK_NAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p'); if [ -z "${ORIG_DISK_NAME}" ]; then continue; fi;
        
        # SCSI ID Parsen
        SCSI_KEY=$(echo "$line" | awk -F. '{print $1}'); UNIT_NUM=$(echo "$SCSI_KEY" | awk -F: '{print $2}')
        case $UNIT_NUM in ''|*[!0-9]*) SUFFIX="" ;; *) SUFFIX=$(printf "-%02d" $UNIT_NUM) ;; esac
        
        if echo "${ORIG_DISK_NAME}" | grep -q "^/"; then SOURCE_DISK_PATH="${ORIG_DISK_NAME}"; else SOURCE_DISK_PATH="${VMX_DIR}/${ORIG_DISK_NAME}"; fi;
        
        # Force Clean Name (-00, -01)
        DEST_DISK_BASENAME="${NEW_VM_NAME}${SUFFIX}.vmdk"
        DEST_DISK_PATH="${TARGET_VM_PATH}/${DEST_DISK_BASENAME}";
        
        log "  -> Klone Festplatte: ${DEST_DISK_BASENAME} (ID: ${SCSI_KEY})"; 
        vmkfstools -i "${SOURCE_DISK_PATH}" -d ${DISK_FORMAT} "${DEST_DISK_PATH}" >> ${LOG_FILE} 2>&1;
        if [ $? -ne 0 ]; then log_error "Klonen fehlgeschlagen!"; CLONE_ERROR=1; break; fi;
        
        # Hard Replace VMX
        sed -i "s|${ORIG_DISK_NAME}|${DEST_DISK_BASENAME}|g" "${NEW_VMX_FILE}"
    done; IFS=$OLD_IFS;
    ...
    log "[6/7] Konfiguration abgeschlossen."
    ...
    if [ ${REGISTER_CODE} -ne 0 ]; then
        log_error "Registrierung fehlgeschlagen: ${REGISTER_OUTPUT}"; OVERALL_STATUS="ERROR";
    else
        NEW_VMID_CLONE=$(echo "${REGISTER_OUTPUT}"); log "  -> VMID: ${NEW_VMID_CLONE}";
        if [ "${POWER_ON}" = "1" ]; then log "Schalte VM ein..."; /bin/vim-cmd vmsvc/power.on "${NEW_VMID_CLONE}" >/dev/null 2>&1; fi
    fi
    log "  -> Entferne Snapshot..."; /bin/vim-cmd vmsvc/snapshot.removeall ${VMID} >/dev/null 2>&1;
    ...
    while [ $WAITED -lt $MAX_WAIT ]; do
        SNAP_COUNT=$(/bin/vim-cmd vmsvc/snapshot.get ${VMID} | wc -l); if [ ${SNAP_COUNT} -le 1 ]; then break; fi;
    ...
    # --- NEU: Directory Listing für das E-Mail Log ---
    log "----------------------------------------------------------"
    log "Inhalt des Zielverzeichnisses (${NEW_VM_NAME}):"
    ls -lh "${TARGET_VM_PATH}" >> "${LOG_FILE}"
    log "----------------------------------------------------------"
...
log "====== MULTI-VM KLON-PROZESS BEENDET ======";
'@

# =======================================================================
# DATEI: CloneScript_Templates_V8.5.0.ps1
# VERSION: 8.9.1 (Final Parity - Based on V54.24 Replication Logic)
# =======================================================================

# -----------------------------------------------------------------------
# 2. MANUELLER SINGLE-VM HOT-KLON
# -----------------------------------------------------------------------
$cloneScriptTemplate = @'
#!/bin/sh
# GhettoGUI Single-VM Clone Helper 8.9.1 (Manual - Stable Email & SCSI-ID)

PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH

# 1. PARAMETER (GUI Placeholders)
...
# --- Helpers (Same as V54.24 Replication) ---
log() { echo "$(date '+%Y-%m-%d %H:%M:%S') -> $1" >> ${LOG_FILE}; sync; }
log_raw() { echo "$1" >> ${LOG_FILE}; sync; }
log_error() { echo "$(date '+%Y-%m-%d %H:%M:%S') -- ERROR: $1" >> ${LOG_FILE}; sync; }
...
send_email_notification() {
    ...
    # Report-Struktur wie in Replikation
    log_raw "\n--- Zusammenfassung der geklonten VMs ---"
    log_raw "${VM_REPORT_LIST}"
    log_raw "-----------------------------------------"
    
    if [ "${OVERALL_STATUS}" = "OK" ]; then 
        FINAL_STATUS_MSG="## Final status: Klonen ERFOLGREICH! ##"
    else 
        FINAL_STATUS_MSG="## Final status: ERROR: Klonen fehlgeschlagen! ##"
    fi
    ...
    if [ "${EMAIL_ENABLED}" = "1" ] && [ -f "${SENDMAIL_PATH}" ]; then
        RECIPIENTS=$(echo "${EMAIL_TO}" | sed 's/[;,]/ /g')
        # Verwendung von EMAIL_DISPLAY_NAME wie in Replication V54.24
        /bin/python ${SENDMAIL_PATH} -f "${EMAIL_FROM}" -N "${EMAIL_DISPLAY_NAME}" -s "${EMAIL_SERVER}" -S "${EMAIL_PORT}" -j "${EMAIL_SUBJECT} - ${FINAL_STATUS_MSG}" -m "${LOG_FILE}" -u "${EMAIL_USER}" -p "${EMAIL_PASS}" ${RECIPIENTS} >> ${LOG_FILE} 2>&1
    fi
}
...
cleanup() {
    ...
    if [ ${EXIT_CODE} -ne 0 ] && [ "${OVERALL_STATUS}" = "OK" ]; then
        log_error "Skript wurde mit Fehlercode ${EXIT_CODE} unterbrochen."
        OVERALL_STATUS="ERROR"
    fi

    # Snapshot entfernen falls vorhanden
    if [ -n "${VMID}" ]; then
        log "Bereinige Snapshot..."
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manuell Single-VM Hot-Klon 8.9.1"
log_raw "  - Quelle: ${SOURCE_VM_NAME}"
log_raw "  - Ziel: ${NEW_VM_NAME}"
log_raw "  - Host: $(hostname)"
log_raw "--- START DES DETAILLOGS ---"
...
# 2. Bereinigung
OLD_VMID=$(vim-cmd vmsvc/getallvms | awk -v name="${NEW_VM_NAME}" '{ n=split($0, a, "\\["); vm=a[1]; sub(/^[0-9]+[ \t]+/, "", vm); sub(/[ \t]+$/, "", vm); if (vm == name) print $1; }')
if [ -n "${OLD_VMID}" ]; then
    log "[1/7] Entferne alten Klon (VMID: ${OLD_VMID})..."
    vim-cmd vmsvc/power.off ${OLD_VMID} >/dev/null 2>&1 || true
    sleep 2
    vim-cmd vmsvc/unregister ${OLD_VMID} >/dev/null 2>&1
fi

TARGET_VM_PATH="${TARGET_DATASTORE}/${NEW_VM_NAME}"
log "[2/7] Erstelle Zielverzeichnis: ${TARGET_VM_PATH}"
rm -rf "${TARGET_VM_PATH}"; mkdir -p "${TARGET_VM_PATH}"

# 3. Pre-Flight Check (Disk-Liste VOR Snapshot)
log "[2.5/7] Lese Disk-Konfiguration..."
DISK_DEFS_ORIG=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)')

# VMX jetzt sichern
(cd "${VMX_DIR}" && find . -maxdepth 1 \( -name "*.vmx" -o -name "*.nvram" \) -exec cp -p '{}' "${TARGET_VM_PATH}/" \;)

log "[3/7] Erstelle Snapshot '${SNAPSHOT_NAME}'..."
vim-cmd vmsvc/snapshot.create ${VMID} "${SNAPSHOT_NAME}" "GhettoGUI Manual Clone" 0 ${SNAP_QUIESCE}
log "  -> Warte 15s..."; sleep 15
...
# 5. Klonen (SCSI-ID Logic)
log "[5/7] Klone Festplatten..."
CLONE_ERROR=0
OLD_IFS=$IFS; IFS='
'; for line in ${DISK_DEFS_ORIG}; do
    IFS=$OLD_IFS
    ORIG_DISK_NAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p'); 
    if [ -z "${ORIG_DISK_NAME}" ]; then continue; fi
    
    # SCSI ID ermitteln (z.B. scsi0:1)
    SCSI_KEY=$(echo "$line" | awk -F. '{print $1}')
    UNIT_NUM=$(echo "$SCSI_KEY" | awk -F: '{print $2}')
    case $UNIT_NUM in ''|*[!0-9]*) SUFFIX="" ;; *) SUFFIX=$(printf "-%02d" $UNIT_NUM) ;; esac
    ...
    log "  -> Klone ${SCSI_KEY}: ${DEST_DISK_BASENAME}"
    
    # Direkter vmkfstools Aufruf für maximale Stabilität
    vmkfstools -i "${SOURCE_DISK_PATH}" -d ${DISK_FORMAT} "${DEST_DISK_PATH}" >> ${LOG_FILE} 2>&1
    if [ $? -ne 0 ]; then log_error "Klonen fehlgeschlagen!"; CLONE_ERROR=1; break; fi;
...
REGISTER_OUTPUT=$(vim-cmd solo/registervm "${TARGET_VM_PATH}/${NEW_VM_NAME}.vmx" 2>&1)
if [ $? -ne 0 ]; then
    log_error "Registrierung fehlgeschlagen: ${REGISTER_OUTPUT}"; OVERALL_STATUS="ERROR"
else
    NEW_VMID_CLONE=$(echo "${REGISTER_OUTPUT}")
    log "  -> VMID: ${NEW_VMID_CLONE}"
    if [ "${POWER_ON}" = "1" ]; then 
        log "Schalte VM ein..."; vim-cmd vmsvc/power.on "${NEW_VMID_CLONE}" >/dev/null 2>&1
    fi
fi
...
# VM_REPORT_LIST füllen für Email
VM_FINAL_SIZE=$(du -sh "${TARGET_VM_PATH}" 2>/dev/null | awk '{print $1}')
VM_REPORT_LIST=$(printf "%s- %s (%s)\n" "${VM_REPORT_LIST}" "${NEW_VM_NAME}" "${VM_FINAL_SIZE}")
...
for SOURCE_VM_NAME in ${VM_LIST}; do
    ...
    log "#########8.7.7########### Starte Klon für VM: ${SOURCE_VM_NAME} ####################"
    log "initiate backup for ${SOURCE_VM_NAME}"
...
#!/bin/sh
# GhettoGUI Scheduled Clone Helper V8.5.9 (Fix: SCSI-ID Based Naming)
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
log "====== GEPLANTER VM KLON-PROZESS GESTARTET (ID: ${UNIQUE_ID}) ======"; log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"; log_raw "Job-Konfiguration:"; log_raw "  - Typ: Geplanter Lokaler Hot-Klon V8.5.9"; log_raw "  - Quelle: ${SOURCE_VM_NAME}"; log_raw "--- START DES DETAILLOGS ---"
...
# --- FIX: Wildcard-Ersetzung ---
NEW_VM_NAME=$(echo "${NEW_VM_NAME}" | sed "s/[*]/${SOURCE_VM_NAME}/g")
log "  -> Ziel-Name: ${NEW_VM_NAME}"
...
# --- PRE-FLIGHT CHECK (Read Config BEFORE Snapshot) ---
log "[2.5/7] Lese Disk-Konfiguration (Pre-Flight)..."
DISK_DEFS_ORIG=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)')
...
if [ "${ORIG_BASENAME}" != "${NEW_VM_NAME}" ]; then
    mv "${ORIG_VMX_FILE}" "${NEW_VM_NAME}.vmx"
fi
NEW_VMX_FILE="${TARGET_VM_PATH}/${NEW_VM_NAME}.vmx"

# Basis-Anpassungen VMX
sed -i "s/^displayName = .*/displayName = \"${NEW_VM_NAME}\"/" "${NEW_VMX_FILE}";
sed -i '/extendedConfigFile/d' "${NEW_VMX_FILE}"; sed -i '/vmxstats.filename/d' "${NEW_VMX_FILE}"; sed -i '/migrate.hostLog/d' "${NEW_VMX_FILE}";
sed -i '/sched.swap.derivedName/d' "${NEW_VMX_FILE}"; sed -i '/uuid.location/d' "${NEW_VMX_FILE}"; sed -i '/uuid.bios/d' "${NEW_VMX_FILE}"; sed -i '/vc.uuid/d' "${NEW_VMX_FILE}";
sed -i '/^uuid\.action/d' "${NEW_VMX_FILE}"; echo "uuid.action = \"${UUID_ACTION}\"" >> "${NEW_VMX_FILE}";

log "[5/7] Klone Festplatten & Aktualisiere VMX..."
CLONE_ERROR=0;
OLD_IFS=$IFS; IFS='
'; for line in ${DISK_DEFS_ORIG}; do
    IFS=$OLD_IFS; 
    ...
    # Extract Key (e.g. scsi0:0) and Unit ID
    SCSI_KEY=$(echo "$line" | awk -F. '{print $1}')
    UNIT_NUM=$(echo "$SCSI_KEY" | awk -F: '{print $2}')
    
    # Validate Unit Num (must be integer)
    case $UNIT_NUM in ''|*[!0-9]*) SUFFIX="" ;; *) SUFFIX=$(printf "-%02d" $UNIT_NUM) ;; esac
    
    # Define Source Path
    if echo "${ORIG_DISK_NAME}" | grep -q "^/"; then SOURCE_DISK_PATH="${ORIG_DISK_NAME}"; else SOURCE_DISK_PATH="${VMX_DIR}/${ORIG_DISK_NAME}"; fi;
    
    # Define Target Name (FORCE Clean Name based on ID)
    # Pattern: NewVMName-XX.vmdk (e.g. MyVM-00.vmdk, MyVM-01.vmdk)
    DEST_DISK_BASENAME="${NEW_VM_NAME}${SUFFIX}.vmdk"
    DEST_DISK_PATH="${TARGET_VM_PATH}/${DEST_DISK_BASENAME}";
...
log "[7/7] Registriere Klon & entferne Snapshot..."; NEW_VMID_CLONE=$(vim-cmd solo/registervm "${TARGET_VM_PATH}/${NEW_VM_NAME}.vmx"); log "  -> Klon registriert als VMID: ${NEW_VMID_CLONE}";
if [ "${POWER_ON}" = "1" ]; then log "Schalte geklonte VM ein..."; vim-cmd vmsvc/power.on "${NEW_VMID_CLONE}"; fi
OVERALL_STATUS="OK";
log "  -> Entferne Snapshot von ${SOURCE_VM_NAME}..."; /bin/vim-cmd vmsvc/snapshot.removeall ${VMID} >/dev/null 2>&1;
# --- NEU: Directory Listing für das E-Mail Log ---
log "----------------------------------------------------------"
log "Inhalt des Zielverzeichnisses (${NEW_VM_NAME}):"
log_raw "Speicherplatz (Nachher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(df -h "${TARGET_DATASTORE}" 2>/dev/null | grep /vmfs/)"
...
# =============================================================================
# DATEI: RestoreScripts854.ps1
# VERSION: V8.5.9 (Final Update - Compatible with ID-Based Disk Names)
# =============================================================================

# -----------------------------------------------------------------------------
# 1. GEPLANTER RESTORE (Scheduled)
# -----------------------------------------------------------------------------
$scheduledRestoreScriptTemplate = @'
#!/bin/sh
# GhettoGUI Scheduled Restore Helper V8.5.9 (Fix: Robust Renaming & Pattern Matching)
# Parameter
...
send_email_notification() {
    ...
    log "Bereite E-Mail vor: ${OVERALL_STATUS}"; log_raw "Duration: ${DURATION_MSG}"; log_raw "${FINAL_STATUS_MSG}";
    if [ "${EMAIL_ENABLED}" = "1" ] && [ -f "${SENDMAIL_PATH}" ]; then
        RECIPIENTS=$(echo "${EMAIL_TO}" | sed 's/,/ /g');
        /bin/python ${SENDMAIL_PATH} -f "${EMAIL_FROM}" -N "${EMAIL_DISPLAY_NAME}" -s "${EMAIL_SERVER}" -S "${EMAIL_PORT}" -j "${EMAIL_SUBJECT} - ${FINAL_STATUS_MSG}" -m "${LOG_FILE}" -u "${EMAIL_USER}" -p "${EMAIL_PASS}" ${RECIPIENTS} >> ${LOG_FILE} 2>&1;
    fi;
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"; log_raw "  - Typ: Geplante GhettoVCB Wiederherstellung 8.9.1"; log_raw "  - Quelle: ${SOURCE_PATH}"; log_raw "  - Ziel-Datastore: ${TARGET_DATASTORE}"; log_raw "  - Neuer VM-Name: ${NEW_VM_NAME}";
log_raw "--- START DES DETAILLOGS ---"
...
# 3. Umbenennen & Patchen
log "[3/5] Passe Konfigurationsdateien an..."; cd "${TARGET_VM_PATH}"; 
ORIG_VMX_FILE=$(find . -maxdepth 1 -name "*.vmx"); 
if [ -z "${ORIG_VMX_FILE}" ]; then log_error "Keine VMX gefunden!"; exit 1; fi
ORIG_BASENAME=$(basename "${ORIG_VMX_FILE}" .vmx); 

log "  -> Original-Name: ${ORIG_BASENAME} -> Neu: ${NEW_VM_NAME}"

# FIX: Robustere Schleife, die auch 'Name-00.vmdk' erwischt (nicht nur 'Name.*')
find . -maxdepth 1 -name "${ORIG_BASENAME}*" | while read f; do
    # Den Dateinamen ohne Pfad extrahieren
    base_f=$(basename "$f")
    # Neuen Namen berechnen: Ersetze den Anfang des Dateinamens
    new_name=$(echo "$base_f" | sed "s/^${ORIG_BASENAME}/${NEW_VM_NAME}/")
    
    if [ "$base_f" != "$new_name" ]; then
        log "     Renaming: $base_f -> $new_name"
        mv -- "$base_f" "$new_name"
    fi
done
...
# FIX: Robustes VMX-Patching (Findet auch Name-00.vmdk Einträge)
sed -i "s/displayName = .*/displayName = \"${NEW_VM_NAME}\"/" "${NEW_VMX_FILE}";
# Ersetze alle Vorkommen des alten Basisnamens in Dateipfaden
sed -i "s/${ORIG_BASENAME}/${NEW_VM_NAME}/g" "${NEW_VMX_FILE}"
echo "uuid.action = \"${UUID_ACTION}\"" >> "${NEW_VMX_FILE}";

# 4. Registrieren
log "[4/5] Registriere neue VM..."; NEW_VMID=$(vim-cmd solo/registervm "${TARGET_VM_PATH}/${NEW_VM_NAME}.vmx"); log "  -> VMID: ${NEW_VMID}";

# 5. Starten
if [ "${POWER_ON}" = "1" ]; then log "[5/5] Schalte VM ein..."; sleep 5; vim-cmd vmsvc/power.on "${NEW_VMID}" >/dev/null 2>&1; fi
...
# -------------------------------------------------------------------------
#  -------------- geplanter Multiclone ----------------------
# -------------------------------------------------------------------------

$scheduledMultiCloneScriptTemplate = @'
#!/bin/sh
# GhettoGUI Scheduled Multi-VM Clone Helper 8.9.1 (Fix: SCSI-ID Based Naming)
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"; log_raw "  - Typ: Geplanter Multi-VM Hot-Klon V8.5.9"; log_raw "  - Ziel-Datastore: ${TARGET_DATASTORE}"; log_raw "  - Namens-Vorlage: ${NAME_TEMPLATE}"; log_raw "Speicherplatz (Vorher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(df -h "${TARGET_DATASTORE}" 2>/dev/null | grep /vmfs/)"; log_raw "--- START DES DETAILLOGS ---";
...
# -----------------------------------------------------------------------------
# 2. MANUELLER RESTORE (Manual)
# -----------------------------------------------------------------------------
$restoreScriptTemplate = @'
#!/bin/sh
# GhettoGUI Cold Restore Helper V8.5.9 (Fix: Robust Renaming)
set -e
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manueller Restore 8.9.1"
log_raw "  - Quelle: ${SOURCE_PATH}"
log_raw "  - Ziel: ${TARGET_DATASTORE}/${NEW_VM_NAME}"
...
if [ "${SUBFOLDER_COUNT}" -eq 1 ]; then
    ...
    log "  -> Unterordner gefunden, nutze: ${ACTUAL_SOURCE_PATH}"
fi

# Schritt 2: Kopieren
log "[2/5] Kopiere Backup-Daten..."
SOURCE_SIZE=$(du -sh "${ACTUAL_SOURCE_PATH}" | awk '{print $1}')
log "  -> Größe: ${SOURCE_SIZE}"
cp -r "${ACTUAL_SOURCE_PATH}/." "${TARGET_VM_PATH}/" &
CP_PID=$!
while kill -0 ${CP_PID} >/dev/null 2>&1; do sleep 5; done
wait ${CP_PID}; if [ $? -ne 0 ]; then log_error "Kopieren fehlgeschlagen!"; exit 1; fi
log "  -> Kopieren fertig."
...
log "  -> Renaming: ${ORIG_BASENAME} -> ${NEW_VM_NAME}"
# FIX: Robustes Renaming (findet auch -00.vmdk Dateien)
find . -maxdepth 1 -name "${ORIG_BASENAME}*" | while read f; do
    base_f=$(basename "$f")
    new_name=$(echo "$base_f" | sed "s/^${ORIG_BASENAME}/${NEW_VM_NAME}/")
    if [ "$base_f" != "$new_name" ]; then
...
sed -i "s/displayName = .*/displayName = \"${NEW_VM_NAME}\"/" "${NEW_VMX_FILE}"
# FIX: Globales Ersetzen des Basisnamens (deckt VM.vmdk und VM-00.vmdk ab)
sed -i "s/${ORIG_BASENAME}/${NEW_VM_NAME}/g" "${NEW_VMX_FILE}"
echo "uuid.action = \"${UUID_ACTION}\"" >> "${NEW_VMX_FILE}"
...
if [ $? -ne 0 ]; then log_error "Fehler bei Registrierung!"; log_error "${REGISTER_OUTPUT}"; exit 1; fi
NEW_VMID=$(echo "${REGISTER_OUTPUT}")
log "  -> VMID: ${NEW_VMID}"

# Schritt 5: Starten
if [ "${POWER_ON}" = "1" ]; then
    log "[5/5] Schalte VM ein..."; sleep 5; vim-cmd vmsvc/power.on "${NEW_VMID}" >/dev/null 2>&1
else
    log "[5/5] Kein automatischer Start."
fi
...
$buttonRestore.Add_Click({
    ...
    if ($restoreResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        # === 3. BESTEHENDE LOGIK: SINGLE KLON / RESTORE ===
            ...
            if ($radioRestoreFromBackup.Checked) {
                $params = @{
                    SOURCE_PATH = $textboxRestoreSourcePath.Text; TARGET_DATASTORE = $textboxRestoreTargetPath.Text; NEW_VM_NAME = $textboxRestoreNewVmName.Text.Trim();
                    POWER_ON = if ($checkboxPowerOnAfterRestore.Checked) { 1 } else { 0 }; UUID_ACTION = if ($radioRestoreMoved.Checked) { "keep" } else { "create" };
                    UNIQUE_ID = "{0}-{1}" -f (Get-Date -Format 'yyyyMMddHHmmss'), (Get-Random -Minimum 100 -Maximum 999); GHETTO_PATH = $textboxGhettoPath.Text;
                    EMAIL_ENABLED = if ($checkboxEmailLog.Checked) { 1 } else { 0 }; EMAIL_TO = $textboxEmailTo.Text; EMAIL_FROM = $textboxEmailFrom.Text; EMAIL_DISPLAY_NAME = $dn; EMAIL_SERVER = $textboxEmailServer.Text; EMAIL_PORT = $textboxEmailPort.Text; EMAIL_USER = $textboxEmailUser.Text; EMAIL_PASS = $textboxEmailPassword.Text;
                ...
                if ([string]::IsNullOrWhiteSpace($comboboxDiskFormat.SelectedItem)) { [System.Windows.Forms.MessageBox]::Show("Bitte wählen Sie zuerst im Hauptfenster ein 'Disk Format' aus (z.B. thin).", "Fehlende Auswahl", "OK", "Warning"); return }
                $params = @{
                    SOURCE_VM_NAME = $textboxRestoreSourcePath.Text; TARGET_DATASTORE = $textboxRestoreTargetPath.Text; NEW_VM_NAME = $textboxRestoreNewVmName.Text.Trim();
                    POWER_ON = if ($checkboxPowerOnAfterRestore.Checked) { 1 } else { 0 }; UUID_ACTION = if ($radioRestoreMoved.Checked) { "keep" } else { "create" };
                    DISK_FORMAT = $comboboxDiskFormat.SelectedItem.ToString(); SNAP_QUIESCE = if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 };
                    UNIQUE_ID = "{0}-{1}" -f (Get-Date -Format 'yyyyMMddHHmmss'), (Get-Random -Minimum 100 -Maximum 999); GHETTO_PATH = $textboxGhettoPath.Text;
                    EMAIL_ENABLED = if ($checkboxEmailLog.Checked) { 1 } else { 0 }; EMAIL_TO = $textboxEmailTo.Text; EMAIL_FROM = $textboxEmailFrom.Text; EMAIL_DISPLAY_NAME = $dn; EMAIL_SERVER = $textboxEmailServer.Text; EMAIL_PORT = $textboxEmailPort.Text; EMAIL_USER = $textboxEmailUser.Text; EMAIL_PASS = $textboxEmailPassword.Text;
...
# ---------------------------------
# ------- Start Zeitplan anzeigen"
# --------------------------------

function Get-HumanReadableCron {
    param ([string]$CronLine)
    
    $parts = $CronLine.Trim() -split "\s+"
    if ($parts.Length -lt 5) { return "Ungültiges Format" }
    
    $min  = $parts[0]
    $hour = $parts[1]
    $dow  = $parts[4] # Wochentage (z.B. 4,1,6)
    
    $dayMap = @{
        "1"="Mo"; "2"="Di"; "3"="Mi"; "4"="Do"; "5"="Fr"; "6"="Sa"; "0"="So"; "7"="So"
    }
    ...
    if ($dow -eq "*") {
        ...
        # Tage sortieren (1,4,6 statt 4,1,6)
        $dayParts = $dow -split "," | Sort-Object
        $translated = foreach ($p in $dayParts) {
            if ($p -match "-") {
                $range = $p -split "-"
                "$($dayMap[$range[0]]) bis $($dayMap[$range[1]])"
            } else {
                $dayMap[$p]
            }
        }
        $daysText = "am " + ($translated -join ", ")
    }
    
    try {
        $timeStr = "{0:D2}:{1:D2}" -f [int]$hour, [int]$min
        return "$timeStr Uhr, $daysText"
    } catch {
        return "Zeitplan: $dow um ${hour}:${min}"
    }
}

function Show-GhettoSchedule {
    if (-not ($Global:ESXiSession -and $Global:ESXiSession.Connected)) {
        [System.Windows.Forms.MessageBox]::Show("Bitte zuerst mit dem ESXi Host verbinden!", "Nicht verbunden", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        return
    }

    # Fenster-Setup
    $schedForm = New-Object System.Windows.Forms.Form
    $schedForm.Text = "GhettoGUI Zeitplan-Verwaltung"
    $schedForm.Size = New-Object System.Drawing.Size(750, 500)
    $schedForm.StartPosition = "CenterParent"
    $schedForm.Font = New-Object System.Drawing.Font("Segoe UI", 10)

    # ListView für tabellarische Ansicht
    $listView = New-Object System.Windows.Forms.ListView
    $listView.View = 'Details'
    $listView.FullRowSelect = $true
    $listView.GridLines = $true
    $listView.Location = New-Object System.Drawing.Point(15, 40)
    $listView.Size = New-Object System.Drawing.Size(705, 320)
    $listView.MultiSelect = $false

    # Spalten definieren
    $listView.Columns.Add("Typ", 100) | Out-Null
    $listView.Columns.Add("Zeitpunkt (UTC)", 250) | Out-Null
    $listView.Columns.Add("Intervall / Zyklus", 170) | Out-Null
    $listView.Columns.Add("ID / Kommentar", 170) | Out-Null

    # Daten abrufen
    $form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    try {
        $sshResult = Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command "cat /var/spool/cron/crontabs/root"
        $lines = @($sshResult.Output)
        $ghettoLines = $lines | Where-Object { $_ -match "GhettoGUI" }
    } catch {
        [System.Windows.Forms.MessageBox]::Show("Fehler beim Lesen der Crontab: $($_.Exception.Message)")
    }
    $form.Cursor = [System.Windows.Forms.Cursors]::Default

    foreach ($line in $ghettoLines) {
        if ([string]::IsNullOrWhiteSpace($line)) { continue }
        
        # --- Präzise Typ-Bestimmung für alle GhettoGUI Job-Arten ---
        $type = "Job"
        $id = "N/A"
        ...
        elseif ($line -match "Replication") { 
            $type = "🔄 Replikation" 
        }
        elseif ($line -match "Restore" -or $line -match "scheduled_restore") { 
        # NEU: Erkennt Restore-Jobs anhand des Kommentars oder Dateinamens
            $type = "⏪ Restore" 
        }
        elseif ($line -match "Multi-Clone") { 
            $type = "👥 Multi-Klon" 
        }
        elseif ($line -match "Single-Clone" -or $line -match "scheduled_clone") { 
            $type = "👤 Single-Klon" 
        }

        # ID extrahieren (bleibt identisch)
        if ($line -match "ID: ([0-9-]+)") { $id = $matches[1] }

        # 2. Zeit & Intervall bestimmen
        $time = Get-HumanReadableCron -CronLine $line
        ...
        if ($line -match "% 2") { 
            ...
            $interval = "Monatlich (3.W)" 
        } elseif ($line -match "-ge 22") { 
            if ($line -match "-le 28") { 
                $interval = "Monatlich (4.W)" 
            } else { 
                $interval = "Monatlich (Ende / ab 22.)" 
            }
        }

        # 3. ListViewItem sicher erstellen (verhindert NullReferenceException)
        $item = New-Object System.Windows.Forms.ListViewItem
        $item.Text = $type
        $item.SubItems.Add($time) | Out-Null
        $item.SubItems.Add($interval) | Out-Null
        $item.SubItems.Add($id) | Out-Null
        $item.Tag = $id  # ID für die Löschfunktion speichern
        
        $listView.Items.Add($item) | Out-Null
    }

    $schedForm.Controls.Add($listView)

    # Button: Löschen
    $btnDel = New-Object System.Windows.Forms.Button
    $btnDel.Text = "Ausgewählten Job entfernen"
    $btnDel.Location = New-Object System.Drawing.Point(15, 380)
    $btnDel.Size = New-Object System.Drawing.Size(200, 40)
    $btnDel.BackColor = [System.Drawing.Color]::MistyRose
    $btnDel.Add_Click({
        if ($listView.SelectedItems.Count -eq 1) {
            $selectedId = $listView.SelectedItems[0].Tag
            if ($selectedId -eq "N/A") {
                [System.Windows.Forms.MessageBox]::Show("Job ohne ID gefunden. Löschen nicht möglich.", "Fehler", "OK", "Error")
                return
            }
            $confirm = [System.Windows.Forms.MessageBox]::Show("Möchten Sie den Job mit der ID $selectedId wirklich entfernen?", "Bestätigung", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)
            if ($confirm -eq "Yes") {
                Remove-GhettoCronJob -JobId $selectedId
                $schedForm.Close()
                Show-GhettoSchedule # Refresh
            }
        }
    })
    $schedForm.Controls.Add($btnDel)

    # Button: Schließen
    $btnClose = New-Object System.Windows.Forms.Button
    $btnClose.Text = "Schließen"
    $btnClose.Location = New-Object System.Drawing.Point(620, 380)
    $btnClose.Size = New-Object System.Drawing.Size(100, 40)
    $btnClose.Add_Click({ $schedForm.Close() })
    $schedForm.Controls.Add($btnClose)

    $schedForm.ShowDialog() | Out-Null
}

function Remove-GhettoCronJob {
    param([string]$JobId)
    try {
        # Wir löschen basierend auf der ID im Kommentar. 
        # Das ist sicher, da die ID keine Sonderzeichen wie Pfade enthält.
        # Wir nutzen ein anderes Trennzeichen für sed (| statt /), falls Pfade doch drin wären.
        $remoteCmd = 'sed -i "/ID: ' + $JobId + '/d" /var/spool/cron/crontabs/root && kill $(cat /var/run/crond.pid) && crond'
        
        $result = Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command $remoteCmd
        
        [System.Windows.Forms.MessageBox]::Show("Job mit ID $JobId wurde erfolgreich entfernt.", "Info", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
    } catch {
        [System.Windows.Forms.MessageBox]::Show("Fehler beim Löschen: $($_.Exception.Message)", "Fehler", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
}

# --- REGISTRIERUNG DES BUTTONS (im Event-Block hinzugefügt) ---
$buttonshowSchedule.Add_Click({
    Show-GhettoSchedule
})

# ---------------------------------
# ------- Ende Zeitplan anzeigen"
# --------------------------------



# ---------------------------------------------------------------------------------
# Zeitplanung scheduled Start Geplante Replikation und Backup und Klon mit CRON + 2-Weekly + Monthly
#----------------------------------------------------------------------------------
    ...
    try {
        ...
        try {
            ...
            if ($radioScheduleReplication.Checked) {
                ...
                if ([int]$valSleepLog -lt 10 -or [int]$valSleepLog -gt 3600) { throw "SleepLog muss zwischen 10 und 3600 Sekunden liegen." }
                ...
                $params = @{ 
					...
					LOG_UPDATE_INTERVAL = $valSleepLog;
                ...
                if ($radioRestoreFromBackup.Checked) {
                    ...
                    if ($selectedVmCount -eq 1) {
                        ...
                        $params = @{ SOURCE_VM_NAME = $checkedListBoxVms.CheckedItems[0].OriginalName; TARGET_DATASTORE = $textboxRestoreTargetPath.Text; NEW_VM_NAME = $textboxRestoreNewVmName.Text.Trim(); POWER_ON = if ($checkboxPowerOnAfterRestore.Checked) { 1 } else { 0 }; UUID_ACTION = if ($radioRestoreMoved.Checked) { "keep" } else { "create" }; DISK_FORMAT = $comboboxDiskFormat.SelectedItem.ToString(); SNAP_QUIESCE = if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 }; UNIQUE_ID = $jobId; GHETTO_PATH = $ghettoPath; EMAIL_ENABLED = if ($checkboxEmailLog.Checked) { 1 } else { 0 }; EMAIL_TO = $textboxEmailTo.Text; EMAIL_FROM = $textboxEmailFrom.Text; EMAIL_DISPLAY_NAME = $dn; EMAIL_SERVER = $textboxEmailServer.Text; EMAIL_PORT = $textboxEmailPort.Text; EMAIL_USER = $textboxEmailUser.Text; EMAIL_PASS = $textboxEmailPassword.Text; EMAIL_SUBJECT = "[Scheduled-Clone] " + $textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName) }
                ...
                "EMAIL_SERVER=`"$($textboxEmailServer.Text.Trim())`"", 
                "EMAIL_SERVER_PORT=$($textboxEmailPort.Text.Trim())", 
                "EMAIL_USER_NAME=`"$($textboxEmailUser.Text.Trim())`"", 
                "EMAIL_USER_PASSWORD=`"$($textboxEmailPassword.Text.Trim())`"", 
                "EMAIL_FROM=`"$($textboxEmailFrom.Text.Trim())`"", 
                "EMAIL_DISPLAY_NAME=`"$dn`"",
...
# 1. PARAMETER
LOG_UPDATE_INTERVAL='__LOG_UPDATE_INTERVAL__'; UNIQUE_ID='__UNIQUE_ID__'; TARGET_HOST='__TARGET_HOST__'; TARGET_DATASTORE='__TARGET_DATASTORE__'; VM_SUFFIX='__VM_SUFFIX__'; REPLICATION_METHOD='__REPLICATION_METHOD__'; GHETTO_PATH='__GHETTO_PATH__';
...
# Default/Validierung: nur Zahlen, sonst 30 Sekunden
if [ -z "${LOG_UPDATE_INTERVAL}" ] || ! echo "${LOG_UPDATE_INTERVAL}" | grep -Eq '^[0-9]+$'; then
  LOG_UPDATE_INTERVAL=30
fi
# Optional clamp (empfohlen)
[ "${LOG_UPDATE_INTERVAL}" -lt 10 ] && LOG_UPDATE_INTERVAL=10
[ "${LOG_UPDATE_INTERVAL}" -gt 3600 ] && LOG_UPDATE_INTERVAL=3600
...
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Geplante Replikation (H2H 8.9.1)"
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
	...
	log "INFO: LOG_UPDATE_INTERVAL=${LOG_UPDATE_INTERVAL}s"
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        ...
        while read -r line; do
            ...
            SOURCE_DISK_PATH_FOR_CLONE=$(resolve_vmdk_path "${VMX_DISK_PATH_BASE}")
        ...
        while read -r line; do
            ...
            VMX_DISK_PATH=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p');
            if [ -z "${VMX_DISK_PATH}" ]; then continue; fi
            SOURCE_DISK_PATH=$(resolve_vmdk_path "${VMX_DISK_PATH}")
...
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manuelle Replikation (H2H) 8.9.1"
log_raw "  - Methode: ${REPLICATION_METHOD}"
...
function Get-SendmailDebugLog {
    ...
    Write-GuiLog "Scanne Log-Verzeichnis nach E-Mail Historie..."
    $form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    
    try {
        $logDir = "$ghettoPathOnESXi/logs"
        
        # Dieser Befehl durchsucht alle .log Dateien nach dem 'Final status' und dem Datum
        # Er gibt das Datum, den Dateinamen und das Ergebnis aus
        $historyCmd = "grep -H 'Final status:' $logDir/*.log | sort -r"
        $historyResult = Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command $historyCmd

        Write-GuiLog "--- START E-Mail Versand Historie (aus allen Logs) ---"
        
        if ($historyResult.ExitStatus -eq 0 -and $historyResult.Output) {
            foreach ($line in $historyResult.Output) {
                # Formatierung der Ausgabe für bessere Lesbarkeit
                # Trennung von Pfad und Statusmeldung
                if ($line -match "(.*/)(.*\.log):(.*###### Final status: )(.*)( ######)") {
                    $fileName = $matches[2]
                    $status = $matches[4]
                    
                    # Farbauswahl basierend auf Status
                    $statusColor = "White"
                    if ($status -like "*OK*") { $statusColor = "Lime" }
                    elseif ($status -like "*ERROR*") { $statusColor = "Red" }
                    elseif ($status -like "*WARNING*") { $statusColor = "Yellow" }

                    Write-GuiLog "Log: $fileName -> STATUS: $status"
                } else {
                    Write-GuiLog $line
                }
            }
        } else {
            Write-GuiLog "Keine E-Mail-Einträge in den Logs gefunden."
        }
        
        Write-GuiLog "--- ENDE E-Mail Versand Historie ---"
        
    } catch {
        Write-GuiLog "FEHLER beim Scannen der Log-Historie: $($_.Exception.Message)"
    } finally {
        $form.Cursor = [System.Windows.Forms.Cursors]::Default
    }
}
...
function Stop-GhettoVCBBackupJob {
    ...
    $confirmResult = [System.Windows.Forms.MessageBox]::Show(
        "ACHTUNG: Dies beendet alle laufenden GhettoVCB-Prozesse, Klon-Vorgänge und SCP-Transfers HART (Kill -9).`n`nNicht gespeicherte Daten in laufenden Backups gehen verloren. Die VM kann beschädigt werden!`n`nWollen Sie den Abbruch erzwingen?",
        "Backup Not-Aus",
        [System.Windows.Forms.MessageBoxButtons]::YesNo,
        [System.Windows.Forms.MessageBoxIcon]::Warning
    )
    if ($confirmResult -ne 'Yes') {
        Write-GuiLog "Abbruch durch Benutzer."
        return
    }

    Write-GuiLog "Starte asynchronen Abbruch (Logik V9.9 - The Assassin)..."
    $form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor

    # Wir stoppen sofort den Log-Abruf
    if ($Global:logPollTimer.Enabled) { 
        $Global:logPollTimer.Stop()
        Write-GuiLog "Automatischer Log-Abruf gestoppt." 
    }

    try {
        # --- FIX V9.9: DECOUPLED EXECUTION ---
        # Wir erstellen erst ein Skript auf dem Host und starten es dann im Hintergrund.
        # Damit läuft der Kill weiter, auch wenn die SSH-Session stirbt.
        
        $killScriptContent = @'
#!/bin/sh
# GhettoGUI Emergency Kill Script
# Wartet kurz, damit die SSH-Session sauber trennen kann
sleep 2

echo "--- START KILL SEQUENCE ---" > /tmp/kill_log.txt

# Patterns
PATTERN="ghettoVCB|scheduled_replication|ghetto_clone|ghetto_restore|launcher|master_replication|vmkfstools|scp|ssh|dbclient"

# Runde 1: Identifizieren und Killen
PIDS=$(ps -c | grep -E "$PATTERN" | grep -v grep | grep -v $$ | awk '{print $1}')

for PID in $PIDS; do
    case $PID in ''|*[!0-9]*) continue ;; esac
    
    # Check gegen Geisterprozesse und Self-Kill
    FULL_CMD=$(ps | grep "$PID" | grep -v grep)
    if [ -z "$FULL_CMD" ]; then continue; fi
    
    # Wir filtern NICHT mehr streng, wir hauen drauf.
    # Aber wir verschonen uns selbst (das laufende Skript)
    MYPID=$$
    if [ "$PID" -eq "$MYPID" ]; then continue; fi

    echo "Killing PID $PID ($FULL_CMD)" >> /tmp/kill_log.txt
    kill -9 "$PID"
done

# Runde 2: Snapshots (Hintergrund)
vim-cmd vmsvc/getallvms | awk '{print $1}' | grep -E '^[0-9]+' | while read vmid; do
    if vim-cmd vmsvc/snapshot.get $vmid | grep -q "ghettoVCB-snapshot"; then
        echo "Removing Snapshot VM $vmid" >> /tmp/kill_log.txt
        vim-cmd vmsvc/snapshot.removeall $vmid > /dev/null 2>&1 &
    fi
done

# Runde 3: Cleanup
find /vmfs/volumes -maxdepth 2 -type d -name "ghetto_clone_*" -exec rm -rf {} +
rm -rf /tmp/ghettoVCB.pid /tmp/ghettoVCB.work /tmp/vmware-root/ghettoVCB.pid
rm -rf /tmp/sendmail.py /tmp/ghettoVCB_patch.sh
rm -rf /tmp/scheduled_replication_*.sh /tmp/ghetto_clone_*.sh /tmp/ghetto_restore_*.sh
rm -rf /tmp/launcher_*.sh /tmp/master_replication_*.sh
rm -rf /tmp/ghetto_pipe_error_* /tmp/ghetto_*.lock /var/run/ghettovcb.lock

echo "--- DONE ---" >> /tmp/kill_log.txt
# Selbstlöschung
rm -- "$0"
'@
        # Zeilenumbrüche für Linux fixen
        $killScriptContent = $killScriptContent.Replace("`r`n", "`n")
        
        # 1. Skript hochladen (via echo, da SFTP hier zu umständlich wäre)
        # Wir nutzen Base64 Encoding trick, um Sonderzeichen-Probleme beim 'echo' zu vermeiden, 
        # oder wir schreiben es einfach direkt, da wir keine schlimmen Sonderzeichen haben.
        # Einfacher Weg: Wir schreiben Zeile für Zeile.
        
        Write-GuiLog "Lade Kill-Skript auf ESXi (/tmp/ghetto_assassin.sh)..."
        $uploadCmd = "cat > /tmp/ghetto_assassin.sh << 'EOF'`n$killScriptContent`nEOF"
        Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command $uploadCmd | Out-Null
        
        # 2. Ausführbar machen
        Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command "chmod +x /tmp/ghetto_assassin.sh" | Out-Null
        
        # 3. STARTEN & VERGESSEN (Fire and Forget)
        # Wir starten es im Hintergrund und leiten allen Output um. 
        # Die SSH Session wartet so NICHT auf das Ende.
        Write-GuiLog "Führe Not-Aus aus (Background Task)..."
        $fireCmd = "/tmp/ghetto_assassin.sh > /dev/null 2>&1 &"
        Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command $fireCmd | Out-Null

        Write-GuiLog "Befehl gesendet. Abbruch läuft im Hintergrund."
        Write-GuiLog "Warte 3 Sekunden..."
        Start-Sleep -Seconds 3
        
        # Optional: Prüfen ob noch was läuft? Nein, wir vertrauen dem Assassin.

    } catch {
        Write-GuiLog "Fehler beim Senden des Abbruch-Signals: $($_.Exception.Message)"
    } finally {
        ...
        Write-GuiLog "Systemstatus: Reset. (Prüfen Sie ggf. das Log manuell, Snapshot, Temp-Clone und Zielordner müssen manuell gelöscht werden. GhettoGUi muss getrennt und neu verbunden werden.)"
    }
}
# -----------------------------------------------
# - Manuell GhettoVBC Backup Start --------------
# -----------------------------------------------
...
function Save-GhettoVCBConfig {
    ...
    $baseBackupPath = $textboxBackupVol.Text.TrimEnd('/'); $subfolder = $textboxSubfolder.Text.Trim().Trim('/'); $fullBackupPath = if (-not [string]::IsNullOrWhiteSpace($subfolder)) { "$baseBackupPath/$subfolder" } else { $baseBackupPath }
	...
	$confLines = @(
        ...
        "POWER_VM_DOWN_BEFORE_BACKUP=0", "ENABLE_HARD_POWER_OFF=0", "ITER_TO_WAIT_SHUTDOWN=3", "POWER_DOWN_TIMEOUT=5", "SNAPSHOT_TIMEOUT=30", "ENABLE_COMPRESSION=0",
