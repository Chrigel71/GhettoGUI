# GhettoGUI_V7.8.1.ps1
#
# --- VERSION V7.8.1 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V7.8.1 / 10.10.2025"
...
$buttonBrowseRestoreTarget.Add_Click({
    ...
    if ($selection) {
        if ($selection -match '\((/vmfs/volumes/.+?)\)') {
            $targetDatastorePath = $Matches[1]
            $textboxRestoreTargetPath.Text = $targetDatastorePath
            Write-GuiLog "Restore-Ziel ausgewählt: $targetDatastorePath"
        } else {
             Write-GuiLog "Konnte Pfad aus Auswahl nicht extrahieren."
        }
    } else {
...
$buttonSaveSchedule.Add_Click({
    ...
    try {
        ...
        if ($radioScheduleReplication.Checked) {
            ...
            # FINALE KORREKTUR: Liest die Einstellung aus den zuverlässigen, zentralen Skript-Variablen.
            $tempPathForSchedule = if ($script:replicationUseLocalTemp) { "" } else { $script:replicationTempPath.TrimEnd('/') }
            ...
            $remoteLogFile = "$ghettoPath/logs/backup-run-$jobId-`$(date +\%F).log"

            $useFixedDirValue = if ($checkboxFixedBackupDir.Checked) { 1 } else { 0 }
            $confLines = @(
                "USE_FIXED_BACKUP_DIR=$useFixedDirValue", "VM_BACKUP_VOLUME=`"$($textboxBackupVol.Text.TrimEnd('/'))/$($textboxSubfolder.Text.Trim('/'))`"", "VM_BACKUP_ROTATION_COUNT=$($textboxRotation.Text)", "DISK_BACKUP_FORMAT=`"$($comboboxDiskFormat.SelectedItem.ToString())`"",
                "VM_SNAPSHOT_MEMORY=$(if ($checkboxSnapMem.Checked) { 1 } else { 0 })", "VM_SNAPSHOT_QUIESCE=$(if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 })", "EMAIL_LOG=$(if ($checkboxEmailLog.Checked) { 1 } else { 0 })", "EMAIL_SERVER=`"$($textboxEmailServer.Text)`"",
                "EMAIL_SERVER_PORT=`"$($textboxEmailPort.Text)`"", "EMAIL_USER_NAME=`"$($textboxEmailUser.Text)`"", "EMAIL_USER_PASSWORD=`"$($textboxEmailPassword.Text)`"", "EMAIL_FROM=`"$($textboxEmailFrom.Text)`"", "EMAIL_TO=`"$($textboxEmailTo.Text)`"", "EMAIL_SUBJECT=`"$($textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName))`"", "EMAIL_BIN=`"$ghettoPath/sendmail`""
            )
            ...
            # KORREKTUR: Tippfehler 'ESXiSfidSshCredential' zu 'ESXiSshCredential' korrigiert
            $sftp = New-SFTPSession -ComputerName $Global:ESXiConnectedHostName -Credential $Global:ESXiSshCredential -AcceptKey -ConnectionTimeout 60
            try { 
                Set-SFTPContent -SFTPSession $sftp -Path $remoteGhettoConf -Value $ghettoConfContent -Encoding UTF8
                Set-SFTPContent -SFTPSession $sftp -Path $remoteVmListFile -Value $unixVmListText -Encoding UTF8 
            } finally { if ($sftp) { Remove-SFTPSession -SFTPSession $sftp } }
            
            $ghettoCommand = "'$remoteGhettoScript' -f '$remoteVmListFile' -g '$remoteGhettoConf' -l '$remoteLogFile'"
            $cronCommand = "$scheduleMinute $scheduleHour * * $selectedDays rm -f '$remoteLogFile'; $ghettoCommand $cronComment"
        }
...
function Load-HostGuiSettings {
    ...
    try {
        ...
        # Setzt die Temp-Einstellungen bei jedem Ladevorgang explizit zurück oder auf den Wert aus der Datei.
        if ($settings.PSObject.Properties.Name -contains 'ReplUseLocalTemp') {
        ...
        if ($settings.PSObject.Properties.Name -contains 'ReplTempPath') {
            $textboxDrTempPath.Text = $settings.ReplTempPath
            $script:replicationTempPath = $settings.ReplTempPath
        } else {
            $textboxDrTempPath.Text = ""
            $script:replicationTempPath = ""
        }
        # --- ENDE DER KORREKTUR ---

        $textboxDrTargetHost.Text = if($settings.PSObject.Properties.Name -contains 'ReplTargetHost') { $settings.ReplTargetHost } else { '' }
        $textboxDrTargetDs.Text = if($settings.PSObject.Properties.Name -contains 'ReplTargetDatastore') { $settings.ReplTargetDatastore } else { '' }
        $textboxDrSuffix.Text = if($settings.PSObject.Properties.Name -contains 'ReplSuffix') { $settings.ReplSuffix } else { '' }
