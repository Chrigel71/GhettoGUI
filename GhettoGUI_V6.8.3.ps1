...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V6.8.3"
...
# In GhettoGUI_V6.7.5.ps1 an beiden Stellen ersetzen:
# In GhettoGUI_V6.7.5.ps1 an beiden Stellen ersetzen:
...
'; done; IFS=$OLD_IFS
        ...
        TAR_PID=$!; log "  -> Transferprozess gestartet mit PID: ${TAR_PID}."; while kill -0 ${TAR_PID} >/dev/null 2>&1; do PROGRESS=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}' 2>/dev/null | awk '{print \$1}'"); if [ -n "${PROGRESS}" ]; then log " -> Transfer-Ziel: ${PROGRESS} von ~${SOURCE_SIZE}"; fi; sleep 30; done; wait ${TAR_PID}; TRANSFER_EXIT_CODE=$?; if [ ${TRANSFER_EXIT_CODE} -ne 0 ]; then log_error "[${SOURCE_VM_NAME}] - TAR-Transfer fehlgeschlagen!"; exit 1; fi; rm -rf "${TEMP_CLONE_DIR}"
    else
...
function Save-HostGuiSettings {
    ...
    # NEU: Einstellungen für Temp-Pfad hinzufügen
    $settings.Add("ReplUseLocalTemp", $checkboxUseLocalTemp.Checked)
    $settings.Add("ReplTempPath", $textboxDrTempPath.Text)
...
function Load-HostGuiSettings {
    ...
    if (Test-Path $settingsFile) {
        ...
        try {
            ...
            # NEU: Gespeicherte Temp-Pfad-Einstellungen laden
