...
# Use for GhettoGUI_V8.9.0 & sendmail 9.9 GettoVCB_patch 9.8.4 Christian Furrer, 24.02.2026
# Fixer Pfad O.K
...
#This Feature enables the device to perform tasks (Deletes/data save for large files) and has the script wait for the NAS to catchup. #This code has been in production on the authors systems for the last 2 years.
...
# Set this value to determine how many times the script tries to work arround I/O errors each time the NAS slows down. # The script will skip past this loop if the NAS is responsive.
...
# This is a Brute-force/Mandatory delay added on top of any delay imposed by the NFS_IO_Hack. # Set a delay timer to allow the NFS server to catch up to GhettoVCB's stream, when the NAS isn't responding timely. # This acts like a cooldown period for the NAS.
...
sanityCheck() {
    ...
    # Der Befehl 'dirname' extrahiert den Verzeichnispfad aus dem vollen Log-Pfad.
    # 'mkdir -p' erstellt das Verzeichnis und unterdrückt Fehler, falls es schon existiert.
    LOG_DIR=$(dirname "${LOG_OUTPUT}")
    ...
    if [[ -n "${VM_BACKUP_VOLUME}" ]]; then
        ...
        # --- FIX: Basis-Ordner erstellen, falls er fehlt ---
        if [ ! -d "${VM_BACKUP_VOLUME}" ]; then
            logger "info" "Backup-Basisverzeichnis fehlt. Versuche zu erstellen: ${VM_BACKUP_VOLUME}"
            if ! mkdir -p "${VM_BACKUP_VOLUME}"; then
                logger "info" "ERROR: Backup-Pfad konnte nicht erstellt werden: ${VM_BACKUP_VOLUME}"
                exit 20
            fi
        fi
        # ---------------------------------------------------
...
'
    for i in ${LIST_BACKUPS}; do
	    ...
	    RETVAL=$? # Added the NFS_IO_HACK check and function call here.  Also set the script to function the same, if the new feature is turned off.
...
ghettoVCB() {
    ...
    echo "  - Typ: GhettoVCB Backup V8.9.0 v9.8.4" >> "${LOG_OUTPUT}"
...
if [[ "${USE_FIXED_BACKUP_DIR}" -eq 1 ]]; then
    ...
    # --- Sicherheits-Checks & Löschen (dein bestehender Case) ---
    ...
    case "${VM_BACKUP_DIR}" in
        "${VM_BACKUP_VOLUME}"/*) : ;;
        *) logger "info" "ERROR: Refusing to delete outside VM_BACKUP_VOLUME (${VM_BACKUP_VOLUME})"; exit 31 ;;
    esac
...
buildHeaders() {
    ...
    echo -ne "X-Mailer: ghettoVCB ${VERSION_STRING}\r\n" >> "${EMAIL_LOG_HEADER}"
    echo -en "\r\n" >> "${EMAIL_LOG_HEADER}"
