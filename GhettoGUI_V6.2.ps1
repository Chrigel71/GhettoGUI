...
# - FIX: Master-Helper-Skript (V6.2.0.0) mit speziellem Diagnose-Schritt
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V6.2.0.0"
...
#!/bin/sh
# GhettoGUI Master Replication Helper V12.0.0 (Definitive VMX Fix)
set -e
set -o pipefail
...
cleanup() {
    ...
    # Der E-Mail-Versand gehört hierher, damit er IMMER ausgeführt wird.
...
send_email_notification() {
    ...
    # Führt ls nur aus, wenn der Transfer erfolgreich war
...
grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)' | while IFS= read -r line; do
    ...
    log "    -> Klone (lokal): ${SOURCE_DISK_PATH} nach ${TEMP_DESTINATION_DISK_PATH}"
...
# Schritt 1: Finde die originale VMX-Datei
...
# Schritt 2: Benenne NUR die VMX-Datei um
RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"
mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"

# Schritt 3: Korrigiere den Inhalt der neuen VMX-Datei
