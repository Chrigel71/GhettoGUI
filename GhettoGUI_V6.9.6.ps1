...
#   Jobs individuell Speichern und Restore Butten
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V6.9.6"
...
$buttonDirectReplicate.Location = New-Object System.Drawing.Point(260, 125)
...
$buttonRestore.Location = New-Object System.Drawing.Point(($buttonApplyVms.Right + 30), $gcOffsetY)
$buttonRestore.Size = New-Object System.Drawing.Size(80, 25)
...
$directReplicationForm.Size = New-Object System.Drawing.Size(420, 370) # Höhe angepasst für neue Optionen
...
# --- Auswahl des temporären Verzeichnisses ---
$checkboxUseLocalTemp = New-Object System.Windows.Forms.CheckBox
$checkboxUseLocalTemp.Text = "Lokalen VM-Temp verwenden (Standard)"
$checkboxUseLocalTemp.Location = New-Object System.Drawing.Point(20, $drCurrentY)
$checkboxUseLocalTemp.AutoSize = $true
$checkboxUseLocalTemp.Checked = $true
...
# --- Auswahl der Replikationsmethode ---
...
# --- Button-Definitionen ---
$buttonSaveInPopup = New-Object System.Windows.Forms.Button; $buttonSaveInPopup.Text = "Einst. speichern"; $buttonSaveInPopup.Size = New-Object System.Drawing.Size(120, 25); $buttonSaveInPopup.Location = New-Object System.Drawing.Point(50, $drCurrentY)
...
# --- HIER IST DIE FEHLENDE LOGIK FÜR DIE NEUEN ELEMENTE ---
$buttonSaveInPopup.Add_Click({
    Write-GuiLog "Speichere Einstellungen aus Replikations-Dialog..."
    Save-HostGuiSettings
})
...
ESXI 6.0:  cat /var/spool/cron/crontabs/root     Cron starten mit "crond"
"@
...
# --- Definition des Restore-Fensters ---
$restoreForm = New-Object System.Windows.Forms.Form
$restoreForm.Text = "VM Wiederherstellungs-Assistent"
$restoreForm.Size = New-Object System.Drawing.Size(550, 380) # Höhe angepasst für neue Optionen
...
# --- Ziel-Datastore ---
$labelRestoreTargetDs = New-Object System.Windows.Forms.Label; $labelRestoreTargetDs.Text = "Ziel-Datastore für die Wiederherstellung:"; $labelRestoreTargetDs.Location = New-Object System.Drawing.Point(20, ($restoreCurrentY + 3)); $labelRestoreTargetDs.AutoSize = $true
...
# --- Neuer VM-Name ---
$labelRestoreNewVmName = New-Object System.Windows.Forms.Label; $labelRestoreNewVmName.Text = "Neuer Name für die wiederhergestellte VM:"; $labelRestoreNewVmName.Location = New-Object System.Drawing.Point(20, ($restoreCurrentY + 3)); $labelRestoreNewVmName.AutoSize = $true
...
# --- NEU: Restore-Optionen ---
$groupRestoreOptions = New-Object System.Windows.Forms.GroupBox; $groupRestoreOptions.Text = "Optionen"; $groupRestoreOptions.Location = New-Object System.Drawing.Point(20, $restoreCurrentY); $groupRestoreOptions.Size = New-Object System.Drawing.Size(500, 100)

$radioRestoreMoved = New-Object System.Windows.Forms.RadioButton; $radioRestoreMoved.Text = "Wiederherstellung als 'verschoben' (behält UUID & MAC-Adresse bei - Standard)"; $radioRestoreMoved.Location = New-Object System.Drawing.Point(15, 25); $radioRestoreMoved.AutoSize = $true; $radioRestoreMoved.Checked = $true
$radioRestoreCopied = New-Object System.Windows.Forms.RadioButton; $radioRestoreCopied.Text = "Wiederherstellung als 'Kopie' (erstellt neue UUID & MAC-Adresse)"; $radioRestoreCopied.Location = New-Object System.Drawing.Point(15, 50); $radioRestoreCopied.AutoSize = $true
$checkboxPowerOnAfterRestore = New-Object System.Windows.Forms.CheckBox; $checkboxPowerOnAfterRestore.Text = "VM nach der Wiederherstellung automatisch einschalten"; $checkboxPowerOnAfterRestore.Location = New-Object System.Drawing.Point(15, 75); $checkboxPowerOnAfterRestore.AutoSize = $true
...
$buttonStartRestore = New-Object System.Windows.Forms.Button; $buttonStartRestore.Text = "Wiederherstellung starten"; $buttonStartRestore.Location = New-Object System.Drawing.Point(230, $restoreCurrentY); $buttonStartRestore.Size = New-Object System.Drawing.Size(170, 25); $buttonStartRestore.DialogResult = [System.Windows.Forms.DialogResult]::OK
...
# --- Start Klick-Event für den Haupt-Restore-Button  ---

# --- Start Klick-Event für den Haupt-Restore-Button  ---
$buttonRestore.Add_Click({
    if (-not ($Global:ESXiSession -and $Global:ESXiSession.Connected)) {
        [System.Windows.Forms.MessageBox]::Show("Bitte zuerst mit dem ESXi-Host verbinden, auf dem die Wiederherstellung stattfinden soll.", "Fehler", "OK", "Warning"); return
    }
    
    # Setze das Formular zurück für einen sauberen Start
    $textboxRestoreSourcePath.Text = ""; $textboxRestoreTargetPath.Text = ""; $textboxRestoreNewVmName.Text = ""
    $checkboxPowerOnAfterRestore.Checked = $false; $radioRestoreMoved.Checked = $true

    # Öffnet das Restore-Fenster
    $restoreResult = $restoreForm.ShowDialog($form)
    
    if ($restoreResult -eq [System.Windows.Forms.DialogResult]::OK) {
        # --- Parameter aus dem Formular sammeln (ERWEITERT FÜR E-MAIL) ---
        $uuidAction = if ($radioRestoreMoved.Checked) { "keep" } else { "create" }
        $params = @{
            SourcePath      = $textboxRestoreSourcePath.Text
            TargetDatastore = $textboxRestoreTargetPath.Text
            NewVmName       = $textboxRestoreNewVmName.Text
            PowerOn         = if ($checkboxPowerOnAfterRestore.Checked) { 1 } else { 0 }
            UuidAction      = $uuidAction
            UniqueId        = "{0}-{1}" -f (Get-Date -Format 'yyyyMMddHHmmss'), (Get-Random -Minimum 100 -Maximum 999)
            # NEU: Alle E-Mail und Ghetto-Pfad Parameter hinzufügen
            ...
            EmailSubject    = "[Restore] " + $textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName)
        }

        # Merke dir Name und Typ für die Abschlussmeldung
        $Global:currentJobTargetName = $params.NewVmName
        $Global:currentJobType = "Wiederherstellung"

        # --- Sicherheitsabfrage ---
        $confirmMessage = @"
Bist Du sicher, dass Du die Wiederherstellung starten möchtest?

- Quell-Backup: $($params.SourcePath)
- Ziel-Datastore: $($params.TargetDatastore)
- Neuer VM-Name: $($params.NewVmName)
"@
        $confirmResult = [System.Windows.Forms.MessageBox]::Show($confirmMessage, "Wiederherstellung bestätigen", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question)

        if ($confirmResult -ne [System.Windows.Forms.DialogResult]::Yes) {
            Write-GuiLog "Wiederherstellung vom Benutzer abgebrochen."
            return # Bricht den Vorgang hier ab
        }

        # --- Validierung ---
        if ([string]::IsNullOrWhiteSpace($params.SourcePath) -or [string]::IsNullOrWhiteSpace($params.TargetDatastore) -or [string]::IsNullOrWhiteSpace($params.NewVmName)) {
            [System.Windows.Forms.MessageBox]::Show("Bitte fülle alle Felder aus: Backup-Quelle, Ziel-Datastore und Neuer VM-Name.", "Fehlende Eingaben", "OK", "Warning"); return
        }

        # --- Shell-Skript-Vorlage für den Restore (JETZT MIT E-MAIL FUNKTION) ---
...
send_email_notification() {
    ...
    log_raw "Backup Duration: ${DURATION_MSG}"; # Der Name ist historisch, passt aber
    log_raw "${FINAL_STATUS_MSG}";
    if [ "${EMAIL_ENABLED}" = "1" ] && [ -f "${SENDMAIL_PATH}" ]; then
        RECIPIENTS=$(echo "${EMAIL_TO}" | sed 's/,/ /g');
        python ${SENDMAIL_PATH} -f "${EMAIL_FROM}" -s "${EMAIL_SERVER}" -S "${EMAIL_PORT}" -j "${EMAIL_SUBJECT} - ${FINAL_STATUS_MSG}" -m "${LOG_FILE}" -u "${EMAIL_USER}" -p "${EMAIL_PASS}" ${RECIPIENTS} >> ${LOG_FILE} 2>&1;
    fi;
}
...
# KORREKTUR: Finde den eigentlichen Backup-Ordner, falls ein übergeordneter Ordner ausgewählt wurde.
...
'@
        ...
        # --- Skript vorbereiten und Job starten (ERWEITERT FÜR E-MAIL) ---
        $finalRestoreScript = $restoreScriptTemplate `
            -replace '__SOURCE_PATH__', $params.SourcePath `
            -replace '__TARGET_DATASTORE__', $params.TargetDatastore `
            -replace '__NEW_VM_NAME__', $params.NewVmName `
            -replace '__POWER_ON__', $params.PowerOn `
            -replace '__UUID_ACTION__', $params.UuidAction `
        ...
        $remoteScriptPath = "/tmp/ghetto_restore_$($params.UniqueId).sh"
        $launcherScriptPath = "/tmp/ghetto_restore_launcher_$($params.UniqueId).sh"

        $sftpSession = $null
        try {
            $sftpSession = New-SFTPSession -ComputerName $Global:ESXiConnectedHostName -Credential $Global:ESXiSshCredential
            Set-SFTPContent -SFTPSession $sftpSession -Path $remoteScriptPath -Value ($finalRestoreScript.Replace("`r`n", "`n"))
            Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command "chmod +x '$remoteScriptPath'"
            $launcherContent = "#!/bin/sh`nnohup sh '$remoteScriptPath' > /dev/null 2>&1 &"
            ...
            [System.Windows.Forms.MessageBox]::Show("Fehler beim Hochladen des Restore-Skripts: $($_.Exception.Message)", "Upload-Fehler", "OK", "Error"); return
        } finally {
            if ($sftpSession) { Remove-SFTPSession -SFTPSession $sftpSession }
        }

        Get-Job | Where-Object { $_.Name -like 'GhettoRestoreWatcher' } | Remove-Job -Force
        $startCommand = "sh '$launcherScriptPath'"
        Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command $startCommand

        $watcherParams = @{
            Host = $Global:ESXiConnectedHostName; Credential = $Global:ESXiSshCredential; RemoteLogPath = "/tmp/ghetto_restore_$($params.UniqueId).log"
        }
        $Global:replicationJob = Start-Job -Name 'GhettoRestoreWatcher' -ScriptBlock { 
            param($p)
            Import-Module Posh-SSH -ErrorAction SilentlyContinue
            $watcherSession = New-SSHSession -ComputerName $p.Host -Credential $p.Credential -AcceptKey
            if (-not $watcherSession.Connected) { Write-Output "FEHLER: Watcher-Job konnte keine SSH-Verbindung herstellen."; return }
            $lastLineNumber = 0; $timeout = (Get-Date).AddHours(4)
            while ((Get-Date) -lt $timeout) {
                ...
                if ((Invoke-SSHCommand -SSHSession $watcherSession -Command $checkFileCmd).Output -join '' -eq 'EXISTS') {
                    ...
                    if ($newLinesResult.Output) {
                        ...
                        if ($lines[-1] -match "====== WIEDERHERSTELLUNG ERFOLGREICH ABGESCHLOSSEN ======") { break }
                    }
                }
            }
        ...
        $Global:replicationJobTimer.Start()
        Write-GuiLog "Restore-Prozess für '$($params.NewVmName)' gestartet. Log wird live überwacht."
    } else {
        Write-GuiLog "Wiederherstellung vom Benutzer abgebrochen."
    }
})
...
$buttonBrowseRestoreSource.Add_Click({
    ...
    while ($true) {
        ...
        # Fall 1: Benutzer klickt "Diesen Ordner auswählen"
        if ($dialogOutput.Result -eq 'Yes') {
            ...
            # Automatisch einen neuen VM-Namen vorschlagen, basierend auf dem letzten Teil des Pfades
            $originalVmName = ($currentPath -split '/')[-1]
            # Entferne eventuelle Datums-Stempel für einen sauberen Namen
            $originalVmName = $originalVmName -replace '-\d{4}-\d{2}-\d{2}_\d{2}-\d{2}-\d{2}$',''
            $textboxRestoreNewVmName.Text = "$($originalVmName)-Restored"
            break
        }
        
        # Fall 2: Benutzer klickt "Öffnen" oder doppelklickt
        elseif ($dialogOutput.Result -eq 'OK') {
            $selectedItem = $dialogOutput.SelectedItem
            if ($selectedItem -eq ".. (Eine Ebene höher)") {
                # Navigiere eine Ebene nach oben, aber nicht über /vmfs/volumes hinaus
                if ($currentPath.Length -gt 15) { 
        ...
        # Fall 3: Benutzer klickt "Abbrechen" oder schliesst das Fenster
        else {
...
$buttonSaveSchedule.Add_Click({
    ...
    if ($radioScheduleReplication.Checked) {
        if ([string]::IsNullOrWhiteSpace($textboxDrTargetHost.Text) -or [string]::IsNullOrWhiteSpace($textboxDrTargetDs.Text)) {
        ...
        if (-not $checkboxUseLocalTemp.Checked -and [string]::IsNullOrWhiteSpace($textboxDrTempPath.Text)) {
            [System.Windows.Forms.MessageBox]::Show("Wenn 'Lokalen VM-Temp verwenden' deaktiviert ist, muss ein temporärer Speicherpfad angegeben werden.", "Fehlende Konfiguration", "OK", "Warning"); return
        }
    }
    ...
    try {
        ...
        if ($radioScheduleReplication.Checked) {
            ...
            Write-GuiLog "-> Geplante Methode: $selectedMethod"
            ...
            $params = @{
                ...
                EmailUser         = $textboxEmailUser.Text;
                EmailPass         = $textboxEmailPassword.Text;
                EmailSubject      = "[Scheduled-Replication] " + $textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName);
                VmList            = ($checkedListBoxVms.CheckedItems | ForEach-Object { $_.OriginalName }) -join "`n"
            }
# In GhettoGUI_V6.8.5.ps1 an beiden Stellen ersetzen:

$masterHelperScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V40.6 (PowerOff Fix)
# - FIXED: power.off command will not fail the script if the VM is already off.
# - ADDED: Proactively unregister and power off existing replica on target host before starting.
# - ADDED: Garantierte Snapshot-Entfernung in der Cleanup-Routine bei Fehlern.
set -e
set -o pipefail
# Parameter
...
cleanup() {
    ...
    if [ "${EXIT_CODE}" -ne 0 ] && [ -n "${VMID}" ] && [ "${REPLICATION_METHOD}" = "robust" ]; then
        SNAPSHOT_EXISTS=$(/bin/vim-cmd vmsvc/snapshot.get ${VMID} 2>/dev/null | grep "ghetto-repl-${UNIQUE_ID}")
        if [ -n "${SNAPSHOT_EXISTS}" ]; then
            log_warn "FEHLER erkannt. Bereinige Snapshot auf Quell-VM ${SOURCE_VM_NAME} (VMID ${VMID})..."
...
'; for line in ${DISK_DEFINITIONS}; do
            ...
            VMKF_PID=$!; log "      -> Klon-Prozess gestartet mit PID: ${VMKF_PID}."; while kill -0 ${VMKF_PID} >/dev/null 2>&1; do if [ -f "${TEMP_DESTINATION_DISK_PATH}" ]; then log " -> Klon-Fortschritt: $(du -h "${TEMP_DESTINATION_DISK_PATH}" | awk '{print $1}')"; fi; sleep 15; done; wait ${VMKF_PID}; CLONE_EXIT_CODE=$?; if [ ${CLONE_EXIT_CODE} -ne 0 ]; then log_error "[${SOURCE_VM_NAME}] - Klonen fehlgeschlagen!"; exit 1; fi
        IFS='
'; done; IFS=$OLD_IFS
...
'@
            ...
            $sftpSession = New-SFTPSession -ComputerName $Global:ESXiConnectedHostName -Credential $Global:ESXiSshCredential
            try {
                Set-SFTPContent -SFTPSession $sftpSession -Path $remoteStarterScriptPath -Value ($finalMasterScript.Replace("`r`n", "`n"))
            ...
            # Logik für geplante Backups (unverändert)
...
# Wir verwenden die gleiche Multi-VM-Vorlage wie bei der geplanten Replikation
# In GhettoGUI_V6.8.5.ps1 an beiden Stellen ersetzen:
...
'@
    ...
    try {
        ...
        $sftpSession = New-SFTPSession -ComputerName $params.SourceHost -Credential $Global:ESXiSshCredential -AcceptKey
...
function Save-HostGuiSettings {
    ...
    if (-not $FilePath) {
        Write-GuiLog "FEHLER: Kein Dateipfad zum Speichern angegeben."
        return
    }
    ...
    $settings = @{
        ...
        # NEU: Speichert den Typ des Zeitplans (welcher Radio-Button ist aktiv)
        ScheduleType = if ($radioScheduleBackup.Checked) { 'Backup' } else { 'Replication' };
...
function Load-HostGuiSettings {
    ...
    if (-not (Test-Path $FilePath)) {
        Write-GuiLog "FEHLER: Die Einstellungsdatei '$FilePath' wurde nicht gefunden."
        return
    }
    ...
    try {
        ...
        # Lade alle Einstellungen in die GUI-Felder
        ...
        # --- KORRIGIERTE LOGIK FÜR DEN ZEITPLAN ---
        ...
        # Liest den Typ des Zeitplans direkt aus und setzt den korrekten Radio-Button
        if ($settings.ScheduleType -eq 'Replication') {
            $radioScheduleReplication.Checked = $true
        } else {
            # Standard ist 'Backup', auch wenn die Eigenschaft fehlen sollte
            $radioScheduleBackup.Checked = $true
        }
        # --- ENDE KORREKTUR ---
        ...
        if ($settings.PSObject.Properties.Name -contains 'ReplMethod') {
            if ($settings.ReplMethod -eq 'vmkfstools') { $directReplicationForm.Controls.Find('radioVmkf', $true)[0].Checked = $true } 
            else { $directReplicationForm.Controls.Find('radioTar', $true)[0].Checked = $true }
        }
# NEU: Lade zuerst die VM-Liste vom Host, DANN synchronisiere die Haken.
        if ($Global:ESXiSession -and $Global:ESXiSession.Connected) {
            ...
            Load-VMListFromESXi # Füllt die checkedListBoxVms
            ...
            Write-GuiLog "Keine Host-Verbindung aktiv. VM-Auswahl konnte nicht synchronisiert werden. Bitte zuerst verbinden und VMs laden."
        }
...
function Load-HostGuiSettings {
    ...
    try {
        ...
        # Automatisch die VM-Liste vom Host abrufen (Logik aus alter, funktionierender Version)
