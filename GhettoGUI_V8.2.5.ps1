# GhettoGUI_V8.2.5.0.ps1
#
# --- VERSION V8.2.5.0 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V8.2.5.0 / 31.12.2025"
...
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V54.1 (Clean Logs)
# - OPTION: Added '-o LogLevel=ERROR' to SSH commands to suppress "Permanently added..." warnings.
# - RETAIN: Sequential Disk Processing (Clone->Transfer->Delete) for massive VMs.
# - RETAIN: All previous fixes (VMX, Size-Calc, Error-Handling).
...
# FIX: LogLevel=ERROR suppresses "Permanently added..." warnings for cleaner logs
SSH_OPTIONS="-T -i /.ssh/id_ecdsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=20 -o Compression=no -o LogLevel=ERROR"; SNAP_MEM=__SNAP_MEM__; SNAP_QUIESCE=__SNAP_QUIESCE__; EMAIL_ENABLED=__EMAIL_ENABLED__; EMAIL_TO='__EMAIL_TO__'; EMAIL_FROM='__EMAIL_FROM__'; EMAIL_DISPLAY_NAME='__EMAIL_DISPLAY_NAME__'; EMAIL_SERVER='__EMAIL_SERVER__'; EMAIL_PORT='__EMAIL_PORT__'; EMAIL_USER='__EMAIL_USER__'; EMAIL_PASS='__EMAIL_PASS__'; EMAIL_SUBJECT='__EMAIL_SUBJECT__'; SENDMAIL_PATH="${GHETTO_PATH}/sendmail"; VM_LIST='
__VM_LIST__
'; OVERALL_STATUS="OK";
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        # HOT-KLON PATH (SEQUENTIAL PROCESSING V54.0)
        ...
        while read -r line; do
            ...
            log "     ...Klonen nach Temp..."
            /sbin/vmkfstools -i "${SOURCE_DISK_PATH_FOR_CLONE}" -d thin "${DESTINATION_DISK_PATH}" > /dev/null 2>&1
            
            # 2. Transferieren (Sofort!)
            log "     ...Transferieren..."
            SOURCE_SIZE_TAR=$(du -sh "${TEMP_CLONE_DIR}" | awk '{print $1}')
            # tar . greift alle Dateien im Ordner (wir löschen sie ja gleich, also ist immer nur die aktuelle Disk drin)
            ( (cd "${TEMP_CLONE_DIR}" && tar -cf - .) | ssh ${SSH_OPTIONS} root@${TARGET_HOST} "tar -xf - -C '${TARGET_VM_PATH}'" )
            if [ $? -ne 0 ]; then log_error "Fehler beim Transfer von Disk ${DISK_IDX}"; OVERALL_STATUS="ERROR"; fi
            
            # 3. Löschen (Platz freimachen für nächste Disk)
            log "     ...Temp-Dateien bereinigen..."
            rm -rf "${TEMP_CLONE_DIR}"/*
            
            # 4. Patch Script aktualisieren
        ...
        # COLD COPY PATH (Bleibt wie bisher, da kein lokaler Temp-Space genutzt wird)
...
#---------------------------------------------------------------------------------
# GhettoGUI Multi-VM Replication Helper (Manuell) - V54.4 Fix
# - FIX: Variablen-Definition (UNIQUE_ID) steht nun VOR dem Lock-Check.
#        Verhindert, dass das Skript wegen leerer ID sofort beendet.
# - RETAIN: Live-Fortschritt für Klonen & Transfer, Sequentielle Verarbeitung.
#---------------------------------------------------------------------------------

#---------------------------------------------------------------------------------
# GhettoGUI Multi-VM Replication Helper (Manuell) - V54.5 Vintage Progress
# - FEATURE: Zeigt beim Klonen wieder die %-Anzeige von vmkfstools an ("Clone: 50% done").
# - FIX: Umgeht das Problem, dass 'du -sh' bei sparse files oft lange "4.0K" anzeigt.
# - RETAIN: Parameter-Fix V54.4, Sequential Processing, Live-Transfer.
#---------------------------------------------------------------------------------

$multiVmScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V54.5 (Manual Mode)
# - Unified Logic with Percentage Progress for Cloning.

PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH

# 1. PARAMETER DEFINIEREN (Zwingend VOR dem Lock-Check!)
UNIQUE_ID='__UNIQUE_ID__'; TARGET_HOST='__TARGET_HOST__'; TARGET_DATASTORE='__TARGET_DATASTORE__'; VM_SUFFIX='__VM_SUFFIX__'; REPLICATION_METHOD='__REPLICATION_METHOD__'; GHETTO_PATH='__GHETTO_PATH__';
LOG_FILE="${GHETTO_PATH}/logs/master_replication_${UNIQUE_ID}.log";
# FIX: LogLevel=ERROR unterdrückt "Permanently added..." Warnungen
SSH_OPTIONS="-T -i /.ssh/id_ecdsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=20 -o Compression=no -o LogLevel=ERROR"; SNAP_MEM=__SNAP_MEM__; SNAP_QUIESCE=__SNAP_QUIESCE__; EMAIL_ENABLED=__EMAIL_ENABLED__; EMAIL_TO='__EMAIL_TO__'; EMAIL_FROM='__EMAIL_FROM__'; EMAIL_DISPLAY_NAME='__EMAIL_DISPLAY_NAME__'; EMAIL_SERVER='__EMAIL_SERVER__'; EMAIL_PORT='__EMAIL_PORT__'; EMAIL_USER='__EMAIL_USER__'; EMAIL_PASS='__EMAIL_PASS__'; EMAIL_SUBJECT='__EMAIL_SUBJECT__'; SENDMAIL_PATH="${GHETTO_PATH}/sendmail"; VM_LIST='
__VM_LIST__
'; OVERALL_STATUS="OK";
TEMP_CLONE_BASE_PATH='__TEMP_CLONE_BASE_PATH__';
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    # --- BRANCHING ---
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        # HOT-KLON PATH (SEQUENTIAL PROCESSING V54.5)
        if [ -z "${TEMP_CLONE_BASE_PATH}" ]; then TEMP_CLONE_CHECK_PATH="${VMX_DIR}"; else TEMP_CLONE_CHECK_PATH="${TEMP_CLONE_BASE_PATH}"; fi
        log "[0/7] Prüfe Speicherplatz für Temp-Klon..."
        SOURCE_SIZE_K=$(du -sk "${VMX_DIR}" | awk '{print $1}')
        ...
        while read -r line; do
            ...
            # 1. Klonen (Mit vmkfstools %-Anzeige)
            log "  -> Processing Disk ${DISK_IDX}: ${SRC_BASENAME_ONLY}"
            log "     ...Klonen nach Temp gestartet..."
            
            # Temp Log für vmkfstools Output erstellen
            VMKF_LOG="/tmp/vmkf_progress_${UNIQUE_ID}_${DISK_IDX}.log"
            
            # vmkfstools starten und Output umleiten
            /sbin/vmkfstools -i "${SOURCE_DISK_PATH_FOR_CLONE}" -d thin "${DESTINATION_DISK_PATH}" > "${VMKF_LOG}" 2>&1 &
            VMKF_PID=$!
            
            while kill -0 ${VMKF_PID} >/dev/null 2>&1; do
                # Greppe die letzte Zeile mit Prozentzeichen
                PROGRESS_LINE=$(grep "%" "${VMKF_LOG}" | tail -n 1)
                
                if [ -n "${PROGRESS_LINE}" ]; then
                     log "     ...${PROGRESS_LINE}"
                else
                     # Fallback auf Größe, falls noch keine % da sind
                     CURRENT_CLONE_SIZE=$(du -sh "${DESTINATION_DISK_PATH}" 2>/dev/null | awk '{print $1}')
                     if [ -n "${CURRENT_CLONE_SIZE}" ]; then
                        log "     ...Klonen init: ${CURRENT_CLONE_SIZE}"
                     fi
                fi
                sleep 30
            done
            ...
            if [ ${CLONE_EXIT} -ne 0 ]; then 
                log_error "Fehler beim Klonen von Disk ${DISK_IDX}"
                OVERALL_STATUS="ERROR"
                continue # Transfer überspringen
            fi
            
            # 2. Transferieren (Mit Live-Fortschritt)
            log "     ...Transferieren..."
            SOURCE_SIZE_TAR=$(du -sh "${TEMP_CLONE_DIR}" | awk '{print $1}')
            ...
            while kill -0 ${TAR_PID} >/dev/null 2>&1; do
                ...
                if [ -n "${PROGRESS}" ]; then 
                    log " Transfer: ${PROGRESS} -> (~${SOURCE_SIZE_TAR})"
                fi
                sleep 30
            done
            wait ${TAR_PID}
