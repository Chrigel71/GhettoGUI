# GhettoGUI_V8.2.7.0.ps1
#
# --- VERSION V8.2.7.0 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V8.2.7.0 / 03.01.2026"
...
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V54.2 (Cold-Progress)
# - FEATURE: Added Progress Monitoring for Cold-Copy (SCP).
# - FIX: Cold-Copy Loop persists.
# - FIX: Percentage output for vmkfstools persists.
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    log "#################### Starte Verarbeitung für VM: ${SOURCE_VM_NAME} ########V54.2############"
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        ...
        while read -r line; do
            ...
            while kill -0 ${VMKF_PID} >/dev/null 2>&1; do
                ...
                if [ -n "${PROGRESS_LINE}" ]; then log "     ...${PROGRESS_LINE}"; else CURRENT_CLONE_SIZE=$(du -sh "${DESTINATION_DISK_PATH}" 2>/dev/null | awk '{print $1}'); if [ -n "${CURRENT_CLONE_SIZE}" ]; then log "     ...Klonen init: ${CURRENT_CLONE_SIZE}"; fi; fi
                sleep 30
            done
            ...
            while kill -0 ${TAR_PID} >/dev/null 2>&1; do
                ...
                if [ -n "${PROGRESS}" ]; then log "     ...Transfer läuft: ${PROGRESS} (Temp: ~${SOURCE_SIZE_TAR})"; fi
                sleep 30
            done
        ...
        while read -r line; do
            ...
            # --- START MONITORED SCP FOR FLAT FILE ---
            ...
            while kill -0 ${SCP_PID} >/dev/null 2>&1; do
                 CURRENT_SIZE=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}/${TARGET_FLAT_NAME}' 2>/dev/null | awk '{print \$1}'")
                 if [ -n "${CURRENT_SIZE}" ]; then log "-> Cold-Copy läuft: ${CURRENT_SIZE}"; fi
                 sleep 15
            done
            wait ${SCP_PID}
    ...
    if echo "${REG_OUT}" | grep -qE '^[0-9]+$'; then log "  -> VM erfolgreich registriert (ID: ${REG_OUT})."; elif echo "${REG_OUT}" | grep -q "AlreadyExists"; then log "  -> INFO: VM war bereits registriert."; else log "  -> Registrierung Warnung: ${REG_OUT}"; fi
