# GhettoGUI_V7.3.0.ps1
#
# --- VERSION V7.3.0 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V7.3.0 / 10.09.2025"
...
$radioScheduleReplication = New-Object System.Windows.Forms.RadioButton; $radioScheduleReplication.Text = "Direkte Repl. (H2H)"; $radioScheduleReplication.Location = New-Object System.Drawing.Point(($radioScheduleBackup.Right + 15), $schedulesGcOffsetY); $radioScheduleReplication.AutoSize = $true
...
# --- NEU: Shell-Skript-Vorlage für den HEISS-KLON (V7.2.0.14 - Konfigurierbar) ---
$cloneScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Clone Helper V49.0 (Configurable Snapshot/Disk)
# - ADDED: Now uses Disk Format and Snap Quiesce settings passed from the GUI.
...
# NEU: Zusätzliche Parameter aus der GUI
...
# KORREKTUR: Snap Memory bleibt 0, Snap Quiesce wird von der GUI übernommen
...
'; for line in ${DISK_DEFINITIONS}; do
    ...
    SOURCE_DISK_PATH="${VMX_DIR}/${BASE_DISK_NAME}"; DESTINATION_DISK_PATH="${TARGET_VM_PATH}/${BASE_DISK_NAME}"

    log "  -> Klone Festplatte: ${SOURCE_DISK_PATH} nach ${DESTINATION_DISK_PATH}"
    # KORREKTUR: Disk-Format wird von der GUI übernommen
    vmkfstools -i "${SOURCE_DISK_PATH}" -d ${DISK_FORMAT} "${DESTINATION_DISK_PATH}" >/dev/null 2>&1 &
    VMKF_PID=$!
    log "    -> Klon-Prozess für ${BASE_DISK_NAME} gestartet (PID: ${VMKF_PID})."
    ...
    wait ${VMKF_PID}
    CLONE_EXIT_CODE=$?
    if [ ${CLONE_EXIT_CODE} -ne 0 ]; then log_error "Klonen von Festplatte ${BASE_DISK_NAME} fehlgeschlagen (Exit-Code: ${CLONE_EXIT_CODE})!"; exit 1; fi
...
#!/bin/sh
# GhettoGUI Multi-VM Local Hot-Clone Helper V4.2 (Final Reporting Fix)
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
set -e
# Parameter
...
finish_job() {
    ...
    # Schreibe die finalen Details ins Log, damit der E-Mail-Parser sie findet
    log_raw "Endzeit: ${END_TIME_S}"
    ...
    # Beende das Skript sauber mit Exit-Code 0 (Erfolg)
    exit 0
}
...
log_raw "Job-Konfiguration:"; 
log_raw "Startzeit: ${START_TIME_S}" # Wird für den E-Mail-Parser benötigt
...
for SOURCE_VM_NAME in ${VM_LIST}; do
    ...
    log "initiate backup for ${SOURCE_VM_NAME}" # Wichtig für E-Mail-Parser
...
# --- NEU: Shell-Skript-Vorlage für den GEPLANTEN KALT-RESTORE (Cron-Version) ---
$scheduledRestoreScriptTemplate = @'
#!/bin/sh
# GhettoGUI Scheduled Restore Helper V1.0 (Cron-Safe)
set -e
# Parameter
...
# send_email_notification und cleanup Funktionen... (identisch zum manuellen Skript)
...
cleanup() { EXIT_CODE=$?; log_raw "Speicherplatz (Nachher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(df -h "${TARGET_DATASTORE}" 2>/dev/null | grep /vmfs/)"; log_raw "--- ENDE DES DETAILLOGS ---"; END_TIME_S=$(date '+%Y-%m-%d %H:%M:%S'); log_raw "Endzeit: ${END_TIME_S}"; END_TIME=$(date +%s); DURATION=$((END_TIME - START_TIME)); MINUTES=$((DURATION / 60)); SECONDS=$((DURATION % 60)); DURATION_STRING="${MINUTES} Minuten, ${SECONDS} Sekunden"; if [ ${EXIT_CODE} -ne 0 ] && [ "${OVERALL_STATUS}" = "OK" ]; then log_error "Skript wurde unerwartet beendet. Setze Status auf ERROR."; OVERALL_STATUS="ERROR"; fi; send_email_notification "${DURATION_STRING}"; log "====== VM RESTORE-PROZESS BEENDET (ID: ${UNIQUE_ID}) ======"; }
...
# (Restliche Logik ist identisch, aber der Kopiervorgang hat keine Live-Schleife)
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"; log_raw "Job-Konfiguration:"; log_raw "  - Typ: Geplante GhettoVCB Wiederherstellung"; log_raw "  - Quelle: ${SOURCE_PATH}"; log_raw "  - Ziel-Datastore: ${TARGET_DATASTORE}"; log_raw "  - Neuer VM-Name: ${NEW_VM_NAME}"; log_raw "  - UUID-Aktion: ${UUID_ACTION}"; log_raw "Speicherplatz (Vorher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(df -h "${TARGET_DATASTORE}" 2>/dev/null | grep /vmfs/)"; log_raw "--- START DES DETAILLOGS ---"
...
#!/bin/sh
# GhettoGUI Scheduled Clone Helper V1.0 (Cron-Safe)
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH

set -e
# Parameter (identisch zur manuellen Version)
...
# Logging, send_email_notification und cleanup (identisch zur manuellen Version)
log() { echo "$(date '+%Y-%m-%d %H:%M:%S') -- INFO: $1" >> ${LOG_FILE}; }
log_error() { echo "$(date '+%Y-%m-%d %H:%M:%S') -- ERROR: $1" >> ${LOG_FILE}; }
log_raw() { echo "$1" >> ${LOG_FILE}; }
log_warn() { echo "$(date '+%Y-%m-%d %H:%M:%S') -- WARN: $1" >> ${LOG_FILE}; }
...
'; for line in ${DISK_DEFINITIONS}; do IFS=$OLD_IFS; DISK_FILE_NAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p'); if [ -z "${DISK_FILE_NAME}" ]; then continue; fi; BASE_DISK_NAME=$(basename "${DISK_FILE_NAME}" | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/'); SOURCE_DISK_PATH="${VMX_DIR}/${BASE_DISK_NAME}"; DESTINATION_DISK_PATH="${TARGET_VM_PATH}/${BASE_DISK_NAME}"; log "  -> Klone Festplatte: ${BASE_DISK_NAME}"; vmkfstools -i "${SOURCE_DISK_PATH}" -d ${DISK_FORMAT} "${DESTINATION_DISK_PATH}" >> ${LOG_FILE} 2>&1; log "  -> Klonen der Festplatte ${BASE_DISK_NAME} abgeschlossen."; IFS='
'; done; IFS=$OLD_IFS
...
(cd "${TARGET_VM_PATH}"; ORIG_VMX_FILE=$(find . -maxdepth 1 -name "*.vmx"); ORIG_BASENAME=$(basename "${ORIG_VMX_FILE}" .vmx); if [ "${ORIG_BASENAME}" != "${NEW_VM_NAME}" ]; then for f in "${ORIG_BASENAME}".*; do new_name=$(echo "$f" | sed "s/^${ORIG_BASENAME}/${NEW_VM_NAME}/"); mv -- "$f" "$new_name"; done; fi; NEW_VMX_FILE="./${NEW_VM_NAME}.vmx"; sed -i 's/-[0-9]\{6\}\.vmdk/\.vmdk/g' "${NEW_VMX_FILE}"; sed -i "s/displayName = .*/displayName = \"${NEW_VM_NAME}\"/" "${NEW_VMX_FILE}"; sed -i "s/${ORIG_BASENAME}\.vmdk/${NEW_VM_NAME}\.vmdk/g" "${NEW_VMX_FILE}"; sed -i "s/${ORIG_BASENAME}\.nvram/${NEW_VM_NAME}\.nvram/g" "${NEW_VMX_FILE}"; sed -i '/sched.swap.derivedName/d' "${NEW_VMX_FILE}"; sed -i '/uuid.location/d' "${NEW_VMX_FILE}"; sed -i '/uuid.bios/d' "${NEW_VMX_FILE}"; sed -i '/vc.uuid/d' "${NEW_VMX_FILE}"; echo "uuid.action = \"${UUID_ACTION}\"" >> "${NEW_VMX_FILE}";)
...
EMAIL_SUBJECT='__EMAIL_SUBJECT__'
SENDMAIL_PATH="${GHETTO_PATH}/sendmail"
# KORREKTUR: Standardstatus ist jetzt OK. Wird nur bei echtem Fehler auf ERROR gesetzt.
OVERALL_STATUS="OK"
...
cleanup() {
    EXIT_CODE=$?
    # KORREKTUR: Robusterer df-Befehl
    log_raw "Speicherplatz (Nachher):"
    log_raw "  - Ziel (${TARGET_DATASTORE}): $(df -h "${TARGET_DATASTORE}" 2>/dev/null | grep /vmfs/)"
    log_raw "--- ENDE DES DETAILLOGS ---"
...
$buttonRestore.Add_Click({
    ...
    # Öffnet das Restore/Klon-Fenster, das sich nun die alten Werte merkt
    ...
    if ($restoreResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ($radioRestoreFromBackup.Checked) {
            ...
            # --- MODUS: KLONEN VON LAUFENDER VM (mit neuen Parametern) ---
            $params = @{
                ...
                POWER_ON         = if ($checkboxPowerOnAfterRestore.Checked) { 1 } else { 0 }
                UUID_ACTION      = if ($radioRestoreMoved.Checked) { "keep" } else { "create" }
                # --- NEU: Parameter aus der Haupt-GUI übernehmen ---
        ...
        # KORREKTUR: Dem Watcher-Job den korrekten, neuen Log-Pfad mitteilen
...
$buttonSaveSchedule.Add_Click({
    ...
    $form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    try {
        # --- KORREKTUR: Dies ist die stabile Vorlage für geplante Replikationen ---
# In GhettoGUI_V7.2.0.12.ps1, innerhalb der $buttonSaveSchedule.Add_Click Funktion
# ...
...
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V42.9 (Final Cron Reporting Fix)
# - Re-added full data collection to cleanup() for complete email reports.
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
cleanup() {
    ...
    # --- DATENSAMMLUNG FÜR E-MAIL HINZUGEFÜGT ---
    FINAL_SIZE_ON_TARGET=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}'" 2>/dev/null | awk '{print $1}')
...
echo "${VM_LIST}" | while read SOURCE_VM_NAME; do
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ]; then
        ...
        log "  -> Klone Basis-Disks nach ${TEMP_CLONE_DIR}..."; vmkfstools -i "${VMX_DIR}/$(basename "${VMX_FULL_PATH}" .vmx).vmdk" -d thin "${TEMP_CLONE_DIR}/$(basename "${VMX_FULL_PATH}" .vmx).vmdk" >> ${LOG_FILE} 2>&1
        ...
        log "[5/7] Starte TAR-Stream..."; (cd "${VMX_DIR}" && tar -cf - .) | ssh ${SSH_OPTIONS} root@${TARGET_HOST} "tar -xf - -C '${TARGET_VM_PATH}'"
    fi
    log "[6/7] Passe Zieldateien an..."; ssh ${SSH_OPTIONS} root@${TARGET_HOST} "cd '${TARGET_VM_PATH}'; ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\"); RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"; mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"; sed -i \"s/displayName = .*/displayName = \\\"${REPLICATED_VM_NAME}\\\"/\" \"\${RENAMED_VMX_FILE}\"; sed -i 's/-[0-9]\{6\}\.vmdk/\.vmdk/g' \"\${RENAMED_VMX_FILE}\""
...
# ... (der Rest der Funktion bleibt unverändert) ...
        ...
        if ($radioScheduleReplication.Checked) {
            ...
            # (Diese Logik bleibt unverändert, sie funktioniert bereits korrekt)
...
# Das ist die Skript-Vorlage aus deiner funktionierenden 7.2.0.7-Version
# Manuell gestartete Replikation
	
$multiVmScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V42.7 (Final Robust Space Check)
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
set -e
# Parameter
...
for SOURCE_VM_NAME in ${VM_LIST}; do
    ...
    # NEU: Stabile Speicherplatz-Prüfung auf dem Ziel-Host
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ]; then
        ...
        DISK_DEFINITIONS=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)');
        echo "${DISK_DEFINITIONS}" | while read -r line; do ORIGINAL_DISK_BASENAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p' | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/'); if [ -z "${ORIGINAL_DISK_BASENAME}" ]; then continue; fi; log "  -> Klone Basis-Disk: ${ORIGINAL_DISK_BASENAME}"; /sbin/vmkfstools -i "${VMX_DIR}/${ORIGINAL_DISK_BASENAME}" -d thin "${TEMP_CLONE_DIR}/${ORIGINAL_DISK_BASENAME}" > /dev/null 2>&1 & VMKF_PID=$!; while kill -0 ${VMKF_PID} >/dev/null 2>&1; do CURRENT_KLON_SIZE=$(du -sh "${TEMP_CLONE_DIR}" | awk '{print $1}'); log "Temp-Klon: ${SOURCE_VM_NAME} -> ${CURRENT_KLON_SIZE} von ~${SOURCE_SIZE_KLON}"; sleep 15; done; wait ${VMKF_PID}; if [ $? -ne 0 ]; then log_error "vmkfstools-Klon von ${ORIGINAL_DISK_BASENAME} fehlgeschlagen!"; exit 1; fi; done
        (cd "${VMX_DIR}" && find . -maxdepth 1 ! -name '*.vmdk' -exec cp -p '{}' "${TEMP_CLONE_DIR}/" \;) 
    ...
    ssh ${SSH_OPTIONS} root@${TARGET_HOST} "cd '${TARGET_VM_PATH}'; ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\"); RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"; mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"; sed -i \"s/displayName = .*/displayName = \\\"${REPLICATED_VM_NAME}\\\"/\" \"\${RENAMED_VMX_FILE}\"; sed -i 's/-[0-9]\{6\}\.vmdk/\.vmdk/g' \"\${RENAMED_VMX_FILE}\""
    if [ $? -ne 0 ]; then
    ...
    # NEU: Verzeichnisliste für diese VM nach Erfolg zum Report hinzufügen
    if LISTING_CMD_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lR '${TARGET_VM_PATH}'" 2>/dev/null); then
...
FINAL_SIZE_ON_TARGET=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}'" | awk '{print $1}')
log_raw "Final size: ${FINAL_SIZE_ON_TARGET}"
log_raw "Speicherplatz (Nachher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "df -h '${TARGET_DATASTORE}' 2>/dev/null | tail -n 1")"
if LISTING_CMD_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lR '${TARGET_VM_PATH}'" 2>/dev/null); then DIRECTORY_LISTING_CONTENT="${LISTING_CMD_OUTPUT}"; fi
...
function Save-HostGuiSettings {
    ...
    $settings = @{
        ...
        # --- NEU: Parameter für Restore/Klon hinzufügen ---
...
function Load-HostGuiSettings {
    ...
    try {
        ...
        # --- Bestehende Einstellungen laden (funktioniert bereits) ---
        ...
        if ($settings.PSObject.Properties.Name -contains 'ReplMethod') { if ($settings.ReplMethod -eq 'vmkfstools') { $directReplicationForm.Controls.Find('radioVmkf', $true)[0].Checked = $true } else { $directReplicationForm.Controls.Find('radioTar', $true)[0].Checked = $true } }

        # --- KORREKTUR: Lade Restore/Klon-Einstellungen ---
        if ($settings.PSObject.Properties.Name -contains 'RestoreIsRestoreMode') {
            if ($settings.RestoreIsRestoreMode) { $radioRestoreFromBackup.Checked = $true }
            else { $radioCloneFromVm.Checked = $true }
        }
        ...
        if ($settings.PSObject.Properties.Name -contains 'RestoreUuidActionMoved') {
            if ($settings.RestoreUuidActionMoved) { $radioRestoreMoved.Checked = $true }
            else { $radioRestoreCopied.Checked = $true }
        }
