# GhettoGUI_V8.2.0.6.ps1
#
# --- VERSION V8.2.0.6 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V8.2.0.6 / 24.12.2025"
...
$groupEmail = New-Object System.Windows.Forms.GroupBox; $groupEmail.Text = "E-Mail Benachrichtigung"; $groupEmail.Location = New-Object System.Drawing.Point($column2X, $currentY_Col2); $groupEmail.Size = New-Object System.Drawing.Size($column2Width, 275)
...
function Show-DatastoreFileSelectionDialog {
    ...
    # In GhettoGUI_V8.0.1.36.ps1, innerhalb der Funktion Show-DirectorySelectionDialog

    # Hilfsfunktion zum Aktualisieren der ListBox
    $updateListBox = {
        param([string]$currentPath)
        $selectionDialog.Text = "Navigiere: $currentPath" # Update Titelzeile
        $listBox.Items.Clear()
        $selectionDialog.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
        try {
            # KORREKTUR: Verwende 'find' für eine robustere Verzeichnisliste
            # -maxdepth 1: Nur im aktuellen Verzeichnis suchen
            # -mindepth 1: Das aktuelle Verzeichnis selbst ausschliessen
            # -type d: Nur nach Verzeichnissen suchen
            # -printf '%f\n': Nur den Basisnamen des Verzeichnisses ausgeben, gefolgt von einem Zeilenumbruch
            $command = "find '$currentPath' -maxdepth 1 -mindepth 1 -type d -printf '%f\n'"
            ...
            if ($result.ExitStatus -eq 0 -and $result.Output) {
                # 'find' gibt direkt die Namen aus, nur noch leere Zeilen filtern und sortieren
                $dirs = $result.Output | Where-Object { $_ -match '\S' } | Sort-Object
                $listBox.Items.AddRange($dirs)
            }
            # Optional: Detailliertere Fehlermeldung loggen
            # elseif ($result.Error) {
            #    $logMessage = "Fehler beim Lesen von '{0}' (find): {1}" -f $currentPath, ($result.Error -join '; ')
            #    Write-GuiLog $logMessage
            # }

        } catch {
            $errorMessage = $_.Exception.Message
            $logMessage = "Ausnahmefehler beim Lesen von '{0}' (find): {1}" -f $currentPath, $errorMessage
            Write-GuiLog $logMessage
        } finally {
...
$buttonSaveSchedule.Add_Click({
    ...
    try {
        ...
        # --- NEU: Display Name abrufen ---
        $dn = $textboxEmailDisplay.Text
        if ([string]::IsNullOrWhiteSpace($dn)) { $dn = "ESXi Backup" }
        # ---------------------------------
        # -- Debug [System.Windows.Forms.MessageBox]::Show("Debug Info: Email Absender: '$dn'", "Debug", "OK", "Information")
...
# In GhettoGUI_V8.1.9.3.ps1

$masterHelperScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V43.9.5 (Fix: Cold Copy via Power State)
# - FIX: Implementiert die Logik zur Prüfung des Power State (on/off), um Cold Copy 
#        bei ausgeschalteten VMs anstelle des Hot-Klon-Pfades zu verwenden.
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
SSH_OPTIONS="-T -i /.ssh/id_ecdsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=20 -o Compression=no"; SNAP_MEM=__SNAP_MEM__; SNAP_QUIESCE=__SNAP_QUIESCE__; EMAIL_ENABLED=__EMAIL_ENABLED__; EMAIL_TO='__EMAIL_TO__'; EMAIL_FROM='__EMAIL_FROM__'; EMAIL_DISPLAY_NAME='__EMAIL_DISPLAY_NAME__'; EMAIL_SERVER='__EMAIL_SERVER__'; EMAIL_PORT='__EMAIL_PORT__'; EMAIL_USER='__EMAIL_USER__'; EMAIL_PASS='__EMAIL_PASS__'; EMAIL_SUBJECT='__EMAIL_SUBJECT__'; SENDMAIL_PATH="${GHETTO_PATH}/sendmail"; VM_LIST='
__VM_LIST__
'; OVERALL_STATUS="OK";
...
# --- Logging & E-Mail Funktionen (UNVERÄNDERT) ---
log() { echo "$(date '+%Y-%m-%d %H:%M:%S') -> $1" >> ${LOG_FILE}; }
log_raw() { echo "$1" >> ${LOG_FILE}; }
log_error() { echo "$(date '+%Y-%m-%d %H:%M:%S') -- ERROR: $1" >> ${LOG_FILE}; }
send_email_notification() {
...
rm -f ${LOG_FILE}; mkdir -p "${GHETTO_PATH}/logs"; START_TIME=$(date +%s)
trap cleanup EXIT
log "====== MASTERHELPER GEPLANTE REPLICATION HELPER GESTARTET (ID: ${UNIQUE_ID}) ======"; log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')";
log_raw "Job-Konfiguration:"
...
TARGET_HOSTNAME=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "hostname" 2>/dev/null || echo "Unknown")
TARGET_DS_NAME=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "esxcli storage filesystem list | grep '${TARGET_DATASTORE}' | awk '{print \$2}'" 2>/dev/null || echo "${TARGET_DATASTORE}")

log_raw "====================================================================="
log_raw "GhettoGUI Multi-VM Replication Helper (Geplant / Cron)"
log_raw "UNIQUE_ID: ${UNIQUE_ID}"
log_raw "====================================================================="
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Geplante Replikation (H2H)"
log_raw "  - Methode: ${REPLICATION_METHOD}"
log_raw "  - Quell-Host: ${SRC_IP} (${SRC_HOSTNAME})"
log_raw "  - Ziel-Host: ${TARGET_HOST} (${TARGET_HOSTNAME})"
log_raw "  - Ziel-Datastore: ${TARGET_DS_NAME}"
log_raw "  - VM-Suffix: ${VM_SUFFIX}"
log_raw "Speicherplatz (Vorher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "df -h '${TARGET_DATASTORE}' 2>/dev/null | tail -n 1" < /dev/null)"; log_raw "--- START DES DETAILLOGS ---"



CLONE_ERROR_FLAG="/tmp/clone_error_manual_${UNIQUE_ID}.flag" # Fehler-Flag für den Hot-Klon
rm -f $CLONE_ERROR_FLAG

CLEAN_VM_LIST=$(echo "${VM_LIST}" | sed '/^$/d')
# In GhettoGUI_V8.1.8.4.ps1
# ERSETZE den gesamten for-Loop von 'for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do' bis zum 'done' am Ende.
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    # NEU: Power State abfragen
    VM_POWER_STATE=$(vim-cmd vmsvc/power.getstate ${VMID} | grep -i "Powered" | awk '{print $NF}')
    [cite_start]log "INFO: VM Power State: ${VM_POWER_STATE}" [cite: 41, 88]
    
    # 1/7 - 2/7: Bereinigung & Zielordner erstellen (unabhängig vom Power State)
    ...
    # --- START: BRANCHING FÜR HOT-KLON ODER COLD COPY ---
    
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        
        # =================================================================
        # HOT-KLON PATH (VM läuft) - Zwingt zur Basis-VMDK
        # =================================================================
        ...
        DS_PATH_FOR_DF=$(echo "${TEMP_CLONE_CHECK_PATH}" | awk '{sub(/\/vmfs\/volumes\//, "/vmfs/volumes/"); print}')
        SOURCE_FREE_GB=$(esxcli storage filesystem list | grep -i "${DS_PATH_FOR_DF}" | awk '{ printf "%.0f", $NF/1024/1024/1024 }')
        ...
        log "[4/7] Erstelle Snapshot..."; vim-cmd vmsvc/snapshot.create ${VMID} "ghetto-repl-${UNIQUE_ID}" "GhettoGUI Replication" ${SNAP_MEM} ${SNAP_QUIESCE}; log "-> Warte 15s..."; sleep 15
        
        # FIX B: Wartezeit für Lock-Freigabe
        [cite_start]log "WARN: Warte zusätzliche 30s auf Freigabe des Snapshot-Locks, um Fehler 16392 zu vermeiden..." [cite: 82]
        sleep 30
        ...
        SOURCE_SIZE_KLON=$(du -sh "${VMX_DIR}" | awk '{print $1}') # Für Log-Anzeige
        ...
        echo "${DISK_DEFINITIONS}" | while read -r line; do 
            ...
            # 1. Quelle: Zwingend die Basis-VMDK (entferne alle möglichen Suffixe)
            VMX_DISK_PATH_BASE=$(echo "${VMX_DISK_PATH}" | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/');

            if echo "${VMX_DISK_PATH_BASE}" | grep -q "^/"; then
                SOURCE_DISK_PATH_FOR_CLONE="${VMX_DISK_PATH_BASE}"
            else
                SOURCE_DISK_PATH_FOR_CLONE="${VMX_DIR}/${VMX_DISK_PATH_BASE}"
            fi
            
            # 2. Ziel: Der bereinigte Basisname (z.B. 'vm.vmdk')
            DESTINATION_DISK_BASENAME=$(basename "${VMX_DISK_PATH_BASE}")
            DESTINATION_DISK_PATH="${TEMP_CLONE_DIR}/${DESTINATION_DISK_BASENAME}"
            
            
            log "  -> Klone Basis-Disk (Chain): ${SOURCE_DISK_PATH_FOR_CLONE}";

            VMKFSTOOLS_LOG="/tmp/vmkfstools_out_${UNIQUE_ID}.log"
            # vmkfstools -i <BASE_VMDK> klont den aktiven Zustand (die Kette)
            /sbin/vmkfstools -i "${SOURCE_DISK_PATH_FOR_CLONE}" -d thin "${DESTINATION_DISK_PATH}" > "${VMKFSTOOLS_LOG}" 2>&1 &
            VMKF_PID=$!; 
            while kill -0 ${VMKF_PID} >/dev/null 2>&1; do 
                CURRENT_KLON_SIZE=$(du -sh "${TEMP_CLONE_DIR}" 2>/dev/null | awk '{print $1}'); 
            ...
            if [ ${CLONE_EXIT_CODE} -ne 0 ]; then
                log_error "Klonen von ${DESTINATION_DISK_BASENAME} fehlgeschlagen! Exit-Code: ${CLONE_EXIT_CODE}"
                CLONE_ERROR=1
                break 
            fi
            rm -f "${VMKFSTOOLS_LOG}"
        ...
        if [ -f "$CLONE_ERROR_FLAG" ] || [ ${CLONE_ERROR} -ne 0 ]; then
             log_error "Überspringe Rest für ${SOURCE_VM_NAME} wegen Klon-Fehler."
             rm -f $CLONE_ERROR_FLAG
             OVERALL_STATUS="ERROR"
             /bin/vim-cmd vmsvc/snapshot.removeall ${VMID} # Snapshot entfernen
             continue
        fi

        (cd "${VMX_DIR}" && find . -maxdepth 1 ! -name '*.vmdk' -exec cp -p '{}' "${TEMP_CLONE_DIR}/" \;) 
        log "-> Lokaler Klon abgeschlossen. Lösche Snapshot JETZT, um VM zu entlasten..."; /bin/vim-cmd vmsvc/snapshot.removeall ${VMID}
        ...
        while kill -0 ${TAR_PID} >/dev/null 2>&1; do
            ...
            if [ -n "${PROGRESS}" ]; then log "->Transfer ${SOURCE_VM_NAME}: ${PROGRESS} von ~${SOURCE_SIZE_TAR}"; fi
            sleep 30
        done
        wait ${TAR_PID};
        TRANSFER_EXIT_CODE=$?
        ...
        # =================================================================
        # COLD COPY PATH (VM ist ausgeschaltet)
        # =================================================================
        [cite_start]log "[3/7] Führe COLD-COPY ohne Snapshot/Klon aus (VM ist ausgeschaltet)..." [cite: 64, 88]
        
        DISK_DEFINITIONS=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)');
        
        CLONE_ERROR=0
        SOURCE_SIZE_KLON=$(du -sh "${VMX_DIR}" 2>/dev/null | awk '{print $1}')
        if [ -z "${SOURCE_SIZE_KLON}" ]; then SOURCE_SIZE_KLON="N/A"; fi

        echo "${DISK_DEFINITIONS}" | while read -r line; do
            VMX_DISK_PATH=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p');
            if [ -z "${VMX_DISK_PATH}" ]; then continue; fi;
            
            # Quellpfad ist die originale VMX-Disk
            if echo "${VMX_DISK_PATH}" | grep -q "^/"; then
                SOURCE_DISK_PATH_FOR_COPY="${VMX_DISK_PATH}"
            else
                SOURCE_DISK_PATH_FOR_COPY="${VMX_DIR}/${VMX_DISK_PATH}"
            fi
            
            # Zielpfad mit dem BEREINIGTEN Namen – nur Dateiname ohne Pfad
            TARGET_DISK_BASENAME=$(basename "$(echo "${VMX_DISK_PATH}" | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/')")
            TARGET_DISK_PATH="${TARGET_VM_PATH}/${TARGET_DISK_BASENAME}";

            
            VMKFSTOOLS_LOG="/tmp/vmkfstools_out_cold_${UNIQUE_ID}.log"
            [cite_start]log "  -> Kopiere Festplatte: ${TARGET_DISK_BASENAME} (Format: thin) [Grösse: ~${SOURCE_SIZE_KLON}]"; [cite: 64, 88]
            
            /sbin/vmkfstools -i "${SOURCE_DISK_PATH_FOR_COPY}" -d thin "${TARGET_DISK_PATH}" > "${VMKFSTOOLS_LOG}" 2>&1 &
            CLONE_PID=$!
            
            # Warteschleife zur Fortschrittsüberwachung
            while /bin/kill -0 ${CLONE_PID} 2>/dev/null; do
                CURRENT_TOTAL_SIZE=$(/usr/bin/du -sh "${TARGET_VM_PATH}" 2>/dev/null | /usr/bin/awk '{print $1}')
                log " -> Cold Copy Fortschritt: ${CURRENT_TOTAL_SIZE} von ~${SOURCE_SIZE_KLON}"
                /bin/sleep 30
            done
            
            /usr/bin/wait ${CLONE_PID}
            CLONE_EXIT_CODE=$?
            
            [cite_start]log " -> Kopieren von ${TARGET_DISK_BASENAME} abgeschlossen. (Exit Code: ${CLONE_EXIT_CODE})" [cite: 65, 89]
            cat "${VMKFSTOOLS_LOG}" >> "${LOG_FILE}"
            /bin/rm -f "${VMKFSTOOLS_LOG}"
            
            # Akzeptiere Exit Code 0 oder 127 als erfolgreichen Abschluss für Cold Copy
            if [ ${CLONE_EXIT_CODE} -ne 0 ] && [ ${CLONE_EXIT_CODE} -ne 127 ]; then
                log_error "Cold Copy von ${VMX_DISK_PATH} fehlgeschlagen! Exit-Code: ${CLONE_EXIT_CODE}"
                CLONE_ERROR=1
                break 
            fi
        done
        ...
        if [ ${CLONE_ERROR} -ne 0 ]; then
            log_error "Fehler beim Cold Copy der Festplatten festgestellt. Breche Verarbeitung für VM '${SOURCE_VM_NAME}' ab."
            OVERALL_STATUS="ERROR"
            continue
        fi

        [cite_start]log "[5/7] Kopiere Konfigurationsdateien (VMX, NVRAM, VMSD)..."; [cite: 88]
        VMX_FILE_NAME=$(/usr/bin/basename "${VMX_FULL_PATH}")
        VM_NVRAM_FILE=$(/bin/grep -i 'nvram' "${VMX_FULL_PATH}" | /usr/bin/awk -F '"' '{print $2}')
        VM_VMSD_FILE=$(/bin/grep -i 'vmsd' "${VMX_FULL_PATH}" | /usr/bin/awk -F '"' '{print $2}')
        
        /bin/cp -p "${VMX_FULL_PATH}" "${TARGET_VM_PATH}/${VMX_FILE_NAME}"
        if [ -n "${VM_NVRAM_FILE}" ]; then /bin/cp -p "${VMX_DIR}/${VM_NVRAM_FILE}" "${TARGET_VM_PATH}/${VM_NVRAM_FILE}"; fi
        if [ -n "${VM_VMSD_FILE}" ]; then /bin/cp -p "${VMX_DIR}/${VM_VMSD_FILE}" "${TARGET_VM_PATH}/${VM_VMSD_FILE}"; fi
        
    elif [ "${REPLICATION_METHOD}" = "stream" ]; then

        # Alte temporäre Klon-Verzeichnisse im Quell-VM-Ordner entfernen,
        # damit sie nicht mit in den TAR-Stream kopiert werden.
        find "${VMX_DIR}" -maxdepth 1 -type d -name 'ghetto_clone_*' -exec rm -rf {} \; 2>/dev/null || true
        
        # =================================================================
        # STREAM PATH (Offline)
        # =================================================================
        log "[3/7] Starte OFFLINE Replikation (TAR-Stream)...";
        
    else
        log_error "FEHLER: Unbekannte Replikationsmethode oder Power State: ${REPLICATION_METHOD} / ${VM_POWER_STATE}"
        OVERALL_STATUS="ERROR"
        continue
    fi
    ...
    ssh ${SSH_OPTIONS} root@${TARGET_HOST} "cd '${TARGET_VM_PATH}' || exit 1; \
      ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\" ! -name \"*~\" | head -n 1); \
      RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"; \
      mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"; \
      \
      # 1) Nur unkritische Metadaten entfernen (UUIDs/GenIDs bleiben drin!)
      /bin/grep -v -E '(^ethernet[0-9]*\\.(generatedAddress|address|addressType)|^sched\\.swap\\.derivedName|^extendedConfigFile|^vmxstats\\.filename|^migrate\\.hostLog|^numa\\.autosize|^tools\\.capability)' \"\${RENAMED_VMX_FILE}\" > \"\${RENAMED_VMX_FILE}.tmp\"; \
      /bin/mv \"\${RENAMED_VMX_FILE}.tmp\" \"\${RENAMED_VMX_FILE}\"; \
      \
      # 2) Sed-Korrekturen (Name, Pfade, Snapshots, independent-Disks)
      /bin/sed -i \
        -e \"s/displayName = .*/displayName = \\\"${REPLICATED_VM_NAME}\\\"/\" \
        -e \"s/-[0-9]\\{6\\}\\.vmdk/\\.vmdk/g\" \
        -e \"s|\\(fileName = \\\"\\)/.*/\\(.*\\.vmdk\\\"\\)|\\1\\2|g\" \
        -e \"/\\.mode = \\\"independent/d\" \
        \"\${RENAMED_VMX_FILE}\"; \
      \
      # 3) uuid.action sicher auf 'prompt' setzen
      /bin/sed -i '/^uuid\\.action\\s*=/{d}' \"\${RENAMED_VMX_FILE}\"; \
      /bin/echo 'uuid.action = \"prompt\"' >> \"\${RENAMED_VMX_FILE}\"; \
      \
      # 4) NVRAM löschen (Cache leeren)
      /bin/rm -f ./*.nvram; \
      \
      echo 'INFO: VMX-Patch abgeschlossen.' \
    "
    ...
    log "[7/7] Registriere VM..."; [cite: 65, 89]
    REGISTER_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "vim-cmd solo/registervm '${TARGET_VM_PATH}/${REPLICATED_VM_NAME}.vmx'")
    if [ $? -ne 0 ]; then
        log_error "[${SOURCE_VM_NAME}] - Fehler beim Registrieren der VM auf dem Ziel-Host!"
        OVERALL_STATUS="ERROR"
        continue
    fi
    log "  -> VM erfolgreich registriert." [cite: 81, 85]
    
    # Aufräumen und Loggen der Größe
    if [ "${VM_POWER_STATE}" = "on" ] && [ "${REPLICATION_METHOD}" = "robust" ]; then
        log "-> Hot-Replikation abgeschlossen." [cite: 85, 109]
    elif [ "${VM_POWER_STATE}" = "off" ]; then
        log "-> Cold-Replikation abgeschlossen." [cite: 65, 81]
    fi
...
# NEU: Fülle das Directory Listing für das E-Mail-Log
if [ -n "${TARGET_VM_PATH}" ]; then
    log "Hole Verzeichnis-Listing vom Ziel-Host für ${REPLICATED_VM_NAME}..."
    DIRECTORY_LISTING_CONTENT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "echo 'Inhalt von: ${TARGET_VM_PATH}'; /bin/ls -lh '${TARGET_VM_PATH}' 2>/dev/null")
fi
...
# GhettoGUI Multi-VM Replication Helper V42.9.16 (Merged & Fixed by User & AI)
# - FEATURE: Kombiniert Hot-Clone (robust) und Cold-Copy (stream) basierend auf Powerstate.
# - FIX: Korrigierte Fehlererkennung in der While-Schleife (Subshell-Problem).

$multiVmScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V42.9.24 (Step 1: No Shutdown Safety)
# - BASE: Basiert auf deiner Version Multivmscript_8193.ps1.
# - SAFETY: Entfernt Shutdown-Logik. Laufende VMs im Offline-Modus werden übersprungen.
# - FIX: SSH-Befehle gehärtet (stdin umgeleitet), um Hänger zu vermeiden.
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
set -e
...
send_email_notification() {
    ...
    if [ "${EMAIL_ENABLED}" = "1" ] && [ -f "${SENDMAIL_PATH}" ]; then
        if [ -z "${EMAIL_SERVER}" ] || [ -z "${EMAIL_PORT}" ] || [ -z "${EMAIL_TO}" ]; then return; fi
        RECIPIENTS=$(echo "${EMAIL_TO}" | sed 's/,/ /g')
        /bin/python ${SENDMAIL_PATH} -f "${EMAIL_FROM}" -N "${EMAIL_DISPLAY_NAME}" -s "${EMAIL_SERVER}" -S "${EMAIL_PORT}" -j "${EMAIL_SUBJECT} - ${FINAL_STATUS_MSG}" -m "${LOG_FILE}" -u "${EMAIL_USER}" -p "${EMAIL_PASS}" ${RECIPIENTS} >> ${LOG_FILE} 2>&1
    fi
}
...
log_raw() { echo "$*" | tee -a "${LOG_FILE}"; }

log_raw "====================================================================="
log_raw "GhettoGUI Masterhelper Replication Helper (Manuell)"
log_raw "UNIQUE_ID: ${UNIQUE_ID}"
log_raw "====================================================================="
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manuelle Replikation (H2H)"
log_raw "  - Methode: ${REPLICATION_METHOD}"
log_raw "  - Quell-Host: ${SRC_IP} (${SRC_HOSTNAME})"
log_raw "  - Ziel-Host: ${TARGET_HOST} (${TARGET_HOSTNAME})"
log_raw "  - Ziel-Datastore: ${TARGET_DS_NAME}"
...
for SOURCE_VM_NAME in ${VM_LIST}; do
    ...
    VM_INFO_LINE=$(/bin/vim-cmd vmsvc/getallvms | awk -v name="${SOURCE_VM_NAME}" 'NR>1 { n=split($0, a, "\\["); vm=a[1]; sub(/^[0-9]+[ \t]+/, "", vm); sub(/[ \t]+$/, "", vm); if (vm == name) print $0; }');
    if [ -z "${VM_INFO_LINE}" ]; then log_error "[${SOURCE_VM_NAME}] - VM nicht gefunden!"; OVERALL_STATUS="ERROR"; continue; fi
    VMID=$(echo "${VM_INFO_LINE}" | awk '{print $1}'); VMX_FULL_PATH=$(echo "${VM_INFO_LINE}" | sed -e 's/.*\[\(.*\)\]\s*\(.*\.vmx\).*/\/vmfs\/volumes\/\1\/\2/'); VMX_DIR=$(dirname "${VMX_FULL_PATH}");

    # NEU: Prüfe den Power State (aus Multivmscript_Lokal_O.K übernommen)
    POWER_STATE=$(/bin/vim-cmd vmsvc/power.getstate ${VMID} | grep -i "Powered" | awk '{print $NF}')
    log "  -> Aktueller Power State: ${POWER_STATE}"

    # Weiche: für ausgeschaltete VMs robuste Replikation als OFFLINE-Stream behandeln
    EFFECTIVE_REPL_METHOD="${REPLICATION_METHOD}"
    if [ "${POWER_STATE}" = "off" ] && [ "${REPLICATION_METHOD}" = "robust" ]; then
        log "  -> VM ist ausgeschaltet, verwende OFFLINE-Replikation anstelle von Temp-Klon."
        EFFECTIVE_REPL_METHOD="stream_offline"
    fi

    # Bereinigung auf Ziel (mit -n Option gegen Hänger)
    log "[1/7] Bereinige alte Replikation...";
    TARGET_VMID=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd vmsvc/getallvms | awk -v name=\"${REPLICATED_VM_NAME}\" 'NR>1 { vm=\$0; match(\$0, /^[0-9]+[ \t]+(.*)[ \t]+\[[^]]+\][ \t]+.*/, arr); if (arr[1] != \"\") { vm=arr[1]; sub(/^[ \t]+/, \"\", vm); sub(/[ \t]+$/, \"\", vm); } if (vm == name) print \$1; }'" || true)
    if [ -n "${TARGET_VMID}" ]; then ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd vmsvc/power.off ${TARGET_VMID} >/dev/null 2>&1; sleep 5; vim-cmd vmsvc/unregister ${TARGET_VMID} >/dev/null 2>&1" || true; fi

    log "[2/7] Erstelle Zielverzeichnis...";
    ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "rm -rf '${TARGET_VM_PATH}'; mkdir -p '${TARGET_VM_PATH}'"

    if [ "${EFFECTIVE_REPL_METHOD}" = "robust" ]; then
        if [ -z "${TEMP_CLONE_BASE_PATH}" ]; then TEMP_CLONE_CHECK_PATH="${VMX_DIR}"; else TEMP_CLONE_CHECK_PATH="${TEMP_CLONE_BASE_PATH}"; fi
        log "[0/7] Prüfe Speicherplatz für Temp-Klon auf Quell-Host..."
        SOURCE_SIZE_K=$(du -sk "${VMX_DIR}" | awk '{print $1}')
        REQUIRED_GB=$(echo "${SOURCE_SIZE_K}" | awk '{ required_k = $1 * 1.1; printf "%.0f", required_k/1024/1024 }')
        SOURCE_FREE_GB=$(esxcli storage filesystem list | grep "${TEMP_CLONE_CHECK_PATH}" | awk '{ printf "%.0f", $NF/1024/1024/1024 }')

        if [ "${SOURCE_FREE_GB}" -lt "${REQUIRED_GB}" ]; then
           log_error "Nicht genügend Speicherplatz! Benötigt: ${REQUIRED_GB}GB, Frei: ${SOURCE_FREE_GB}GB"
    ...
    # Prüfen, ob doppelte VMDK-Basenamen existieren (z.B. gleiche .vmdk auf verschiedenen Datastores)
    DUPLICATE_BASENAMES=0
    DISK_BASENAMES=$(echo "${DISK_DEFINITIONS}" \
        | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p' \
        | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/' \
        | sed 's#.*/##')
    if [ -n "${DISK_BASENAMES}" ]; then
        for bn in ${DISK_BASENAMES}; do
            count=$(echo "${DISK_BASENAMES}" | grep -F -x "${bn}" | wc -l)
            if [ "${count}" -gt 1 ]; then
    ...
    echo "${DISK_DEFINITIONS}" | while read -r line; do
            ...
            if echo "${ORIGINAL_DISK_BASENAME}" | grep -q "^/"; then SOURCE_DISK_PATH_FOR_CLONE="${ORIGINAL_DISK_BASENAME}"; else SOURCE_DISK_PATH_FOR_CLONE="${VMX_DIR}/${ORIGINAL_DISK_BASENAME}"; fi
                    # Ziel-Dateiname: bei doppelten Basenamen nach SCSI-Unit nummerieren, sonst wie bisher
        if [ "${DUPLICATE_BASENAMES}" -eq 1 ]; then
            # SCSI-Key aus der VMX-Zeile, z.B. 'scsi0:1.fileName'
            SCSI_KEY=$(echo "$line" | awk '{print $1}')
            # Unit-Nummer extrahieren (hinter dem ':')
            SCSI_UNIT=$(echo "${SCSI_KEY}" | awk -F':' '{print $2}' | sed 's/[^0-9]//g')
            [ -z "${SCSI_UNIT}" ] && SCSI_UNIT=0
            SCSI_SUFFIX=$(printf "%02d" "${SCSI_UNIT}")

            BASE_NAME=$(basename "${ORIGINAL_DISK_BASENAME}")   # z.B. name.vmdk
            BASE_NOEXT="${BASE_NAME%.vmdk}"                     # → name
            DESTINATION_DISK_BASENAME="${BASE_NOEXT}_${SCSI_SUFFIX}.vmdk"
        else
            ...
            wait ${VMKF_PID}
            CLONE_EXIT_CODE=$?
            cat "${VMKFSTOOLS_LOG}" >> "${LOG_FILE}"; rm -f "${VMKFSTOOLS_LOG}"

            if [ ${CLONE_EXIT_CODE} -ne 0 ]; then
                log_error "Klonen fehlgeschlagen!"; CLONE_ERROR=1; break
            fi
        done

        if [ ${CLONE_ERROR} -ne 0 ]; then
            log_error "Fehler beim Klonen. Abbruch."; OVERALL_STATUS="ERROR"
            /bin/vim-cmd vmsvc/snapshot.removeall ${VMID}; rm -rf "${TEMP_CLONE_DIR}"; continue
        fi

        (cd "${VMX_DIR}" && find . -maxdepth 1 ! -name '*.vmdk' -exec cp -p '{}' "${TEMP_CLONE_DIR}/" \;)
        log "-> Lokaler Klon abgeschlossen. Lösche Snapshot JETZT im Hintergrund..."; /bin/vim-cmd vmsvc/snapshot.removeall ${VMID}
        ...
        log "  -> Transferprozess gestartet mit PID: ${TAR_PID}."
        while kill -0 ${TAR_PID} >/dev/null 2>&1; do
            PROGRESS=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}' 2>/dev/null | awk '{print \$1}'")
            if [ -n "${PROGRESS}" ]; then log "->Transfer ${SOURCE_VM_NAME}: ${PROGRESS} von ~${SOURCE_SIZE_TAR}"; fi
            sleep 30
        done
        wait ${TAR_PID}
        if [ $? -ne 0 ]; then
            log_error "TAR-Transfer fehlgeschlagen!"; OVERALL_STATUS="ERROR"; rm -rf "${TEMP_CLONE_DIR}"; continue
        fi
        rm -rf "${TEMP_CLONE_DIR}"

    # --- OFFLINE (Stream) Mode - SICHERHEITSANGEPASST ---
    else 
        log "[3/7] Starte OFFLINE Replikation..."
        
        # SICHERHEITSCHECK: Läuft die VM?
        if vim-cmd vmsvc/power.getstate ${VMID} | grep -q "Powered on"; then
            log_error "FEHLER: VM '${SOURCE_VM_NAME}' läuft! 'Stream/Offline'-Methode erfordert ausgeschaltete VM. Abbruch zum Schutz der Daten."
            OVERALL_STATUS="ERROR"
            continue
        fi
        
        # HIER WURDE DER SHUTDOWN BEFEHL ENTFERNT

        # Alte temporäre Klon-Verzeichnisse im Quell-VM-Ordner entfernen,
        # damit sie nicht mit in den TAR-Stream kopiert werden.
        find "${VMX_DIR}" -maxdepth 1 -type d -name 'ghetto_clone_*' -exec rm -rf {} \; 2>/dev/null || true

        log "[5/9] Starte TAR-Stream (Cold Copy)..."
        SOURCE_SIZE=$(du -sh "${VMX_DIR}" | awk '{print $1}')
        ( (cd "${VMX_DIR}" && tar -cf - .) | ssh ${SSH_OPTIONS} root@${TARGET_HOST} "tar -xf - -C '${TARGET_VM_PATH}'" ) &
        TAR_PID=$!
		
        while kill -0 ${TAR_PID} >/dev/null 2>&1; do 
            PROGRESS=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}' 2>/dev/null | awk '{print \$1}'")
            if [ -n "${PROGRESS}" ]; then log "->Transfer (OFFLINE) ${SOURCE_VM_NAME}: ${PROGRESS} von ~${SOURCE_SIZE}"; fi
            sleep 30
        done
        wait ${TAR_PID}
        if [ $? -ne 0 ]; then log_error "TAR-Transfer fehlgeschlagen!"; OVERALL_STATUS="ERROR"; continue; fi
    fi

        log "[6/7] Passe Zieldateien an..."
        ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "cd '${TARGET_VM_PATH}' || exit 1; \
          ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\" ! -name \"*~\" | head -n 1); \
          RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"; \
          mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"; \
          /bin/grep -v -E '(^ethernet[0-9]*\\.(generatedAddress|address|addressType)|^sched\\.swap\\.derivedName|^extendedConfigFile|^vmxstats\\.filename|^migrate\\.hostLog|^numa\\.autosize|^tools\\.capability)' \"\${RENAMED_VMX_FILE}\" > \"\${RENAMED_VMX_FILE}.tmp\"; \
    ...
    log "[7/7] Registriere VM..."
    REGISTER_OUTPUT=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd solo/registervm '${TARGET_VM_PATH}/${REPLICATED_VM_NAME}.vmx'")
    if [ $? -ne 0 ]; then
        log_error "[${SOURCE_VM_NAME}] - Fehler beim Registrieren!"
        OVERALL_STATUS="ERROR"
        continue
    fi
    ...
    # HIER WURDE DER RESTART BEFEHL ENTFERNT
    
    log "-> Verarbeitung für ${SOURCE_VM_NAME} abgeschlossen."
    
    if LISTING_CMD_OUTPUT=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lR '${TARGET_VM_PATH}'" 2>/dev/null); then
        DIRECTORY_LISTING_CONTENT="${DIRECTORY_LISTING_CONTENT}\n\n--- Verzeichnis für ${REPLICATED_VM_NAME} ---\n${LISTING_CMD_OUTPUT}"
    fi
    
    VM_SIZE=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}'" < /dev/null | awk '{print $1}')
    if [ -n "${VM_SIZE}" ]; then
        NEW_LINE_WITH_NEWLINE=$(printf -- "- %s: %s\n" "${REPLICATED_VM_NAME}" "${VM_SIZE}")
        VM_REPORT_LIST="${VM_REPORT_LIST}${NEW_LINE_WITH_NEWLINE}"
    fi
...
FINAL_SIZE_ON_TARGET=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "du -sch '${TARGET_DATASTORE}'/*${VM_SUFFIX} 2>/dev/null | grep total" | awk '{print $1}')
log_raw "Final size: ${FINAL_SIZE_ON_TARGET}"
log_raw "Speicherplatz (Nachher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "df -h '${TARGET_DATASTORE}' 2>/dev/null | tail -n 1")"
if [ "${OVERALL_STATUS}" = "OK" ]; then
    if LISTING_CMD_OUTPUT=$(ssh -n ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lR '${TARGET_DATASTORE}'/*${VM_SUFFIX}" 2>/dev/null); then 
...
# =====================================================================================
# --- START BLOCK (Lade-Funktion V7.3 - Mit Display Name) ---
# =====================================================================================
...
function Load-HostGuiSettings {
    ...
    try {
        ...
        # --- KORREKTUR: Vereinfachte Lade-Logik für den Anzeigenamen ---
        # Prüft direkt, ob der Wert im JSON existiert und nicht leer ist.
        if (-not [string]::IsNullOrWhiteSpace($settings.EmailDisplayName)) {
            $textboxEmailDisplay.Text = $settings.EmailDisplayName
        } else {
            # Fallback, falls im JSON leer oder nicht vorhanden
        ...
        # Lade Replikations- & Klon/Restore-Einstellungen
