# GhettoGUI_V8.3.4.0.ps1
#
# --- VERSION V8.3.4.0 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V8.3.4.0 / 16.01.2026"
...
log_raw "Job-Konfiguration:"; log_raw "  - Typ: Manueller Multi-VM Hot-Klon V8.3.4"; log_raw "  - Ziel-Datastore: ${TARGET_DATASTORE}"; log_raw "  - Namens-Vorlage: ${NAME_TEMPLATE}"; log_raw "Speicherplatz (Vorher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(df -h "${TARGET_DATASTORE}" 2>/dev/null | grep /vmfs/)"; log_raw "--- START DES DETAILLOGS ---";
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"; log_raw "  - Typ: Lokaler Hot-Klon V8.3.4"; log_raw "  - Quelle: ${SOURCE_VM_NAME}"; log_raw "--- START DES DETAILLOGS ---"
...
log "====== GEPLANTER VM KLON-PROZESS GESTARTET (ID: ${UNIQUE_ID}) ======"; log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"; log_raw "Job-Konfiguration:"; log_raw "  - Typ: Geplanter Lokaler Hot-Klon V8.3.4"; log_raw "  - Quelle: ${SOURCE_VM_NAME}"; log_raw "--- START DES DETAILLOGS ---"
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"; log_raw "  - Typ: Geplanter Multi-VM Hot-Klon V8.3.4"; log_raw "  - Ziel-Datastore: ${TARGET_DATASTORE}"; log_raw "  - Namens-Vorlage: ${NAME_TEMPLATE}"; log_raw "Speicherplatz (Vorher):"; log_raw "  - Ziel (${TARGET_DATASTORE}): $(df -h "${TARGET_DATASTORE}" 2>/dev/null | grep /vmfs/)"; log_raw "--- START DES DETAILLOGS ---";
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"
log_raw "  - Typ: GhettoVCB Wiederherstellung V8.3.4"
    ...
    try {
        ...
        try {
            ...
            if ($radioScheduleReplication.Checked) {
                ...
                # --- HIER IST DIE ÄNDERUNG IN $confLines ---
                $confLines = @( 
...
# UPDATE V56.1: Dynamic Log Filename & Disk Format Support
# - FEATURE: Log-Dateiname enthält jetzt Zeitstempel, um Überschreiben bei Cron-Jobs zu verhindern.
# - FEATURE: Unterstützt Thin, ZeroedThick und EagerZeroedThick (via GUI).
# - LOGIC: 'vmkfstools -K' (Hole Punching) läuft nur, wenn 'thin' gewählt wurde.
# - RETAINS: ESXi 6 SSH-Fix, Late Copy, 2-Step Rename.
# ------------------------------------------------------------------

$masterHelperScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V56.1 (Log History & Disk Format)
# - FEATURE: Log file now includes timestamp to prevent overwriting history.
# - FEATURE: Uses __DISK_FORMAT__ for Hot-Clones via vmkfstools.
# - LOGIC: Only executes Hole Punching (-K) for Cold-Clones/Transfers if format is 'thin'.
# - FIX ESXi 6: Removes '-T' flag from SCP commands.
...
# --- V56.1 UPDATE: Added Date to Log Filename ---
...
# --- V54.9 FIX: Define Base Options, then split for SSH vs SCP ---
...
SSH_OPTS="-T ${BASE_OPTS}"   # SSH needs -T
SCP_OPTS="${BASE_OPTS}"      # SCP crashes with -T on ESXi 6
# ---------------------------------------------------------------
...
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Geplante Replikation (H2H V8.3.4)"
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    log "#################### Starte Verarbeitung für VM: ${SOURCE_VM_NAME} ########V56.1############"
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        ...
        while read -r line; do
            ...
            # V56.1: Use dynamic DISK_FORMAT variable
            ...
            # --- V56.1 THIN PROVISIONING ENFORCEMENT (CONDITIONAL) ---
            if [ "${DISK_FORMAT}" = "thin" ]; then
                log "     ...Optimiere Speicherplatz auf Ziel (Hole Punching)..."
                ssh ${SSH_OPTS} root@${TARGET_HOST} "vmkfstools -K '${TARGET_VM_PATH}/${TARGET_FILENAME}'"
            fi
            # ---------------------------------------------------------
        ...
        while read -r line; do
            ...
            while kill -0 ${SCP_PID} >/dev/null 2>&1; do
                 ...
                 if [ -n "${CURRENT_SIZE}" ]; then log "Cold-Copy: ${SOURCE_VM_NAME} ${CURRENT_SIZE}"; fi
                 sleep 15
            done
            wait ${SCP_PID}
            ...
            if [ "${DISK_FORMAT}" = "thin" ]; then
                log "     ...Optimiere Speicherplatz auf Ziel (Hole Punching)..."
                ssh ${SSH_OPTS} root@${TARGET_HOST} "vmkfstools -K '${TARGET_VM_PATH}/${TARGET_DISK_NAME}'"
            fi
            # ---------------------------------------------------------
        ...
        # 1. Copy ORIGINAL filename with correct options
    ...
    # --- DEBUGGING OUTPUT ---
    log "DEBUG: Inhalt des Zielordners vor Registrierung:"
    DEBUG_LS=$(ssh ${SSH_OPTS} root@${TARGET_HOST} "ls -la '${TARGET_VM_PATH}'" 2>&1)
    log_raw "${DEBUG_LS}"
    # ------------------------
...
#---------------------------------------------------------------------------------
# GhettoGUI Multi-VM Replication Helper - V54.10 (Anti-Hang Fix)
# - FIX: Added '-o BatchMode=yes' to prevent endless hangs if SSH asks for password/confirmation.
# - FIX: Replaced SSH_OPTIONS with split logic (SSH_OPTS / SCP_OPTS) to support ESXi 6.
#---------------------------------------------------------------------------------

$multiVmScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V54.11 (Manual - Safe Parsing)
# - FIX: Replaces complex remote SSH pipes with simple fetch + local parsing to prevent hangs on ESXi 6.
# - INCLUDES: BatchMode, Split SSH/SCP Options, Late Copy, 2-Step Rename.
...
# --- SSH CONFIGURATION (V54.11) ---
# BatchMode=yes: Prevents hanging on password prompts.
# ConnectTimeout=20: Prevents hanging on network drops.
...
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manuelle Replikation (H2H) V8.3.4"
log_raw "  - Methode: ${REPLICATION_METHOD}"
log_raw "  - Quell-Host: ${SRC_IP} (${SRC_HOSTNAME})"
log_raw "  - Ziel-Host: ${TARGET_HOST} (${TARGET_HOSTNAME})"
log_raw "  - Ziel-Datastore: ${TARGET_DS_NAME}"
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    log "#################### Starte Verarbeitung für VM: ${SOURCE_VM_NAME} ########V54.11############"
    ...
    # --- V54.11 FIX: SAFE REMOTE VM CHECK (No complex pipes over SSH) ---
    log "[1/7] Bereinige alte Replikation..."
    
    # 1. Hole rohe Liste vom Ziel (simpler Befehl, weniger Fehleranfällig)
    ...
    # 2. Suche ID lokal (Sicher & Robust)
    # Spalte 1 = ID, Spalte 2 = Name. Wir suchen exakten Namen.
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ] && [ "${VM_POWER_STATE}" = "on" ]; then
        ...
        # --- Step 3 SKIP (Moved to end for Hot-Clone) ---
        ...
        while read -r line; do
            ...
            while kill -0 ${VMKF_PID} >/dev/null 2>&1; do
                ...
                if [ -n "${PROGRESS_LINE}" ]; then 
                    ...
                    CURRENT_CLONE_SIZE=$(du -sh "${TEMP_CLONE_DIR}" 2>/dev/null | awk '{print $1}')
                    if [ -n "${CURRENT_CLONE_SIZE}" ]; then log "     ...Klonen init: ${CURRENT_CLONE_SIZE}"; fi
                fi
                sleep 30
            done
            wait ${VMKF_PID}; if [ $? -ne 0 ]; then log_error "Fehler beim Klonen von Disk ${DISK_IDX}"; OVERALL_STATUS="ERROR"; continue; fi
        ...
        # --- V54.9: LATE COPY with SCP_OPTS (NO -T FLAG) ---
        log "[5/7] Kopiere Konfigurationsdateien (LATE + 2-STEP)..."
        VMX_FILE_NAME=$(/usr/bin/basename "${VMX_FULL_PATH}")
        VM_NVRAM_FILE=$(/bin/grep -i 'nvram' "${VMX_FULL_PATH}" | /usr/bin/awk -F '"' '{print $2}')
        ...
        # Use SSH_OPTS (with -T) for rename
        ssh ${SSH_OPTS} root@${TARGET_HOST} "cd '${TARGET_VM_PATH}' && mv '${VMX_FILE_NAME}' '${REPLICATED_VM_NAME}.vmx'"
        ...
        # --- COLD COPY FINALE (2-STEP & SCP_OPTS) ---
        log "[5/7] Kopiere Konfigurationsdateien..."
        VMX_FILE_NAME=$(/usr/bin/basename "${VMX_FULL_PATH}")
        VM_NVRAM_FILE=$(/bin/grep -i 'nvram' "${VMX_FULL_PATH}" | /usr/bin/awk -F '"' '{print $2}')
