...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V6.3.5"
...
# =====================================================================================
# --- STARTBLOCK ZUM ERSETZEN (Die Klick-Funktion des Firewall-Buttons) ---
# =====================================================================================
...
# ---------------------------------------------------------------------------------
# Start Replikation und Backup planen#
#----------------------------------------------------------------------------------
...
# Dies ist die finale Vorlage, die die bewährte Logik der manuellen Replikation enthält
            $masterHelperScriptTemplate = @'
#!/bin/sh
# GhettoGUI Scheduled Replication Helper V36.0.0 (Final Email Parsing Fix)
set -e
set -o pipefail
...
LOG_FILE="${GHETTO_PATH}/logs/scheduled_replication_${UNIQUE_ID}.log"
...
DIRECTORY_LISTING_CONTENT="Directory Listing konnte nicht abgerufen werden."

log() { echo "$(date '+%Y-%m-%d %H:%M:%S') -- $1" >> ${LOG_FILE}; }

send_email_notification() {
    # *** HIER IST FIX #1 ***
    log "--- START Backup Directory Listing ---"
    echo "${DIRECTORY_LISTING_CONTENT}" >> ${LOG_FILE}
    log "--- END Backup Directory Listing ---"

    log "Bereite E-Mail vor mit finalem Gesamtstatus: ${OVERALL_STATUS}"
    ...
    echo "Backup Duration: N/A" >> ${LOG_FILE}; echo "${FINAL_STATUS_MSG}" >> ${LOG_FILE}
    if [ "${EMAIL_ENABLED}" = "1" ]; then
        if [ -f "${SENDMAIL_PATH}" ]; then
            log "Sende E-Mail-Benachrichtigung an: ${EMAIL_TO}"
...
cleanup() {
    FINAL_EXIT_CODE=$?
    log "INFO: Globale Aufräumarbeiten werden mit Exit-Code ${FINAL_EXIT_CODE} ausgeführt..."
    
    if [ ${FINAL_EXIT_CODE} -ne 0 ] && [ "${OVERALL_STATUS}" != "OK" ]; then
        log "Ein Fehler ist im Hauptskript aufgetreten. Status bleibt 'ERROR'."
    fi

    send_email_notification
    log "====== SCHEDULED REPLICATION HELPER BEENDET (ID: ${UNIQUE_ID}) ======"
}
...
for SOURCE_VM_NAME in ${VM_LIST}; do
    ...
    if [ -z "${VM_INFO_LINE}" ]; then log "FEHLER: VM '${SOURCE_VM_NAME}' nicht gefunden! Überspringe..."; continue; fi
    VMID=$(echo "${VM_INFO_LINE}" | awk '{print $1}')
    VMX_FULL_PATH=$(echo "${VM_INFO_LINE}" | sed -e 's/.*\[\(.*\)\]\s*\(.*\.vmx\).*/\/vmfs\/volumes\/\1\/\2/')
    VMX_DIR=$(dirname "${VMX_FULL_PATH}")
    log "  -> VMID: ${VMID}, VMX_DIR: ${VMX_DIR}"

    log "[2/9] Erstelle Snapshot..."
    /bin/vim-cmd vmsvc/snapshot.create ${VMID} "ghetto-sched-repl-${UNIQUE_ID}" "GhettoGUI Sched" ${SNAP_MEM} ${SNAP_QUIESce}
    log "Warte 10 Sekunden..."
    sleep 10

    log "[3/9] Starte lokalen Klon-Prozess..."
    TEMP_CLONE_DIR="${VMX_DIR}/ghetto_clone_${UNIQUE_ID}_${SOURCE_VM_NAME}"
    mkdir -p "${TEMP_CLONE_DIR}"
    log "  -> Temporäres Verzeichnis: ${TEMP_CLONE_DIR}"
    grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)' | while IFS= read -r line; do
        DISK_FILE_NAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p')
        if [ -z "${DISK_FILE_NAME}" ]; then continue; fi
        ORIGINAL_DISK_BASENAME=$(echo "${DISK_FILE_NAME}" | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/')
        SOURCE_DISK_PATH="${VMX_DIR}/${ORIGINAL_DISK_BASENAME}"
        TEMP_DESTINATION_DISK_PATH="${TEMP_CLONE_DIR}/${ORIGINAL_DISK_BASENAME}"
        log "    -> Klone (Basis-Disk): ${SOURCE_DISK_PATH} nach ${TEMP_DESTINATION_DISK_PATH}"
        /sbin/vmkfstools -i "${SOURCE_DISK_PATH}" -d thin "${TEMP_DESTINATION_DISK_PATH}" >> ${LOG_FILE} 2>&1
    done
    (cd "${VMX_DIR}" && find . -maxdepth 1 ! -name '*.vmdk' -exec cp -p '{}' "${TEMP_CLONE_DIR}/" \;)
    log "  -> OK: Lokaler Klon-Prozess abgeschlossen."

    log "[4/9] Starte TAR-Transfer zum Ziel-Host..."
    ssh ${SSH_OPTIONS} root@${TARGET_HOST} "rm -rf '${TARGET_VM_PATH}'; mkdir -p '${TARGET_VM_PATH}'"
    SOURCE_SIZE=$(du -sh "${TEMP_CLONE_DIR}" | awk '{print $1}')
    log "  -> Gesamtgröße: ~${SOURCE_SIZE}"
    ( (cd "${TEMP_CLONE_DIR}" && tar -cf - .) | ssh ${SSH_OPTIONS} root@${TARGET_HOST} "tar -xf - -C '${TARGET_VM_PATH}'" )
    log "  -> OK: Datentransfer erfolgreich."
    
    log "[5/9] Passe Zieldateien an..."
    ssh ${SSH_OPTIONS} root@${TARGET_HOST} "
    set -e
    cd '${TARGET_VM_PATH}'
    ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\")
    if [ -z \"\${ORIG_VMX_FILE}\" ]; then echo 'FEHLER: Keine VMX-Datei gefunden!'; exit 1; fi
    RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"
    mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"
    sed -i -e \"s/^displayName = .*/displayName = \\\"${REPLICATED_VM_NAME}\\\"/\" -e 's/-0000[0-9]*.vmdk/.vmdk/g' \"\${RENAMED_VMX_FILE}\"
    " >> ${LOG_FILE} 2>&1

    log "[6/9] Registriere VM..."
    TARGET_VMX_PATH_ON_TARGET="${TARGET_VM_PATH}/${REPLICATED_VM_NAME}.vmx"
    REG_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd solo/registervm '${TARGET_VMX_PATH_ON_TARGET}'")
    if [ \$? -ne 0 ]; then log "FATAL: VM-Registrierung fehlgeschlagen!"; exit 1; fi
    log "  -> OK: VM registriert als VMID ${REG_OUTPUT}."

    log "[7/9] Lösche Snapshot für diese VM..."
    /bin/vim-cmd vmsvc/snapshot.removeall ${VMID}

    log "[8/9] Lösche temporäres Verzeichnis für diese VM..."
    rm -rf "${TEMP_CLONE_DIR}"

    log "[9/9] Prüfe Konsolidierung und hole Directory Listing..."
    NEW_VMID=$(echo $REG_OUTPUT)
    if [ -n "$NEW_VMID" ]; then
        if REMOTE_CMD_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd vmsvc/get.needssnapshotconsolidation ${NEW_VMID}" 2>/dev/null); then
            NEEDS_CONSOLIDATION=$(echo "${REMOTE_CMD_OUTPUT}" | tail -n 1 | tr -d "'[:space:]'")
            if [ "${NEEDS_CONSOLIDATION}" = "1" ] || [ "${NEEDS_CONSOLIDATION}" = "true" ]; then
                log "  -> Konsolidierung wird benötigt. Starte Task..."
                ssh ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd vmsvc/snapshot.consolidate ${NEW_VMID}"
            else
                log "  -> Keine Konsolidierung notwendig."
            fi
        else
            log "  -> WARNUNG: Konnte den Konsolidierungs-Status nicht prüfen. Überspringe."
        fi
    fi
    ...
    if LISTING_CMD_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lR '${TARGET_VM_PATH}'" 2>/dev/null); then
        ...
        log "  -> Directory Listing erfolgreich abgerufen."
    else
        log "  -> WARNUNG: Directory Listing konnte nicht abgerufen werden."
    fi
    
    log "###### Verarbeitung für VM: ${SOURCE_VM_NAME} erfolgreich abgeschlossen."
done

# Wenn wir diesen Punkt erreichen, war alles erfolgreich.
OVERALL_STATUS="OK"
'@
            ...
            # Bestehende, funktionierende Logik für geplante Backups
            ...
            $confLines = @(
                "USE_FIXED_BACKUP_DIR=$useFixedDirValue", # HIER IST DER FIX
                "VM_BACKUP_VOLUME=`"$($textboxBackupVol.Text.TrimEnd('/'))/$($textboxSubfolder.Text.Trim('/'))`"", "VM_BACKUP_ROTATION_COUNT=$($textboxRotation.Text)", "DISK_BACKUP_FORMAT=`"$($comboboxDiskFormat.SelectedItem.ToString())`"", "SNAPSHOT_MEMORY=$(if ($checkboxSnapMem.Checked) { 1 } else { 0 })", "SNAPSHOT_QUIESCE=$(if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 })",
                "EMAIL_LOG=$(if ($checkboxEmailLog.Checked) { 1 } else { 0 })", "EMAIL_SERVER=`"$($textboxEmailServer.Text)`"", "EMAIL_SERVER_PORT=`"$($textboxEmailPort.Text)`"",
                "EMAIL_USER_NAME=`"$($textboxEmailUser.Text)`"", "EMAIL_USER_PASSWORD=`"$($textboxEmailPassword.Text)`"", "EMAIL_FROM=`"$($textboxEmailFrom.Text)`"", "EMAIL_TO=`"$($textboxEmailTo.Text)`"", "EMAIL_SUBJECT=`"$($textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName))`"", "EMAIL_BIN=`"$ghettoPath/sendmail`""
            )
            ...
            $sftpSession = New-SFTPSession -ComputerName $Global:ESXiConnectedHostName -Credential $Global:ESXiSshCredential
            try {
                Set-SFTPContent -SFTPSession $sftpSession -Path $remoteGhettoConf -Value $ghettoConfContent -Encoding UTF8
                Set-SFTPContent -SFTPSession $sftpSession -Path $remoteVmListFile -Value $unixVmListText -Encoding UTF8
            } finally {
                if ($sftpSession) { Remove-SFTPSession -SFTPSession $sftpSession }
            }
...
# =====================================================================================
# --- START BLOCK (Exakter Name via "["-Split + Originalzeile im Log) ---
# =====================================================================================
...
function Start-ReplicationForVM {
    ...
    $masterHelperScriptTemplate = @'
#!/bin/sh
# GhettoGUI Master Replication Helper V31.0.0 (User's Logic + Base Disk Clone Fix)
set -e
set -o pipefail

# ... (Parameter-Definitionen und Funktionen bleiben unverändert) ...
UNIQUE_ID='__UNIQUE_ID__'
SOURCE_VM_NAME='__SOURCE_VM_NAME__'
TARGET_HOST='__TARGET_HOST__'
TARGET_DATASTORE='__TARGET_DATASTORE__'
VM_SUFFIX='__VM_SUFFIX__'
REPLICATED_VM_NAME="${SOURCE_VM_NAME}__VM_SUFFIX__"
...
cleanup() {
    ...
    if [ -n "${TEMP_CLONE_DIR}" ]; then
        log "INFO: Lösche temporäres Klon-Verzeichnis auf Quell-Host: ${TEMP_CLONE_DIR}"
        rm -rf "${TEMP_CLONE_DIR}"
    fi
    if [ -n "${VMID}" ]; then
        log "INFO: Entferne eventuellen Snapshot von VMID ${VMID}..."
        vim-cmd vmsvc/snapshot.removeall ${VMID} >> ${LOG_FILE} 2>&1
    fi
    send_email_notification
...
send_email_notification() {
    ...
    if [ "${REPLICATION_STATUS}" = "OK" ]; then
        ssh ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lR '${TARGET_DATASTORE}/${REPLICATED_VM_NAME}'" >> ${LOG_FILE} 2>&1
    fi
    log "--- END Backup Directory Listing ---"
    log "Bereite E-Mail vor mit finalem Status: ${REPLICATION_STATUS}"
    if [ "${REPLICATION_STATUS}" = "OK" ]; then FINAL_STATUS_MSG="###### Final status: All VMs backed up OK! ######"; else FINAL_STATUS_MSG="###### Final status: ERROR: Replication failed! ######"; fi
    echo "Backup Duration: N/A" >> ${LOG_FILE}; echo "${FINAL_STATUS_MSG}" >> ${LOG_FILE}
    if [ "${EMAIL_ENABLED}" = "1" ]; then
        if [ -f "${SENDMAIL_PATH}" ]; then
            log "Sende E-Mail-Benachrichtigung an: ${EMAIL_TO}"
...
log "info: Initiate backup for ${SOURCE_VM_NAME}"
log "Ziel: ${TARGET_HOST} -> ${TARGET_DATASTORE}";

log "[1/9] Finde VMID und VMX-Pfad..."
VM_INFO_LINE=$(vim-cmd vmsvc/getallvms | awk -v name="${SOURCE_VM_NAME}" '{ n=split($0, a, "\\["); vm=a[1]; sub(/^[0-9]+[ \t]+/, "", vm); sub(/[ \t]+$/, "", vm); if (vm == name) print $0; }')
if [ -z "${VM_INFO_LINE}" ]; then log "FEHLER: VM '${SOURCE_VM_NAME}' nicht gefunden!"; exit 1; fi
VMID=$(echo "${VM_INFO_LINE}" | awk '{print $1}')
VMX_FULL_PATH=$(echo "${VM_INFO_LINE}" | sed -e 's/.*\[\(.*\)\]\s*\(.*\.vmx\).*/\/vmfs\/volumes\/\1\/\2/')
VMX_DIR=$(dirname "${VMX_FULL_PATH}")

log "[2/9] Erstelle Snapshot (um Basis-VMDK freizugeben)..."
vim-cmd vmsvc/snapshot.create ${VMID} "ghetto-repl-${UNIQUE_ID}" "GhettoGUI Replication" __SNAP_MEM__ __SNAP_QUIESCE__
log "Warte 10 Sekunden..."
sleep 10

log "[3/9] Starte lokalen Klon-Prozess auf dem Quell-Datastore..."
TEMP_CLONE_DIR="${VMX_DIR}/ghetto_clone_${UNIQUE_ID}"
mkdir -p "${TEMP_CLONE_DIR}"
log "  -> Temporäres Verzeichnis: ${TEMP_CLONE_DIR}"

# ##################################################################
# ## FINALE ANPASSUNG: Klone die Basis-Festplatte(n), nicht den Snapshot
# ##################################################################
grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)' | while IFS= read -r line; do
    DISK_FILE_NAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p')
    if [ -z "${DISK_FILE_NAME}" ]; then continue; fi

    # Leite den Namen der Basis-Festplatte ab und verwende diesen als Quelle
    ORIGINAL_DISK_BASENAME=$(echo "${DISK_FILE_NAME}" | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/')
    SOURCE_DISK_PATH="${VMX_DIR}/${ORIGINAL_DISK_BASENAME}"
    TEMP_DESTINATION_DISK_PATH="${TEMP_CLONE_DIR}/${ORIGINAL_DISK_BASENAME}"
    
    log "    -> Klone (Basis-Disk): ${SOURCE_DISK_PATH} nach ${TEMP_DESTINATION_DISK_PATH}"
    vmkfstools -i "${SOURCE_DISK_PATH}" -d thin "${TEMP_DESTINATION_DISK_PATH}" >> ${LOG_FILE} 2>&1
done
(cd "${VMX_DIR}" && find . -maxdepth 1 ! -name '*.vmdk' -exec cp -p '{}' "${TEMP_CLONE_DIR}/" \;)
log "  -> OK: Lokaler Klon-Prozess abgeschlossen."
...
( (cd "${TEMP_CLONE_DIR}" && tar -cf - .) | ssh ${SSH_OPTIONS} root@${TARGET_HOST} "tar -xf - -C '${TARGET_VM_PATH}'" ) &
TAR_PID=$!
log "  -> TAR-Prozess gestartet mit PID: ${TAR_PID}"
...
while kill -0 ${TAR_PID} >/dev/null 2>&1; do
    ...
    if [ -n "${PROGRESS}" ]; then log " -> Fortschritt: ${PROGRESS} von ~${SOURCE_SIZE}"; fi
    sleep 15
done
wait ${TAR_PID}
TRANSFER_EXIT_CODE=$?
if [ ${TRANSFER_EXIT_CODE} -ne 0 ]; then log "FATAL: TAR-Transfer fehlgeschlagen mit Exit-Code ${TRANSFER_EXIT_CODE}!"; exit 1; fi
log "  -> OK: Datentransfer erfolgreich."

log "[5/9] Passe Zieldateien an (vereinfachte Methode)..."
ssh ${SSH_OPTIONS} root@${TARGET_HOST} "
set -e
cd '${TARGET_VM_PATH}'
ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\")
if [ -z \"\${ORIG_VMX_FILE}\" ]; then 
    echo 'FEHLER: Keine VMX-Datei im Zielverzeichnis gefunden!'
    exit 1
fi
RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"
mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"
sed -i -e \"s/^displayName = .*/displayName = \\\"${REPLICATED_VM_NAME}\\\"/\" \
       -e 's/-0000[0-9]*.vmdk/.vmdk/g' \"\${RENAMED_VMX_FILE}\"
" >> ${LOG_FILE} 2>&1

log "[6/9] Registriere VM..."
TARGET_VMX_PATH_ON_TARGET="${TARGET_VM_PATH}/${REPLICATED_VM_NAME}.vmx"
REG_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "vim-cmd solo/registervm '${TARGET_VMX_PATH_ON_TARGET}'")
if [ \$? -ne 0 ]; then log "FATAL: VM-Registrierung fehlgeschlagen!"; exit 1; fi
log "  -> OK: VM registriert."

REPLICATION_STATUS="OK"

log "[7/9] Lösche Snapshot..."
vim-cmd vmsvc/snapshot.removeall ${VMID}

log "[8/9] Prüfe und führe Konsolidierung auf Ziel-VM aus..."
NEW_VMID=$(echo $REG_OUTPUT)
if [ -n "$NEW_VMID" ]; then
    ...
    if [ "${NEEDS_CONSOLIDATION}" = "1" ] || [ "${NEEDS_CONSOLIDATION}" = "true" ]; then
        log "  -> Konsolidierung wird benötigt. Starte Task..."
        ssh ${SSH_OPTIONS} root@${TARGET_HOST} "vim-cmd vmsvc/snapshot.consolidate ${NEW_VMID}"
    else
        log "  -> Keine Konsolidierung notwendig."
    fi
fi
log "[9/9] Replikation erfolgreich abgeschlossen."
'@
    
    $finalMasterScript = $masterHelperScriptTemplate `
        -replace '__UNIQUE_ID__', $params.UniqueId `
        -replace '__SOURCE_VM_NAME__', $params.SourceVmName `
        -replace '__TARGET_HOST__', $params.TargetHost `
        -replace '__TARGET_DATASTORE__', $params.TargetDatastore `
        -replace '__VM_SUFFIX__', $params.Suffix `
        -replace '__SNAP_MEM__', $params.SnapMem `
    ...
    try {
        ...
        [System.Windows.Forms.MessageBox]::Show("Fehler beim Hochladen der Skripte: $($_.Exception.Message)", "Upload-Fehler", "OK", "Error")
        return
    } finally {
        if ($sftpSession) { Remove-SFTPSession -SFTPSession $sftpSession }
    }
    ...
    try {
        ...
        Write-GuiLog "-> INFO: Erwarteter Posh-SSH-Timeout. Das ist normal."
    }

    $plainTextPassword = $Global:ESXiSshCredential.GetNetworkCredential().Password
    $jobParamsForWatcher = @{
        Host = $params.SourceHost; Username = $Global:ESXiSshCredential.UserName; PlainTextPassword = $plainTextPassword; RemoteLogPath = "/tmp/master_replication_$($params.UniqueId).log"
    }
    ...
    $Global:replicationJob = Start-Job -ScriptBlock { 
        ...
        try {
            ...
            Write-Output "FATALER FEHLER im Watcher-Job: $($_.Exception.Message)"
        } finally {
            if ($watcherSession) { Remove-SSHSession -SSHSession $watcherSession }
        }
...
# NEU: Diese Funktion steuert den Installations-Flow
function Start-GhettoVCBInstallationFlow {
    ...
    # Ansonsten passiert nichts (Dialog wurde geschlossen)
}
