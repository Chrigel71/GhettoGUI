# GhettoGUI_V7.6.3.ps1
#
# --- VERSION V7.6.3 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V7.6.3 / 29.09.2025"
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    log "[1/6] Erstelle Zielverzeichnis..."; rm -rf "${TARGET_VM_PATH}"; mkdir -p "${TARGET_VM_PATH}";
    log "[2/6] Erstelle Snapshot..."; /bin/vim-cmd vmsvc/snapshot.create ${VMID} "${SNAPSHOT_NAME}" "GhettoGUI Multi-Clone" 0 ${SNAP_QUIESCE}; log "  -> Warte 15s..."; sleep 15;

    log "[3/6] Klone BASIS-Festplatten...";
...
'; for line in ${DISK_DEFS_ORIG}; do
    ...
    log "[4/6] Kopiere essentielle Konfigurationsdateien...";
    (cd "${VMX_DIR}" && find . -maxdepth 1 \( -name "*.vmx" -o -name "*.nvram" \) -exec cp -p '{}' "${TARGET_VM_PATH}/" \;)

    log "[5/6] Passe Konfigurationsdateien an...";
    (
    ...
    log "[6/6] Registriere Klon & entferne Snapshot...";
...
$cloneScriptTemplate = @'
#!/bin/sh
# GhettoGUI Single-VM Clone Helper V50.1 (Golden Master)
# - FINAL: Stops copying non-essential metadata (.vmsd, .vmxf) to prevent "Invalid State" errors.
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
if [ "${OVERALL_STATUS}" = "OK" ]; then
    TARGET_VM_PATH="${TARGET_DATASTORE}/${NEW_VM_NAME}"; log "[1/6] Erstelle Zielverzeichnis..."; rm -rf "${TARGET_VM_PATH}"; mkdir -p "${TARGET_VM_PATH}";
    log "[2/6] Erstelle Snapshot..."; vim-cmd vmsvc/snapshot.create ${VMID} "${SNAPSHOT_NAME}" "GhettoGUI Live Clone" 0 ${SNAP_QUIESCE}; log "  -> Warte 15s..."; sleep 15;
    log "[3/6] Klone BASIS-Festplatten...";
    DISK_DEFS_ORIG=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)'); CLONE_ERROR=0;
    OLD_IFS=$IFS; IFS='
'; for line in ${DISK_DEFS_ORIG}; do
...
if [ "${OVERALL_STATUS}" = "OK" ]; then
    if [ "${OVERALL_STATUS}" = "OK" ]; then
    log "[4/6] Kopiere essentielle Konfigurationsdateien..."; (cd "${VMX_DIR}" && find . -maxdepth 1 \( -name "*.vmx" -o -name "*.nvram" \) -exec cp -p '{}' "${TARGET_VM_PATH}/" \;)
    log "[5/6] Passe Konfigurationsdateien an...";
    (
        cd "${TARGET_VM_PATH}"; ORIG_VMX_FILE=$(find . -maxdepth 1 -name "*.vmx"); ORIG_BASENAME=$(basename "${ORIG_VMX_FILE}" .vmx);
        for f in "${ORIG_BASENAME}"*; do
...
VM_SIZE=$(du -sh "${TARGET_VM_PATH}" | awk '{print $1}');
VM_REPORT_LIST=$(printf "%s- %s -> %s (%s)\n" "" "${SOURCE_VM_NAME}" "${NEW_VM_NAME}" "${VM_SIZE}")
...
TARGET_VM_PATH="${TARGET_DATASTORE}/${NEW_VM_NAME}"; log "[1/6] Erstelle Zielverzeichnis..."; rm -rf "${TARGET_VM_PATH}"; mkdir -p "${TARGET_VM_PATH}";
log "[2/6] Erstelle Snapshot..."; vim-cmd vmsvc/snapshot.create ${VMID} "${SNAPSHOT_NAME}" "GhettoGUI Scheduled Clone" 0 ${SNAP_QUIESCE}; log "  -> Warte 15s..."; sleep 15;
log "[3/6] Klone BASIS-Festplatten...";
DISK_DEFS_ORIG=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)'); CLONE_ERROR=0;
OLD_IFS=$IFS; IFS='
'; for line in ${DISK_DEFS_ORIG}; do
...
log "[4/6] Kopiere Konfigurationsdateien..."; (cd "${VMX_DIR}" && find . -maxdepth 1 \( -name "*.vmx" -o -name "*.nvram" -o -name "*.vmsd" \) -exec cp -p '{}' "${TARGET_VM_PATH}/" \;)
log "[5/6] Passe Konfigurationsdateien an...";
(
    cd "${TARGET_VM_PATH}"; ORIG_VMX_FILE=$(find . -maxdepth 1 -name "*.vmx"); ORIG_BASENAME=$(basename "${ORIG_VMX_FILE}" .vmx);
    for f in "${ORIG_BASENAME}"*; do
...
#!/bin/sh
# GhettoGUI Scheduled Multi-VM Clone Helper V4.0 (Final Logic)
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
# --- Skriptstart ---
START_TIME=$(date +%s)
# Minimaler Trap nur für absolute Notfälle
trap 'log_error "Unerwarteter, fataler Fehler. Notfall-Abbruch."; exit 1' EXIT
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    TARGET_VM_PATH="${TARGET_DATASTORE}/${NEW_VM_NAME}"
    log "[1/6] Erstelle Zielverzeichnis..."; rm -rf "${TARGET_VM_PATH}"; mkdir -p "${TARGET_VM_PATH}"
    log "[2/6] Erstelle Snapshot..."; /bin/vim-cmd vmsvc/snapshot.create ${VMID} "${SNAPSHOT_NAME}" "GhettoGUI Scheduled Clone" 0 ${SNAP_QUIESCE}; log "  -> Warte 15s..."; sleep 15
...
$buttonRestore.Add_Click({
    ...
    if ($selectedVmCount -gt 1) {
        ...
        $labelRestoreSource.Text = "Zu klonende Quell-VM:"; $labelRestoreNewVmName.Text = "Neuer Name für die geklonte VM:"
        ...
        $labelRestoreSource.Text = "Zu wiederherstellendes Backup:"; $labelRestoreNewVmName.Text = "Neuer Name für die wiederhergestellte/geklonte VM:"
    }
...
$buttonSaveSchedule.Add_Click({
    ...
    try {
        ...
        if ($radioScheduleReplication.Checked) {
            ...
            # --- HIER IST DIE KORREKTUR ---
            # Die fehlerhafte Abfrage eines nicht-existenten Buttons wurde entfernt und die Methode fest auf 'robust' gesetzt.
            $selectedMethod = 'robust'
            
            $tempPathForSchedule = if ($checkboxUseLocalTemp.Checked) { "" } else { $textboxDrTempPath.Text.TrimEnd('/') }
            
            $params = @{
                UNIQUE_ID            = $jobId; TARGET_HOST = $textboxDrTargetHost.Text; TARGET_DATASTORE = $textboxDrTargetDs.Text; VM_SUFFIX = $textboxDrSuffix.Text;
                REPLICATION_METHOD   = $selectedMethod; TEMP_CLONE_BASE_PATH = $tempPathForSchedule; SNAP_MEM = if ($checkboxSnapMem.Checked) { 1 } else { 0 };
                SNAP_QUIESCE         = if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 }; GHETTO_PATH = $ghettoPath; EMAIL_ENABLED = if ($checkboxEmailLog.Checked) { 1 } else { 0 };
                EMAIL_TO             = $textboxEmailTo.Text; EMAIL_FROM = $textboxEmailFrom.Text; EMAIL_SERVER = $textboxEmailServer.Text; EMAIL_PORT = $textboxEmailPort.Text;
                EMAIL_USER           = $textboxEmailUser.Text; EMAIL_PASS = $textboxEmailPassword.Text; EMAIL_SUBJECT = "[Scheduled-Repl.] " + $textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName);
            ...
            $selectedVmCount = $checkedListBoxVms.CheckedItems.Count
            if ($selectedVmCount -eq 0) { throw "Für einen geplanten Klon-Job muss mindestens eine VM in der Hauptliste ausgewählt sein." }
            
            $isRestoreMode = $radioRestoreFromBackup.Checked
            ...
            if ($isRestoreMode) {
                ...
                if ([string]::IsNullOrWhiteSpace($source) -or [string]::IsNullOrWhiteSpace($targetDs) -or [string]::IsNullOrWhiteSpace($newName)) {
                    throw "Für einen geplanten Restore-Job müssen die Details zuerst über den 'Restore / Klon' Button konfiguriert werden."
                }
                $params = @{
                ...
                if ($selectedVmCount -eq 1) {
                    Write-GuiLog "Erstelle geplanten Single-VM Klon-Job..."
                    $params = @{
                        SOURCE_VM_NAME = $checkedListBoxVms.CheckedItems[0].OriginalName; TARGET_DATASTORE = $textboxRestoreTargetPath.Text; NEW_VM_NAME = $textboxRestoreNewVmName.Text; 
                        POWER_ON = if ($checkboxPowerOnAfterRestore.Checked) { 1 } else { 0 }; UUID_ACTION = if ($radioRestoreMoved.Checked) { "keep" } else { "create" };
                        DISK_FORMAT = $comboboxDiskFormat.SelectedItem.ToString(); SNAP_QUIESCE = if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 };
                        UNIQUE_ID = $jobId; GHETTO_PATH = $ghettoPath; EMAIL_ENABLED = if ($checkboxEmailLog.Checked) { 1 } else { 0 }; EMAIL_TO = $textboxEmailTo.Text; EMAIL_FROM = $textboxEmailFrom.Text; EMAIL_SERVER = $textboxEmailServer.Text; EMAIL_PORT = $textboxEmailPort.Text; EMAIL_USER = $textboxEmailUser.Text; EMAIL_PASS = $textboxEmailPassword.Text;
                    ...
                    $cronComment = "# GhettoGUI - Scheduled Single-Clone (ID: $jobId)"
                    $remoteStarterScriptPath = "$ghettoPath/scheduled_clone_$jobId.sh"

                } else {
                    Write-GuiLog "Erstelle geplanten Multi-VM Klon-Job..."
                    ...
                    $params = @{
                        VM_LIST = ($checkedListBoxVms.CheckedItems | ForEach-Object { $_.OriginalName }) -join "`n"; NAME_TEMPLATE = $textboxRestoreNewVmName.Text;
                        TARGET_DATASTORE = $textboxRestoreTargetPath.Text; POWER_ON = if ($checkboxPowerOnAfterRestore.Checked) { 1 } else { 0 }; UUID_ACTION = if ($radioRestoreMoved.Checked) { "keep" } else { "create" };
                        DISK_FORMAT = $comboboxDiskFormat.SelectedItem.ToString(); SNAP_QUIESCE = if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 };
                        UNIQUE_ID = $jobId; GHETTO_PATH = $ghettoPath; EMAIL_ENABLED = if ($checkboxEmailLog.Checked) { 1 } else { 0 }; EMAIL_TO = $textboxEmailTo.Text; EMAIL_FROM = $textboxEmailFrom.Text; EMAIL_SERVER = $textboxEmailServer.Text; EMAIL_PORT = $textboxEmailPort.Text; EMAIL_USER = $textboxEmailUser.Text; EMAIL_PASS = $textboxEmailPassword.Text;
                        EMAIL_SUBJECT = "[Scheduled-Multi-Clone] " + $textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName)
                    }
                    $finalMasterScript = $scheduledMultiCloneScriptTemplate
                    $cronComment = "# GhettoGUI - Scheduled Multi-Clone (ID: $jobId)"
                    $remoteStarterScriptPath = "$ghettoPath/scheduled_multiclone_$jobId.sh"
                }
            }
            foreach ($key in $params.Keys) { $finalMasterScript = $finalMasterScript.Replace("__$($key)__", $params[$key]) }
            ...
            $confLines = @(
                "USE_FIXED_BACKUP_DIR=$useFixedDirValue", "VM_BACKUP_VOLUME=`"$($textboxBackupVol.Text.TrimEnd('/'))/$($textboxSubfolder.Text.Trim('/'))`"",
                "VM_BACKUP_ROTATION_COUNT=$($textboxRotation.Text)", "DISK_BACKUP_FORMAT=`"$($comboboxDiskFormat.SelectedItem.ToString())`"",
                "VM_SNAPSHOT_MEMORY=$(if ($checkboxSnapMem.Checked) { 1 } else { 0 })", "VM_SNAPSHOT_QUIESCE=$(if ($checkboxSnapQuiesce.Checked) { 1 } else { 0 })",
                "EMAIL_LOG=$(if ($checkboxEmailLog.Checked) { 1 } else { 0 })", "EMAIL_SERVER=`"$($textboxEmailServer.Text)`"",
                "EMAIL_SERVER_PORT=`"$($textboxEmailPort.Text)`"", "EMAIL_USER_NAME=`"$($textboxEmailUser.Text)`"", "EMAIL_USER_PASSWORD=`"$($textboxEmailPassword.Text)`"",
                "EMAIL_FROM=`"$($textboxEmailFrom.Text)`"", "EMAIL_TO=`"$($textboxEmailTo.Text)`"", "EMAIL_SUBJECT=`"$($textboxEmailSubject.Text.Replace('%h', $Global:ESXiConnectedHostName))`"",
...
# Fix: Multi VMDK Pfad, Email Ordner Grösse
# ------------------------------------------------------------------

$masterHelperScriptTemplate = @'
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V43.8.1 (Final Reporting Fix)
# - ADDED: Job configuration details are now logged at the start of the script,
#   which are then parsed by sendmail.py and included in the final email report.
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
# NEU: Job-Konfiguration für den E-Mail-Bericht loggen
log_raw "Job-Konfiguration:"
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ]; then
        ...
        echo "${DISK_DEFINITIONS}" | while read -r line; do ORIGINAL_DISK_BASENAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p' | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/'); if [ -z "${ORIGINAL_DISK_BASENAME}" ]; then continue; fi; if echo "${ORIGINAL_DISK_BASENAME}" | grep -q "^/"; then SOURCE_DISK_PATH_FOR_CLONE="${ORIGINAL_DISK_BASENAME}"; else SOURCE_DISK_PATH_FOR_CLONE="${VMX_DIR}/${ORIGINAL_DISK_BASENAME}"; fi; DESTINATION_DISK_BASENAME=$(basename "${ORIGINAL_DISK_BASENAME}"); vmkfstools -i "${SOURCE_DISK_PATH_FOR_CLONE}" -d thin "${TEMP_CLONE_DIR}/${DESTINATION_DISK_BASENAME}" >> ${LOG_FILE} 2>&1; done
        ...
        log "-> Verifiziere Transfer..."
        SOURCE_SIZE_KB=$(du -sk "${TEMP_CLONE_DIR}" | awk '{print $1}')
        TARGET_SIZE_KB=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "du -sk '${TARGET_VM_PATH}'" < /dev/null | awk '{print $1}')
        MIN_SIZE_KB=$((SOURCE_SIZE_KB * 95 / 100))
        
        if [ -z "${TARGET_SIZE_KB}" ] || [ "${TARGET_SIZE_KB}" -lt "${MIN_SIZE_KB}" ]; then
            log_error "Transfer-Verifizierung fehlgeschlagen! Quelle: ${SOURCE_SIZE_KB}KB, Ziel: ${TARGET_SIZE_KB:-0}KB."
            OVERALL_STATUS="ERROR"; rm -rf "${TEMP_CLONE_DIR}"; ssh ${SSH_OPTIONS} root@${TARGET_HOST} "rm -rf '${TARGET_VM_PATH}'" < /dev/null; continue
        else
            log "-> Transfer erfolgreich verifiziert. Lösche temporäres Verzeichnis."
    ...
    if [ -n "${VM_SIZE}" ]; then
        VM_REPORT_LIST=$(printf "%s- %s: %s\n" "${VM_REPORT_LIST}" "${REPLICATED_VM_NAME}" "${VM_SIZE}")
    fi
done
'@
...
function Save-HostGuiSettings {
    ...
    $settings = @{
        ...
        # --- NEU: Speichern der Klon/Restore-Einstellungen ---
    ...
    try {
        ...
        # Aktualisiere die Job-Liste, um die neue Datei sofort anzuzeigen
        Populate-JobComboBox
    } catch {
...
function Load-HostGuiSettings {
    ...
    try {
        ...
        if ($settings.PSObject.Properties.Name -contains 'DirectTarMode') {
            if ($settings.DirectTarMode -eq 'Backup') { $radioModeBackup.Checked = $true } else { $radioModeReplication.Checked = $true }
        ...
        # --- NEU: Laden der Klon/Restore-Einstellungen ---
