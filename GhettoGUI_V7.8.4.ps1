# GhettoGUI_V7.8.4.ps1
#
# --- VERSION V7.8.4 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V7.8.4 / 12.10.2025"
...
$groupSchedule = New-Object System.Windows.Forms.GroupBox; $groupSchedule.Text = "Zeitplanung"; $groupSchedule.Location = New-Object System.Drawing.Point($column2X, $currentY_Col2); $groupSchedule.Size = New-Object System.Drawing.Size($column2Width, 180) # Höhe weiter reduziert
...
# ---------------------------------------------------------------------------------
# Start Geplante Replikation und Backup und Klon mit CRON + 2-Weekly
#----------------------------------------------------------------------------------
...
$buttonSaveSchedule.Add_Click({
    ...
    try {
        ...
        try {
            ...
            # --- START DER KORREKTUR: Logik für gerade/ungerade Wochen ---
            ...
            switch ($selectedMode) {
                "BIWEEKLY_EVEN" {
                    ...
                    # Führt den Befehl nur aus, wenn das Ergebnis von (Wochennummer % 2) gleich 0 ist.
                    $finalExecutableCommand = '[ $(expr $(date +\%V) \% 2) -eq 0 ] && ' + $commandToRun
                }
                "BIWEEKLY_ODD" {
                    Write-GuiLog "-> 2-wöchentlicher Zeitplan (ungerade KW) ausgewählt."
                    # Führt den Befehl nur aus, wenn das Ergebnis von (Wochennummer % 2) ungleich 0 ist.
                    $finalExecutableCommand = '[ $(expr $(date +\%V) \% 2) -ne 0 ] && ' + $commandToRun
                }
                "MONTHLY_1" {
                ...
                default {
                    ...
                    # $finalExecutableCommand ist bereits korrekt
                }
            }
            # --- ENDE DER KORREKTUR ---
