...
# - FIX: Master-Helper-Skript (V5.1.0) mit speziellem Diagnose-Schritt
...
$form.Text = "ESXi & GhettoVCB Manager V5.1.0"
...
# -------------Netzwerk-Monitor-Abschnitt ersetzen
...
$buttonFirewallCheck.Add_Click({
    if (-not ($Global:ESXiSession -and $Global:ESXiSession.Connected)) {
        Write-GuiLog "FEHLER: Für die Firewall-Einrichtung muss eine Verbindung zum ESXi-Host bestehen."; return
    }

    $form.Cursor = [System.Windows.Forms.Cursors]::WaitCursor
    try {
        # --- TEIL 1: SMTP-Regel für E-Mail (bestehende Logik) ---
        Write-GuiLog "Prüfe Firewall-Regel für E-Mail (SMTP)..."
        $fwSmtpResult = Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command "esxcli network firewall ruleset list | grep -i smtps"
        $smtpRuleLine = $fwSmtpResult.Output | Where-Object { $_ -match 'smtps' } | Select-Object -First 1

        if ($smtpRuleLine -and $smtpRuleLine -match 'true$') {
            Write-GuiLog " -> SMTP-Regel 'smtps' ist bereits AKTIV."
        } elseif ($smtpRuleLine -and $smtpRuleLine -match 'false$') {
            Write-GuiLog " -> SMTP-Regel 'smtps' ist DEAKTIVIERT. Aktiviere sie..."
            ...
            Write-GuiLog " -> SMTP-Regel erfolgreich aktiviert!"
        } else {
            Write-GuiLog " -> SMTP-Regel 'smtps' nicht gefunden. Keine Aktion durchgeführt."
        }

        # --- TEIL 2: Neue Replikations-Regel (für vmkfstools | ssh dd) ---
        Write-GuiLog "Prüfe Firewall-Regel für direkte Replikation (sshClient)..."
        $replRuleId = "sshClient"
        $checkReplRuleResult = Invoke-SSHCommand -SSHSession $Global:ESXiSession -Command "esxcli network firewall ruleset get --ruleset-id=$replRuleId"

        if ($checkReplRuleResult.ExitStatus -eq 0) {
             $ruleEnabled = ($checkReplRuleResult.Output | Where-Object {$_ -match "Enabled"} | Out-String) -match "true"
             if ($ruleEnabled) {
                 Write-GuiLog " -> Replikations-Regel '$replRuleId' ist bereits AKTIV."
             } else {
                 Write-GuiLog " -> Replikations-Regel '$replRuleId' ist DEAKTIVIERT. Aktiviere sie jetzt..."
                 ...
                 if ($enableResult.ExitStatus -eq 0) {
                     Write-GuiLog " -> Regel '$replRuleId' erfolgreich aktiviert."
                 } else {
                     Write-GuiLog " -> FEHLER beim Aktivieren der Regel '$replRuleId'."
                 }
             }
        } else {
            Write-GuiLog " -> Standard-Firewallregel '$replRuleId' konnte nicht geprüft werden. Dies ist unerwartet."
        }
         [System.Windows.Forms.MessageBox]::Show("Die Firewall-Prüfung für E-Mail und Replikation wurde durchgeführt.", "Firewall-Check abgeschlossen", "OK", "Information")
    } catch {
...
#!/bin/sh
# GhettoGUI Master Replication Helper V3.1.0 (Exakter VM-Name via "["-Split + Originalzeile im Log)
set -o pipefail
...
# [1/10] Exakter Match via "["-Split
log "[1/10] Finde VMID und VMX-Pfad..."
log "--- Ausgabe von 'vim-cmd vmsvc/getallvms' ---"
vim-cmd vmsvc/getallvms >> ${LOG_FILE}
log "--- Ende der VM-Liste ---"
log "Suche exakten Match für: ${SOURCE_VM_NAME}"

VM_INFO_LINE=$(vim-cmd vmsvc/getallvms | awk -v name="${SOURCE_VM_NAME}" '
{
    split($0, parts, "\\[");              # Alles nach erstem "[" abschneiden
    vmline = parts[1];                    # Linken Teil nehmen
    sub(/^[0-9]+[ \t]+/, "", vmline);     # VMID entfernen
    sub(/[ \t]+$/, "", vmline);           # Leerzeichen am Ende entfernen
    if (vmline == name) { print $0 }
}')

if [ -z "${VM_INFO_LINE}" ]; then
    log "FEHLER: VM '${SOURCE_VM_NAME}' nicht gefunden!"
    exit 1
fi

# NEU: Originalzeile aus getallvms ins Log
log "Gefundene VM-Zeile: ${VM_INFO_LINE}"
...
log "  -> OK: VMID=${VMID}, VMX_DIR=${VMX_DIR}"

log "[2/10] Erstelle Snapshot..."
vim-cmd vmsvc/snapshot.create ${VMID} "ghetto-repl-${UNIQUE_ID}" "GhettoGUI Replication" ${SNAP_MEM} ${SNAP_QUIESCE} >> ${LOG_FILE} 2>&1
sleep 5

log "[3/10] Erstelle Zielverzeichnis..."
...
if [ $? -ne 0 ]; then
    log "FEHLER: Konnte Zielverzeichnis auf ${TARGET_HOST} nicht erstellen!"
    exit 1
fi
log "  -> OK: Zielverzeichnis bereit."
...
(
    ...
    if [ "${REPLICATION_METHOD}" = "tar" ]; then
        ...
        for line in ${DISK_LIST}; do
            ...
            TEMP_CLONE_DISK_NAME=$(echo "${DISK_FILE_NAME}" | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/')
            TEMP_CLONE_DISK_PATH="${TEMP_CLONE_DIR}/${TEMP_CLONE_DISK_NAME}"
            log "    -> Klone lokal: ${SOURCE_DISK_PATH}"
            vmkfstools -i "${SOURCE_DISK_PATH}" -d thin "${TEMP_CLONE_DISK_PATH}" >> "${LOG_FILE}" 2>&1
        done
        cd "${VMX_DIR}" || exit
        find . -maxdepth 1 ! -name '*.vmdk' -exec cp -p '{}' "${TEMP_CLONE_DIR}/" \;
        cd "${TEMP_CLONE_DIR}" || exit
        tar -cf - . | ssh ${SSH_OPTIONS} root@${TARGET_HOST} "tar -xf - -C '${TARGET_VM_PATH}'"
        rm -rf "${TEMP_CLONE_DIR}"
    fi
    ssh ${SSH_OPTIONS} root@${TARGET_HOST} "touch /tmp/ghetto_repl_${UNIQUE_ID}_done.flag"
) &

log "[5/10] Überwache Netzwerktransfer..."
...
while ! ssh ${SSH_OPTIONS} root@${TARGET_HOST} "[ -f /tmp/ghetto_repl_${UNIQUE_ID}_done.flag ]"; do
    ...
    if [ -n "${progress}" ]; then
        log " -> ${progress} von ~${SOURCE_SIZE} "
    fi
    sleep 15
done
log "  -> OK: Datentransfer erfolgreich."

log "[6/10] Lösche Snapshot..."
vim-cmd vmsvc/snapshot.removeall ${VMID} >> ${LOG_FILE} 2>&1

log "[7/10] Passe VMX-Datei auf Ziel-Host an..."
ssh ${SSH_OPTIONS} root@${TARGET_HOST} /bin/sh << 'EOF_VMX'
set -e
cd "__TARGET_VM_PATH__"
ORIGINAL_VMX_FILE=$(find . -maxdepth 1 -name "*.vmx")
ORIGINAL_VMDK_IN_VMX=$(grep -iE '^scsi.*\.fileName' "${ORIGINAL_VMX_FILE}" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p' | head -n 1)
...
TARGET_VMX_FILE="./__REPLICATED_VM_NAME__.vmx"
TEMP_VMX_FILE="${TARGET_VMX_FILE}.tmp"
sed \
    ...
    -e '/snapshot\./d' -e '/\.latest/d' -e '/sched\.swap\.derivedName/d' \
    -e '/uuid\./d' -e '/vc\.uuid/d' \
    -e '/ethernet[0-9]\+\.generatedAddress/d' \
    -e "s/ethernet[0-9]\+\.addressType = \"vpx\"/ethernet0.addressType = \"static\"/" \
    "${TARGET_VMX_FILE}" > "${TEMP_VMX_FILE}"
mv "${TEMP_VMX_FILE}" "${TARGET_VMX_FILE}"
EOF_VMX
log "  -> OK: VMX-Anpassung abgeschlossen."

log "[8/10] Registriere VM..."
TARGET_VMX_PATH_ON_TARGET="${TARGET_VM_PATH}/__REPLICATED_VM_NAME__.vmx"
REG_OUTPUT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "vim-cmd solo/registervm '${TARGET_VMX_PATH_ON_TARGET}'")
log "  -> OK: VM registriert mit ID: $REG_OUTPUT"

log "[9/10] Prüfe und führe Konsolidierung auf Ziel-VM aus..."
...
'@
    ...
    $finalMasterScript = $masterHelperScriptTemplate `
        ...
        -replace '__REPLICATED_VM_NAME__', "$($params.SourceVmName)$($params.Suffix)"
