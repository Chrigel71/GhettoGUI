# GhettoGUI_V7.6.1.6.ps1
#
# --- VERSION V7.6.1.6 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V7.6.1.6 / 24.09.2025"
...
#!/bin/sh
# GhettoGUI Multi-VM Replication Helper V45.2.0 (Golden Master Final)
# - FINAL: Completely reworked based on user feedback.
# - The script now only logs per-VM size and does not calculate a faulty total.
# - A critical 'sed' bug that corrupted VMX files has been fixed.
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
log_raw() { echo "$1" >> ${LOG_FILE}; }
log_error() { echo "$(date '+%Y-%m-%d %H:%M:%S') -- ERROR: $1" >> ${LOG_FILE}; }
send_email_notification() { DURATION_MSG="$1"; if [ -z "${DURATION_MSG}" ]; then DURATION_MSG="N/A"; fi; DIRECTORY_LISTING_CONTENT=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "ls -lR '${TARGET_DATASTORE}'/*${VM_SUFFIX}" 2>/dev/null < /dev/null); log_raw "--- START Backup Directory Listing ---"; echo "${DIRECTORY_LISTING_CONTENT}" >> ${LOG_FILE}; log_raw "--- END Backup Directory Listing ---"; if [ "${OVERALL_STATUS}" = "OK" ]; then FINAL_STATUS_MSG="## Final status: All VMs backed up OK! ##"; else FINAL_STATUS_MSG="## Final status: ERROR: Replication failed! ##"; fi; log "Bereite E-Mail vor: ${OVERALL_STATUS}"; log_raw "Backup Duration: ${DURATION_MSG}"; log_raw "${FINAL_STATUS_MSG}"; if [ "${EMAIL_ENABLED}" = "1" ] && [ -f "${SENDMAIL_PATH}" ]; then RECIPIENTS=$(echo "${EMAIL_TO}" | sed 's/,/ /g'); /bin/python ${SENDMAIL_PATH} -f "${EMAIL_FROM}" -s "${EMAIL_SERVER}" -S "${EMAIL_PORT}" -j "${EMAIL_SUBJECT} - ${FINAL_STATUS_MSG}" -m "${LOG_FILE}" -u "${EMAIL_USER}" -p "${EMAIL_PASS}" ${RECIPIENTS} >> ${LOG_FILE} 2>&1; fi; }
cleanup() { trap - EXIT; EXIT_CODE=$?; if [ "${EXIT_CODE}" -ne 0 ] && [ "${OVERALL_STATUS}" != "ERROR" ]; then log_error "FEHLER erkannt (Exit Code: ${EXIT_CODE})."; OVERALL_STATUS="ERROR"; fi; log_raw "--- ENDE DES DETAILLOGS ---"; END_TIME_S=$(date '+%Y-%m-%d %H:%M:%S'); log_raw "Endzeit: ${END_TIME_S}"; END_TIME=$(date +%s); DURATION=$((END_TIME - START_TIME)); MINUTES=$((DURATION / 60)); SECONDS=$((DURATION % 60)); DURATION_STRING="${MINUTES} Minuten, ${SECONDS} Sekunden"; send_email_notification "${DURATION_STRING}"; log "====== MULTI-VM REPLICATION HELPER BEENDET (ID: ${UNIQUE_ID}) ======"; exit 0; }

# --- Skriptstart ---
mkdir -p "${GHETTO_PATH}/logs"; rm -f ${LOG_FILE}
START_TIME=$(date +%s); trap cleanup EXIT
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log "====== MULTI-VM REPLICATION HELPER GESTARTET (ID: ${UNIQUE_ID}) ======"
log_raw "Job-Konfiguration:"; log_raw "  - Typ: Direkte Replikation (Host-zu-Host)"; log_raw "  - Ziel-Host: ${TARGET_HOST}"; log_raw "  - Ziel-Datastore: ${TARGET_DATASTORE}"; log_raw "  - Methode: ${REPLICATION_METHOD}"; log_raw "  - VM-Suffix: ${VM_SUFFIX}"
...
for SOURCE_VM_NAME in ${CLEAN_VM_LIST}; do
    ...
    VMID=$(echo "${VM_INFO_LINE}" | awk '{print $1}'); VMX_FULL_PATH=$(echo "${VM_INFO_LINE}" | sed -e 's/.*\[\(.*\)\]\s*\(.*\.vmx\).*/\/vmfs\/volumes\/\1\/\2/'); VMX_DIR=$(dirname "${VMX_FULL_PATH}");
    log "[1/7] Bereinige alte Replikation..."; TARGET_VMID=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd vmsvc/getallvms | awk -v name=\"${REPLICATED_VM_NAME}\" '{ n=split(\$0, a, \"\\[\"); vm=a[1]; sub(/^[0-9]+[ \t]+/, \"\", vm); sub(/[ \t]+$/, \"\", vm); if (vm == name) print \$1; }'" < /dev/null)
    if [ -n "${TARGET_VMID}" ]; then ssh ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd vmsvc/power.off ${TARGET_VMID} >/dev/null 2>&1 || true; sleep 5; /bin/vim-cmd vmsvc/unregister ${TARGET_VMID} >/dev/null 2>&1" < /dev/null; fi
    ...
    if [ "${REPLICATION_METHOD}" = "robust" ]; then
        log "[3/7] Starte ONLINE Replikation..."; log "[4/7] Erstelle Snapshot..."; /bin/vim-cmd vmsvc/snapshot.create ${VMID} "ghetto-repl-${UNIQUE_ID}" "GhettoGUI Replication" ${SNAP_MEM} ${SNAP_QUIESCE}; log "  -> Warte 10s..."; sleep 10
        TEMP_CLONE_DIR="${VMX_DIR}/ghetto_clone_${UNIQUE_ID}"; if [ -n "${TEMP_CLONE_BASE_PATH}" ]; then TEMP_CLONE_DIR="${TEMP_CLONE_BASE_PATH}/ghetto_clone_${UNIQUE_ID}"; fi; rm -rf "${TEMP_CLONE_DIR}"; mkdir -p "${TEMP_CLONE_DIR}"
        DISK_DEFINITIONS=$(grep -iE '(\.vmdk)' "${VMX_FULL_PATH}" | grep -iE '(^scsi|^ide|^sata|^nvme)');
        echo "${DISK_DEFINITIONS}" | while read -r line; do ORIGINAL_DISK_BASENAME=$(echo "$line" | sed -n 's/.*"\(.*\.vmdk\)".*/\1/p' | sed 's/-[0-9]\{6\}\.vmdk/\.vmdk/'); if [ -z "${ORIGINAL_DISK_BASENAME}" ]; then continue; fi; if echo "${ORIGINAL_DISK_BASENAME}" | grep -q "^/"; then SOURCE_DISK_PATH_FOR_CLONE="${ORIGINAL_DISK_BASENAME}"; else SOURCE_DISK_PATH_FOR_CLONE="${VMX_DIR}/${ORIGINAL_DISK_BASENAME}"; fi; DESTINATION_DISK_BASENAME=$(basename "${ORIGINAL_DISK_BASENAME}"); /sbin/vmkfstools -i "${SOURCE_DISK_PATH_FOR_CLONE}" -d thin "${TEMP_CLONE_DIR}/${DESTINATION_DISK_BASENAME}" >> ${LOG_FILE} 2>&1; done
        (cd "${VMX_DIR}" && find . -maxdepth 1 ! -name '*.vmdk' -exec cp -p '{}' "${TEMP_CLONE_DIR}/" \;) 
        log "-> Lokaler Klon abgeschlossen. Lösche Snapshot..."; /bin/vim-cmd vmsvc/snapshot.removeall ${VMID}
        log "[5/7] Starte TAR-Transfer zum Ziel-Host..."; 
        { (cd "${TEMP_CLONE_DIR}" && tar -cf - .) | ssh ${SSH_OPTIONS} root@${TARGET_HOST} "tar -xf - -C '${TARGET_VM_PATH}'"; } && rm -rf "${TEMP_CLONE_DIR}"
    fi
    log "[6/7] Passe Zieldateien an..."; ssh ${SSH_OPTIONS} root@${TARGET_HOST} "cd '${TARGET_VM_PATH}'; ORIG_VMX_FILE=\$(find . -maxdepth 1 -name \"*.vmx\"); RENAMED_VMX_FILE=\"./${REPLICATED_VM_NAME}.vmx\"; mv \"\${ORIG_VMX_FILE}\" \"\${RENAMED_VMX_FILE}\"; sed -i \"s/displayName = .*/displayName = \\\"${REPLICATED_VM_NAME}\\\"/\" \"\${RENAMED_VMX_FILE}\"; sed -i \"s/${SOURCE_VM_NAME}\.vmdk/${REPLICATED_VM_NAME}\.vmdk/g\" \"\${RENAMED_VMX_FILE}\"; sed -i 's|\(fileName = \"\)/.*/\(.*\.vmdk\"\)|\\1\\2|g' \"\${RENAMED_VMX_FILE}\"" < /dev/null
    log "[7/7] Registriere VM..."; ssh ${SSH_OPTIONS} root@${TARGET_HOST} "/bin/vim-cmd solo/registervm '${TARGET_VM_PATH}/${REPLICATED_VM_NAME}.vmx'" < /dev/null
    log "-> Verarbeitung für ${SOURCE_VM_NAME} abgeschlossen."
    
    SIZE=$(ssh ${SSH_OPTIONS} root@${TARGET_HOST} "du -sh '${TARGET_VM_PATH}'" 2>/dev/null | awk '{print \$1}')
    log_raw "${REPLICATED_VM_NAME} (Groesse: ${SIZE:-N/A})"
done
'@
