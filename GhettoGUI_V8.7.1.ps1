# GhettoGUI_V8.7.1.ps1
#
# --- VERSION V8.7.1 (Final Stable Release by Gemini & Chrigel#71)
...
$form.Text = "GhettoGUI ESXi & GhettoVCB Manager V8.7.1 / 31.01.2026"
        ...
        if ($confirmInstall -eq [System.Windows.Forms.DialogResult]::Yes) {
            try {
                ...
                if (-not (Get-PackageProvider -Name NuGet -ErrorAction SilentlyContinue)) {
                    ...
                    Install-PackageProvider -Name NuGet -MinimumVersion 2.8.7.101 -Force | Out-Null
...
$buttonKillOrphans.Add_Click({
    ...
    $confirmResult = [System.Windows.Forms.MessageBox]::Show(
        "WARNUNG: Dieser Vorgang analysiert laufende Prozesse und beendet GhettoVCB-Jobs, Klon-Vorgänge (vmkfstools) und Replikationen (scp)."+
        "`n`nSind Sie sicher?",
        "Bereinigung starten",
        [System.Windows.Forms.MessageBoxButtons]::YesNo,
...
# VERSION: V8.7.1 (Fix: Log-Filename for Live-View & Stable Logic)
# =======================================================================
...
log_raw "  - Typ: Manueller Multi-VM Hot-Klon V8.7.1"
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manuell Single-VM Hot-Klon V8.7.1"
...
for SOURCE_VM_NAME in ${VM_LIST}; do
    ...
    log "#########8.7.1########### Starte Klon für VM: ${SOURCE_VM_NAME} ####################"
    log "initiate backup for ${SOURCE_VM_NAME}"
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"; log_raw "  - Typ: Geplante GhettoVCB Wiederherstellung V8.7.1"; log_raw "  - Quelle: ${SOURCE_PATH}"; log_raw "  - Ziel-Datastore: ${TARGET_DATASTORE}"; log_raw "  - Neuer VM-Name: ${NEW_VM_NAME}";
log_raw "--- START DES DETAILLOGS ---"
...
#!/bin/sh
# GhettoGUI Scheduled Multi-VM Clone Helper V8.7.1 (Fix: SCSI-ID Based Naming)
PATH=/bin:/sbin:/usr/bin:/usr/sbin:/usr/lib/vmware/bin
export PATH
...
log_raw "Startzeit: $(date '+%Y-%m-%d %H:%M:%S')"
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manueller Restore V8.7.1"
...
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Geplante Replikation (H2H V8.7.1)"
...
log_raw "Job-Konfiguration:"
log_raw "  - Typ: Manuelle Replikation (H2H) V8.7.1"
log_raw "  - Methode: ${REPLICATION_METHOD}"
...
function Stop-GhettoVCBBackupJob {
    ...
    $confirmResult = [System.Windows.Forms.MessageBox]::Show(
        "ACHTUNG: Dies beendet alle laufenden GhettoVCB-Prozesse, Klon-Vorgänge und SCP-Transfers HART (Kill -9).`n`nNicht gespeicherte Daten in laufenden Backups gehen verloren.`n`nWollen Sie den Abbruch erzwingen?",
        "Backup Not-Aus",
        [System.Windows.Forms.MessageBoxButtons]::YesNo,
        [System.Windows.Forms.MessageBoxIcon]::Warning
    )
